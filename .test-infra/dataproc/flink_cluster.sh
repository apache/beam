...
function create_cluster() {
  ...
  if [[ -n "${HIGH_MEM_MACHINE:=}" ]]; then
    ...
    --properties="flink:taskmanager.memory.task.heap.size=2g,flink:taskmanager.memory.managed.size=2g,flink:taskmanager.memory.process.size=8g,flink:taskmanager.jvm-overhead.min=256mb, flink:taskmanager.jvm-overhead.max=4gb"
  else
