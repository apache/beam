...
function start_tunnel() {
  ...
  local jobmanager_rpc_port=`echo $job_server_config | python3 -c "import sys, json; print([e['value'] for e in json.load(sys.stdin) if e['key'] == u'$key'][0])"`
...
function create_cluster() {
  ...
  gcloud dataproc clusters create $CLUSTER_NAME --region=global --num-workers=$num_dataproc_workers  --metadata "${metadata}", --image-version=$image_version --zone=$GCLOUD_ZONE  --optional-components=FLINK --initialization-actions $DOCKER_INIT --quiet
}
