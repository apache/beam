...
FINAL_REPORT = os.path.join(
    WORKING_SPACE, "/src/.test-infra/jenkins/metrics_report/beam-metrics_report.html"
)
PERF_DASHBOARDS = os.path.join(
    WORKING_SPACE, "/src/.test-infra/metrics/grafana/dashboards/perftests_metrics/"
)
...
def get_retention_policies_names(client, database):
    return (
        i.get("name") for i in client.get_list_retention_policies(database=database)
    )
