...
tasks.register<TerraformTask>("setPlaygroundFunctionDelUrl") {
    group = "deploy"

    dependsOn("terraformInit")
    dependsOn("terraformRef")

    args("output", "playground_function_del_url")
    standardOutput = ByteArrayOutputStream()

    doLast {
        project.rootProject.extra["playground_function_del_url"] =
            standardOutput.toString().trim().replace("\"", "")
    }
}
...
tasks.register("takeConfig") {
    ...
    dependsOn("setPlaygroundFunctionDelUrl")
    ...
    doLast {
        ...
        val pgfuncdel = project.rootProject.extra["playground_function_del_url"]
