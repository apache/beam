...
func Test_getApplicationEnvsFromOsEnvs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "CacheKeyExpirationTimeKey is set with the correct value",
			want:      NewApplicationEnvs("/app", defaultLaunchSite, defaultProjectId, defaultPipelinesFolder, defaultBucketName, defaultPlaygroundSalt, defaultDatastoreEmulatorHost, defaultOrigin, defaultSDKConfigPath, &CacheEnvs{defaultCacheType, defaultCacheAddress, defaultCacheKeyExpirationTime}, convertedTime, defaultDBType, defaultMaxSnippetSize, defaultIdLength),
			wantErr:   false,
