...
async def test_verify_examples(mock_get_statuses, mock_verify_examples):
    helper = CIHelper()
    await helper.verify_examples([], Origin.PG_EXAMLPES)
...
async def test__verify_examples(mock_get_compile_output, mock_get_run_output):
    ...
    with pytest.raises(VerifyException):
        await helper._verify_examples(examples_with_errors, Origin.PG_EXAMLPES)
    with pytest.raises(VerifyException):
        await helper._verify_examples(examples_without_def_ex, Origin.PG_EXAMLPES)
    with pytest.raises(VerifyException):
        await helper._verify_examples(examples_with_several_def_ex, Origin.PG_EXAMLPES)
    await helper._verify_examples(examples_without_errors, Origin.PG_EXAMLPES)
