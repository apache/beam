...
public class BeamUnionRel extends Union implements BeamRelNode {
  ...
  @Override public PCollection<BeamSQLRow> buildBeamPipeline(BeamPipelineCreator planCreator)
      throws Exception {
    List<RelNode> inputRelNodes = getInputs();
    PCollection<BeamSQLRow> leftRows = BeamSQLRelUtils.getBeamRelInput(
        inputRelNodes.get(0)).buildBeamPipeline(planCreator);
    PCollection<BeamSQLRow> rightRows = BeamSQLRelUtils.getBeamRelInput(
        inputRelNodes.get(1)).buildBeamPipeline(planCreator);
    ...
    if (!leftWindow.isCompatible(rightWindow)) {
      throw new IllegalArgumentException("inputs of Union have different window strategy: "
          + leftWindow + " VS " + rightWindow);
    }
    PCollectionList<BeamSQLRow> rowPCollectionList = PCollectionList.of(leftRows)
        .and(rightRows);
    return rowPCollectionList.apply(Flatten.<BeamSQLRow>pCollections());
  }
}
