...
public class BeamMinusRel extends Minus implements BeamRelNode {
  ...
  @Override public PCollection<BeamSQLRow> buildBeamPipeline(BeamPipelineCreator planCreator)
    ...
    PCollection<KV<BeamSQLRow, CoGbkResult>> coGbkResultCollection = KeyedPCollectionTuple
        .of(leftTag, leftRows.apply("CreateLeftIndex",
            MapElements.via(new BeamSQLRow2KvFn(false))))
        .and(rightTag, rightRows.apply("CreateRightIndex",
            MapElements.via(new BeamSQLRow2KvFn(false))))
    ...
    PCollection<BeamSQLRow> ret = coGbkResultCollection
        .apply(ParDo.of(new SetOperatorFilteringDoFn(leftTag, rightTag,
            SetOperatorFilteringDoFn.OpType.MINUS, all)));
    return ret;
  }
}
