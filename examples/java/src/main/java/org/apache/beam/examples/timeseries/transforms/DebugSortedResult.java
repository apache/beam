        ...
        ParDo.of(new DoFn<KV<TimeSeriesData.TSKey, Iterable<TimeSeriesData.TSAccum>>, String>() {
          ...
          @ProcessElement public void process(ProcessContext c) {
            ...
            for (TimeSeriesData.TSAccum accum : accums) {
              ...
              sb.append("KEY : " + c.element().getKey().getMajorKey() + c.element().getKey()
                  .getMinorKeyString() + "\n");
              sb.append("\n- Is this a Heart Beat Value?  " + heartBeat + "\n" +

                  "- Window Start " + Timestamps.toString(accum.getLowerWindowBoundary()) + "\n"
                  + "- Window End " + Timestamps.toString(accum.getUpperWindowBoundary())
                  + ((heartBeat) ?
                  ...
                  + "- Count = " + accum.getDataAccum().getCount() + "\n" + "- Min = " + accum
                  .getDataAccum().getMinValue() + "- Max = " + accum.getDataAccum().getMaxValue()
                  + "\n" + "- Previous Upper Window Boundary = " + Timestamps
                  .toString(accum.getPreviousWindowValue().getUpperWindowBoundary()) + "\n"
                  + "- Previous_Min = " + accum.getPreviousWindowValue().getDataAccum()
                  .getMinValue() + "- Previous_Max = " + accum.getPreviousWindowValue()
                  .getDataAccum().getMaxValue());
              sb.append("\n\n");
            }
