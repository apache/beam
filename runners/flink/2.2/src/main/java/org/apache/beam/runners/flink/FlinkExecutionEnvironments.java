...
public class FlinkExecutionEnvironments {
  ...
  static StreamExecutionEnvironment createBatchExecutionEnvironment(
    ...
    if ("[local]".equals(flinkMasterHostPort)) {
      ...
      flinkBatchEnv = StreamExecutionEnvironment.createLocalEnvironment(flinkConfiguration);
      ...
      throw new UnsupportedOperationException(
          "CollectionEnvironment has been removed in Flink 2. Use [local] instead.");
      ...
      if (!options.getAttachedMode()) {
        flinkConfiguration.set(DeploymentOptions.ATTACHED, options.getAttachedMode());
      }
      flinkBatchEnv =
  ...
  private static void configureCheckpointing(
    ...
    if (checkpointInterval != -1) {
      ...
      if (options.getShutdownSourcesAfterIdleMs() == -1) {
        // If not explicitly configured, we never shutdown sources when checkpointing is disabled.
  ...
  private static void configureStateBackend(FlinkPipelineOptions options, Configuration config) {
    if (options.getStateBackend() != null) {
      ...
      config.set(StateBackendOptions.STATE_BACKEND, options.getStateBackendFactory().getName());
    }
  }

  private static void configureRestartStrategy(FlinkPipelineOptions options, Configuration config) {
    ...
    if (numRetries != -1) {
      ...
      config.set(RestartStrategyOptions.RESTART_STRATEGY_FIXED_DELAY_ATTEMPTS, numRetries);
    }
  ...
  private static void setManagedMemoryByFraction(final Configuration config) {
    if (!config.containsKey("taskmanager.memory.managed.size")) {
      float managedMemoryFraction = config.get(TaskManagerOptions.MANAGED_MEMORY_FRACTION);
