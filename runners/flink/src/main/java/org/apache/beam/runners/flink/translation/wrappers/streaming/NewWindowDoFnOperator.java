...
public class NewWindowDoFnOperator<K, InputT, OutputT>
  ...
  protected DoFnRunner<KeyedWorkItem<K, InputT>, KV<K, OutputT>> createWrappingDoFnRunner(
    ...
    return DoFnRunners.lateDataDroppingRunner(doFnRunner, timerInternals, windowingStrategy);
  }

  @Override
  protected PushbackSideInputDoFnRunner<KeyedWorkItem<K, InputT>, KV<K, OutputT>>
      createPushbackSideInputDoFnRunner(
          DoFnRunner<KeyedWorkItem<K, InputT>, KV<K, OutputT>> doFnRunner,
          Collection<PCollectionView<?>> views,
          SideInputHandler sideInputHandler) {
    return new SingletonKeyedWorkItemPushbackSideInputDoFnRunner<>(
        doFnRunner, views, sideInputHandler);
  }
  ...
  protected DoFn<KeyedWorkItem<K, InputT>, KV<K, OutputT>> getDoFn() {
    ...
    final WindowingStrategy<?, BoundedWindow> castWindowStrategy =
        (WindowingStrategy<?, BoundedWindow>) windowingStrategy;
    return GroupAlsoByWindowViaWindowSetNewDoFn.create(
        castWindowStrategy,
  ...
  protected void fireTimer(TimerData timer) {
    @SuppressWarnings("unchecked")
    final K key = (K) keyedStateInternals.getKey();
