...
public class FlinkExecutionEnvironmentsTest {
  ...
  public void shouldSetParallelismBatch() {
    ...
    StreamExecutionEnvironment bev =
  ...
  public void useDefaultParallelismFromContextBatch() {
    ...
    assertThat(bev, instanceOf(LocalStreamEnvironment.class));
  ...
  public void shouldParsePortForRemoteEnvironmentBatch() {
    ...
    assertThat(bev, instanceOf(RemoteStreamEnvironment.class));
  ...
  private String extractFlinkConfig() throws IOException {
    InputStream inputStream = getClass().getResourceAsStream("/flink-test-config.yaml");
    File root = temporaryFolder.getRoot();
    Files.copy(inputStream, new File(root, "config.yaml").toPath());
  ...
  public void shouldCreateFileSystemStateBackend() {
    ...
    assertEquals("hashmap", sev.getConfiguration().get(StateBackendOptions.STATE_BACKEND));
  }
  ...
  public void shouldCreateRocksDbStateBackend() {
    ...
    assertEquals("rocksdb", sev.getConfiguration().get(StateBackendOptions.STATE_BACKEND));
  }
  ...
  private void checkHostAndPort(Object env, String expectedHost, int expectedPort) {
    String host =
        ((Configuration) Whitebox.getInternalState(env, "configuration")).get(RestOptions.ADDRESS);
    int port =
        ((Configuration) Whitebox.getInternalState(env, "configuration")).get(RestOptions.PORT);
    assertThat(
