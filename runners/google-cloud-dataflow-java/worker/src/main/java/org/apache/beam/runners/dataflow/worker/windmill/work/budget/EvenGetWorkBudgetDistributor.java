...
/** Evenly distributes the provided budget across the available {@link HasGetWorkBudget}(s). */
@Internal
final class EvenGetWorkBudgetDistributor implements GetWorkBudgetDistributor {
  ...
  @Override
  public <T extends HasGetWorkBudget> void distributeBudget(
    ...
    for (Entry<T, GetWorkBudget> streamAndDesiredBudget : desiredBudgets.entrySet()) {
      HasGetWorkBudget hasGetWorkBudget = streamAndDesiredBudget.getKey();
      GetWorkBudget desired = streamAndDesiredBudget.getValue();
      GetWorkBudget remaining = hasGetWorkBudget.remainingBudget();
      if (isBelowFiftyPercentOfTarget(remaining, desired)) {
        GetWorkBudget adjustment = desired.subtract(remaining);
        hasGetWorkBudget.adjustBudget(adjustment);
      }
    }
  }

  private <T extends HasGetWorkBudget> ImmutableMap<T, GetWorkBudget> computeDesiredBudgets(
