...
public class StreamingEngineConfigLoaderTest {
  private final WorkUnitClient mockDataflowServiceClient = mock(WorkUnitClient.class);
  private StreamingEngineConfigLoader streamingEngineConfigLoader;

  private StreamingEngineConfigLoader createConfigLoader(
      boolean waitForInitialConfig,
      long globalConfigRefreshPeriod,
      Consumer<StreamingEnginePipelineConfig> onPipelineConfig) {
    return StreamingEngineConfigLoader.forTesting(
  ...
  public void testGetComputationConfig() throws IOException {
    ...
    Optional<StreamingEnginePipelineConfig> actualPipelineConfig =
        streamingEngineConfigLoader.getComputationConfig(computationId);

    assertTrue(actualPipelineConfig.isPresent());
    assertThat(receivedPipelineConfig)
        .containsExactly(
            StreamingEnginePipelineConfig.builder()
                .setComputationConfigs(ImmutableList.of(pipelineConfig))
                .build());
  }
  ...
  public void testGetComputationConfig_noComputationPresent() throws IOException {
    ...
    Optional<StreamingEnginePipelineConfig> pipelineConfig =
        streamingEngineConfigLoader.getComputationConfig("someComputationId");
