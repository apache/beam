 ...
 */
class OrFinallyReactor extends TriggerReactor {

  private static final int ACTUAL = 0;
  private static final int UNTIL = 1;

  @VisibleForTesting
  OrFinallyReactor(TriggerReactor actual, TriggerReactor.OnceTriggerReactor until) {
  ...
  public void onFire(TriggerReactor.TriggerContext context) throws Exception {
    ExecutableTriggerReactor actualSubtrigger = context.trigger().subTrigger(ACTUAL);
    ExecutableTriggerReactor untilSubtrigger = context.trigger().subTrigger(UNTIL);
