...
import org.apache.beam.runners.core.reactors.TriggerReactor.MergingTriggerInfo;
import org.apache.beam.runners.core.reactors.TriggerReactor.TriggerInfo;
 ...
 */
public class TriggerReactorContextFactory<W extends BoundedWindow> {

  private final WindowFn<?, W> windowFn;
  private StateInternals<?> stateInternals;
  private final Coder<W> windowCoder;

  public TriggerReactorContextFactory(WindowFn<?, W> windowFn,
  ...
  public TriggerReactor.TriggerContext base(W window, Timers timers,
      ExecutableTriggerReactor rootTrigger, FinishedTriggers finishedSet) {
    return new TriggerContextImpl(window, timers, rootTrigger, finishedSet);
  }

  public TriggerReactor.OnElementContext createOnElementContext(
  ...
  public TriggerReactor.OnMergeContext createOnMergeContext(W window, Timers timers,
      ExecutableTriggerReactor rootTrigger, FinishedTriggers finishedSet,
      Map<W, FinishedTriggers> finishedSets) {
    return new OnMergeContextImpl(window, timers, rootTrigger, finishedSet, finishedSets);
  }

  public StateAccessor<?> createStateAccessor(W window, ExecutableTriggerReactor trigger) {
    return new StateAccessorImpl(window, trigger);
  }

  public MergingStateAccessor<?, W> createMergingStateAccessor(
      W mergeResult, Collection<W> mergingWindows, ExecutableTriggerReactor trigger) {
    return new MergingStateAccessorImpl(trigger, mergingWindows, mergeResult);
  }

  private class TriggerInfoImpl implements TriggerReactor.TriggerInfo {

    protected final ExecutableTriggerReactor trigger;
    protected final FinishedTriggers finishedSet;
    private final TriggerReactor.TriggerContext context;

    public TriggerInfoImpl(ExecutableTriggerReactor trigger, FinishedTriggers finishedSet,
    ...
    @Override
    public Iterable<ExecutableTriggerReactor> subTriggers() {
      return trigger.subTriggers();
    }

    @Override
    public ExecutableTriggerReactor subTrigger(int subtriggerIndex) {
    ...
    @Override
    public Iterable<ExecutableTriggerReactor> unfinishedSubTriggers() {
      return FluentIterable
          .from(trigger.subTriggers())
          .filter(new Predicate<ExecutableTriggerReactor>() {
            @Override
            public boolean apply(ExecutableTriggerReactor trigger) {
              return !finishedSet.isFinished(trigger);
            }
          });
    }
    ...
    @Override
    public ExecutableTriggerReactor firstUnfinishedSubTrigger() {
  ...
  private class MergingTriggerInfoImpl
      extends TriggerInfoImpl implements TriggerReactor.MergingTriggerInfo {
  ...
  implements MergingStateAccessor<Object, W> {
    ...
    public MergingStateAccessorImpl(
        ExecutableTriggerReactor trigger, Collection<W> activeToBeMerged, W mergeResult) {
  ...
  private class TriggerContextImpl extends TriggerReactor.TriggerContext {
    ...
    @Override
    public TriggerReactor.TriggerContext forTrigger(ExecutableTriggerReactor trigger) {
  ...
  private class OnElementContextImpl extends TriggerReactor.OnElementContext {
    ...
    @Override
    public TriggerReactor.OnElementContext forTrigger(ExecutableTriggerReactor trigger) {
  ...
  private class OnMergeContextImpl extends TriggerReactor.OnMergeContext {
    ...
    @Override
    public TriggerReactor.OnMergeContext forTrigger(ExecutableTriggerReactor trigger) {
