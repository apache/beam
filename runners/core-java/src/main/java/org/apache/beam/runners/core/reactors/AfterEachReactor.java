...
/**
 * A composite {@link TriggerReactor} that executes its sub-triggers in order.
 ...
 * </ul>
 */
public class AfterEachReactor extends TriggerReactor {

  private AfterEachReactor(List<TriggerReactor> subTriggers) {
    super(subTriggers);
   ...
   */
  @SafeVarargs
  public static TriggerReactor inOrder(TriggerReactor... triggers) {
    return new AfterEachReactor(Arrays.<TriggerReactor>asList(triggers));
  }
  ...
  public boolean shouldFire(TriggerReactor.TriggerContext context) throws Exception {
    ExecutableTriggerReactor firstUnfinished = context.trigger().firstUnfinishedSubTrigger();
