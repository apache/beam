 ...
 *
 * <ul>
 * <li> {@link AfterWatermarkReactor} for firing when the watermark passes a timestamp determined
 *     from either the end of the window or the arrival of the first element in a pane.
 * <li> {@link AfterProcessingTimeReactor} for firing after some amount of processing time has
 *     elapsed (typically since the first element in a pane).
 * <li> {@link AfterPaneReactor} for firing off a property of the elements in the current pane, such
 *     as the number of elements that have been assigned to the current pane.
 * </ul>
 *
 * <p>In addition, {@code Trigger}s can be combined in a variety of ways:
 *
 * <ul>
 * <li> {@link RepeatedlyReactor#forever} to create a trigger that executes forever. Any time its
 *     argument finishes it gets reset and starts over. Can be combined with {@link
 *     TriggerReactor#orFinally} to specify a condition that causes the repetition to stop.
 * <li> {@link AfterEachReactor#inOrder} to execute each trigger in sequence, firing each (and
 *     every) time that a trigger fires, and advancing to the next trigger in the sequence when it
 *     finishes.
 * <li> {@link AfterFirstReactor#of} to create a trigger that fires after at least one of its
 *     arguments fires. An {@link AfterFirstReactor} trigger finishes after it fires once.
 * <li> {@link AfterAllReactor#of} to create a trigger that fires after all least one of its
 *     arguments have fired at least once. An {@link AfterAllReactor} trigger finishes after it
 *     fires once.
 * </ul>
 *
 ...
 * callback returns.
 */
public abstract class TriggerReactor implements Serializable {
  ...
  public interface TriggerInfo {
     ...
     * possibility of {@link TriggerReactor#onMerge} being called. If false, {@link
     * TriggerReactor#onMerge} will never be called.
     */
    boolean isMerging();
     ...
     */
    Iterable<ExecutableTriggerReactor> subTriggers();

    /**
     * Access the executable version of the specified sub-trigger.
     */
    ExecutableTriggerReactor subTrigger(int subtriggerIndex);
     ...
     */
    Iterable<ExecutableTriggerReactor> unfinishedSubTriggers();

    /**
     * Returns the first unfinished sub-trigger.
     */
    ExecutableTriggerReactor firstUnfinishedSubTrigger();
   ...
   *
   * <p>Used directly in {@link TriggerReactor#shouldFire} and {@link TriggerReactor#clear}, and
  ...
  public abstract class TriggerContext {
    ...
    public abstract TriggerContext forTrigger(ExecutableTriggerReactor trigger);
  ...
  public abstract class OnElementContext extends TriggerContext {
    ...
    @Override
    public abstract OnElementContext forTrigger(ExecutableTriggerReactor trigger);
  }
  ...
  public abstract class OnMergeContext extends TriggerContext {
    ...
    @Override
    public abstract OnMergeContext forTrigger(ExecutableTriggerReactor trigger);
  ...
  @Nullable
  protected final List<TriggerReactor> subTriggers;

  protected TriggerReactor(@Nullable List<TriggerReactor> subTriggers) {
   ...
   * ExecutableTriggerReactor#invokeOnMerge} on their sub-triggers, and applying appropriate logic.
   *
   * <p>A trigger such as {@link AfterWatermarkReactor#pastEndOfWindow} may no longer be finished;
   * it is the responsibility of the trigger itself to record this fact. It is forbidden for a
   * trigger to become finished due to {@link #onMerge}, as it has not yet fired the pending
   ...
   * {@link RepeatedlyReactor} trigger will reset its inner trigger, since it has fired.
   *
  ...
  public void prefetchOnElement(StateAccessor<?> state) {
    if (subTriggers != null) {
      for (TriggerReactor trigger : subTriggers) {
  ...
  public void clear(TriggerContext c) throws Exception {
    if (subTriggers != null) {
      for (ExecutableTriggerReactor trigger : c.trigger().subTriggers()) {
        trigger.invokeClear(c);
      }
    }
  }

  public Iterable<TriggerReactor> subTriggers() {
    return subTriggers;
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof TriggerReactor)) {
      return false;
    }
    TriggerReactor that = (TriggerReactor) obj;
   ...
   *
   * <p>For example the final firing of the following trigger may only have 1 element: <pre> {@code
   * Repeatedly.forever(AfterPane.elementCountAtLeast(2))
   *     .orFinally(AfterPane.elementCountAtLeast(5))
   * } </pre>
   *
   * <p>Note that if {@code t1} is {@link OnceTriggerReactor}, then {@code t1.orFinally(t2)} is the
   * same as {@code AfterFirst.of(t1, t2)}.
   */
  public TriggerReactor orFinally(OnceTriggerReactor until) {
    return new OrFinallyReactor(this, until);
  }

  /**
   * {@link TriggerReactor}s that are guaranteed to fire at most once should extend from this,
   * rather than the general {@link TriggerReactor} class to indicate that behavior.
   */
  public abstract static class OnceTriggerReactor extends TriggerReactor {
    protected OnceTriggerReactor(List<TriggerReactor> subTriggers) {
      super(subTriggers);
    }
