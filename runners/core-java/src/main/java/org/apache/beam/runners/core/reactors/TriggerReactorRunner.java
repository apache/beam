 ...
 *
 * <ul>
 *   <li>Invoking the trigger's methods via its {@link ExecutableTriggerReactor} wrapper by
 ...
 * and commits the information when the method of the {@link ExecutableTriggerReactor} returns.
 *
 * @param <W> The kind of windows being processed.
 */
public class TriggerReactorRunner<W extends BoundedWindow> {
  @VisibleForTesting
  ...
  private final ExecutableTriggerReactor rootTrigger;
  private final TriggerReactorContextFactory<W> contextFactory;

  public TriggerReactorRunner(
      ExecutableTriggerReactor rootTrigger, TriggerReactorContextFactory<W> contextFactory) {
  ...
  public void processValue(W window, Instant timestamp, Timers timers, StateAccessor<?> state)
    ...
    TriggerReactor.OnElementContext triggerContext = contextFactory.createOnElementContext(
  ...
  public void onMerge(W window, Timers timers, MergingStateAccessor<?, W> state) throws Exception {
    ...
    TriggerReactor.OnMergeContext mergeContext = contextFactory.createOnMergeContext(
  ...
  public boolean shouldFire(W window, Timers timers, StateAccessor<?> state) throws Exception {
    ...
    TriggerReactor.TriggerContext context = contextFactory.base(window, timers,
  ...
  private boolean isFinishedSetNeeded() {
    ...
    return !(rootTrigger.getSpec() instanceof DefaultTriggerReactor);
  }
}
