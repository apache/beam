...
public class StatefulDoFnRunnerTest {
  ...
  public void setup() {
    ...
    MetricsContainer mc = new MetricsContainer("test_step");
    MetricsEnvironment.setCurrentContainer(mc);
  ...
  public void testLateDropping() throws Exception {
    ...
    DoFnRunner<KV<String, Integer>, Integer> runner = DoFnRunners.defaultStatefulDoFnRunner(
        fn, getDoFnRunner(fn), mockStepContext, null, WINDOWING_STRATEGY);

    runner.startBundle();

    IntervalWindow window = new IntervalWindow(new Instant(0), new Instant(0L + WINDOW_SIZE));
    Instant timestamp = new Instant(0);
    CounterCell droppedDueToLateness = MetricsEnvironment.getCurrentContainer().getCounter(
        MetricName.named("main", StatefulDoFnRunner.DROPPED_DUE_TO_LATENESS_COUNTER));
    runner.processElement(
        WindowedValue.of(KV.of("hello", 1), timestamp, window, PaneInfo.NO_FIRING));
    assertEquals(1L, droppedDueToLateness.getCumulative().longValue());

    runner.onTimer("processTimer", window, timestamp, TimeDomain.PROCESSING_TIME);
    assertEquals(2L, droppedDueToLateness.getCumulative().longValue());

    runner.onTimer("synchronizedProcessTimer", window, timestamp,
        TimeDomain.SYNCHRONIZED_PROCESSING_TIME);
    assertEquals(3L, droppedDueToLateness.getCumulative().longValue());
