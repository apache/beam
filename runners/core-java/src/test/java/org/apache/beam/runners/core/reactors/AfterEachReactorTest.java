...
/**
 * Tests for {@link AfterEachReactor}.
 */
@RunWith(JUnit4.class)
public class AfterEachReactorTest {
  ...
  /**
   * Tests that the {@link AfterEachReactor} trigger fires and finishes the first trigger then the
   * second.
   */
  @Test
    ...
    tester = TriggerReactorTester.forTrigger(
        AfterEachReactor.inOrder(
            RepeatedlyReactor.forever(AfterPaneReactor.elementCountAtLeast(2))
                .orFinally(AfterPaneReactor.elementCountAtLeast(3)),
  ...
  public void testToString() {
    TriggerReactor trigger = AfterEachReactor.inOrder(
        StubTriggerReactor.named("t1"),
        StubTriggerReactor.named("t2"),
        StubTriggerReactor.named("t3"));
