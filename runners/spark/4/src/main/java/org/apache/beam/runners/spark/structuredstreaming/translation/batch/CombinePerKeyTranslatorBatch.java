...
class CombinePerKeyTranslatorBatch<K, InT, AccT, OutT>
  ...
  public void translate(Combine.PerKey<K, InT, OutT> transform, Context cxt) {
    ...
    if (globalGroupBy || groupByWindow) {
      ...
      if (globalGroupBy) {
        // Drop window and group by key globally to run the aggregation (combineFn),
        // afterwards the
        ...
        result =
            cxt.getDataset(cxt.getInput())
                ...
                .mapValues(fun1(t -> t._2()), inputEnc)
                .agg(valueAgg.toColumn())
                .map(windowedKV(), wvOutputEnc);
      }
    } else {
      // Optimized aggregator for non-merging and session window functions, all others
      // depend on
