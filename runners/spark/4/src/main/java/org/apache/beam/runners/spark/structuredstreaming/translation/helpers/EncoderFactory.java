...
public class EncoderFactory {
  ...
  static <T> ExpressionEncoder<T> create(
    ...
    try {
      // Try Spark 4.x approach: ExpressionEncoder may have factory method or
      // different constructor
      // First attempt: try static apply/create method
      try {
        java.lang.reflect.Method applyMethod =
            ExpressionEncoder.class.getMethod(
                "apply",
                org.apache.spark.sql.catalyst.encoders.AgnosticEncoder.class,
                org.apache.spark.sql.catalyst.encoders.AgnosticEncoder.class,
                ClassTag.class);
        // Need to wrap expressions - try using ExpressionEncoder's internal methods
        Object serializerWrapper =
            Class.forName("org.apache.spark.sql.catalyst.encoders.ExpressionEncoder$FlatEncoder")
        ...
        return (ExpressionEncoder<T>)
            applyMethod.invoke(null, serializerWrapper, deserializerWrapper, ClassTag.apply(clazz));
      } catch (NoSuchMethodException | ClassNotFoundException e) {
        // Spark 4.x doesn't support direct Expression-based ExpressionEncoder
        // constructor
        throw new RuntimeException(
            "Failed to create ExpressionEncoder for Spark 4.x. "
                + "The required FlatEncoder class or apply method was not found.",
            e);
      }
    } catch (Exception e) {
      throw new RuntimeException("Failed to create ExpressionEncoder for Spark 4.x", e);
    }
  }
  ...
  private static Expression invoke(
    ...
    try {
      ...
      // expressions are
      // created reflectively. This is fine as it's just needed once to create the
      // query plan.
