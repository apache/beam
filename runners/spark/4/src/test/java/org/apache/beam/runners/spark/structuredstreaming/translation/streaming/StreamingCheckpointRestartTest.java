...
import org.apache.beam.runners.spark.structuredstreaming.io.streaming.BeamSourceCheckpoint;
 ...
 * directory. Spark hands the source the per source location {@code <query checkpoint>/sources/0},
 * where {@link BeamSourceCheckpoint} pins the split list and writes one mark file per split and
 * epoch. {@link BeamReaderCache#invalidateAll} is called between the two runs to simulate a fresh
 * JVM: every in-memory reader and pending mark is dropped, forcing the second run's readers to
 * restore from the durable marks the first run wrote.
 *
 ...
 * {@link BeamSourceCheckpoint} and {@link BeamReaderCache}: Spark may replay the last micro-batch
 ...
 * regression to in-memory-only marks would look like.
 */
...
public class StreamingCheckpointRestartTest implements Serializable {
  ...
  public void restartedPipelineResumesFromDurableCheckpointMarks() throws Exception {
    ...
    // The durable layout of the first run must exist under the location Spark hands the source,
    // <checkpointDir>/<leaf index>/sources/<source index>: the pinned split list and at least one
    // persisted checkpoint mark.
    File sourceRoot = new File(new File(checkpointPath, "0"), "sources/0");
    assertTrue("expected the source checkpoint directory " + sourceRoot, sourceRoot.isDirectory());
  ...
  private PipelineResult runPipeline(String checkpointPath, String collectorId) {
    ...
    // One element per split per micro-batch (a batch limit below the split count still gives every
    // split one record): the 10 elements are spread across several micro-batches instead of a
    // single one, so even a replayed in-flight batch can only ever carry a small tail of the range.
