...
interface SparkInputDataProcessor<FnInputT, FnOutputT, OutputT> {
  ...
  /**
   * @return {@link OutputManager} to be used by {@link org.apache.beam.runners.core.DoFnRunner} for emitting processing results
   */
   ...
   */
  <K> Iterable<OutputT> process(Iterator<WindowedValue<FnInputT>> input, SparkProcessContext<K, FnInputT, FnOutputT> ctx);

  /**
   * Creates a synchronous {@link SparkInputDataProcessor} which does processing in calling thread.
   * It is doing so by processing fully input element and then iterating over the output retrieved from that processing.
   * The result of processing one element fit into memory.
   */
  static <FnInputT, FnOutputT> SparkInputDataProcessor<FnInputT, FnOutputT, Tuple2<TupleTag<?>, WindowedValue<?>>> createSync() {
    return new SyncSparkInputDataProcessor<>();
  }

  /**
   * Create and asynchronous {@link SparkInputDataProcessor} which does process input elements in separate thread and observes
   * produced outputs asynchronously. This does not require results of processing one element to fit into the memory.
   */
  static <FnInputT, FnOutputT> SparkInputDataProcessor<FnInputT, FnOutputT, Tuple2<TupleTag<?>, WindowedValue<?>>> createAsync() {
    return new AsyncSparkInputDataProcessor<>();
  }
}


class SyncSparkInputDataProcessor<FnInputT, FnOutputT> implements SparkInputDataProcessor<FnInputT, FnOutputT, Tuple2<TupleTag<?>, WindowedValue<?>>> {
  ...
  @Override
  public <K> Iterable<Tuple2<TupleTag<?>, WindowedValue<?>>> process(Iterator<WindowedValue<FnInputT>> input, SparkProcessContext<K, FnInputT, FnOutputT> ctx) {
    if (!input.hasNext()) {
  ...
  private class SyncInOutIterator<K> extends AbstractIterator<Tuple2<TupleTag<?>, WindowedValue<?>>> {
...
class AsyncSparkInputDataProcessor<FnInputT, FnOutputT> implements SparkInputDataProcessor<FnInputT, FnOutputT, Tuple2<TupleTag<?>, WindowedValue<?>>> {
  ...
  /**
   * Output manager which can hold limited number of output elements. If capacity is reached, then attempt to output
   * more elements will block until some elements are consumed.
   */
  private class BlockingDoFnOutputManager
          implements OutputManager, Iterable<Tuple2<TupleTag<?>, WindowedValue<?>>> {

    private final LinkedBlockingQueue<Tuple2<TupleTag<?>, WindowedValue<?>>> queue = new LinkedBlockingQueue<>(500);
  ...
  private class AsyncInOutIterator<K, InputT, OutputT> extends AbstractIterator<Tuple2<TupleTag<?>, WindowedValue<?>>> {
    ...
    private final ExecutorService executorService = Executors.newSingleThreadScheduledExecutor(
            new ThreadFactoryBuilder().setNameFormat("async-in/out-iterator-%d").setDaemon(true).build()
    );
    private Future<?> consumeTask = null;

    private volatile RuntimeException inputConsumeFailure = null;
    private boolean isBundleStarted = false;
    ...
    protected Tuple2<TupleTag<?>, WindowedValue<?>> computeNext() {
      ...
      if (consumeTask == null) {
        consumeTask = executorService.submit(() -> {
