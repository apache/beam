...
def parse_arguments():
  ...
  Return: github_token, user_github_id, repo_url, release_branch, release_commit, artifacts_dir, is_rc_version
  """
  ...
  is_rc_version = args.is_rc_version
...
def filter_artifacts(artifacts, is_rc_version):
  def filter_source(artifact_name):
    if is_rc_version:
      return artifact_name.startswith("source_zip_rc")
    return artifact_name.startswith("source_zip") and "_rc" not in artifact_name

  def filter_wheels(artifact_name):
    if is_rc_version:
      return artifact_name.startswith("wheelhouse-rc")
...
def fetch_github_artifacts(run_id, repo_url, artifacts_dir, github_token, is_rc_version):
  ...
  filtered_artifacts = filter_artifacts(artifacts, is_rc_version)
...
if __name__ == "__main__":
  ...
  try:
    ...
    fetch_github_artifacts(run_id, repo_url, artifacts_dir, github_token, is_rc_version)
