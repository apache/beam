...
public class WordCount {
  ...
  private static void runBigQueryIOStorageWritePipeline(BigQueryOptions bqOptions, int rowCount, WriteMode writeMode) {
    ...
    try {
      ...
      LOG.info("!!!!" + response.getRows().get(0).getF().get(0).getV());
    } catch (Exception e) {
      LOG.info(e.toString());
  ...
  public static void main(String[] args) {
    ...
    BigQueryOptions bqOptions = PipelineOptionsFactory.fromArgs(args).withValidation().as(BigQueryOptions.class);
    ...
    runBigQueryIOStorageWritePipeline(bqOptions, 3000000, WriteMode.EXACT_ONCE);
  }
}
