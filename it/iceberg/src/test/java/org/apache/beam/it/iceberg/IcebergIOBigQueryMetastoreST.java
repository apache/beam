...
/**
 * IcebergIO stress tests using BigQueryMetastore catalog. The test is designed to assess the performance of
 * IcebergIO under various conditions.
 */
public final class IcebergIOBigQueryMetastoreST extends IOStressTestBase {
  ...
  private static final BigqueryClient BQ_CLIENT = new BigqueryClient("IcebergIOBigQueryMetastoreST");
  static final String DATASET = "managed_iceberg_bqms_load_tests_" + System.nanoTime();;
  ...
  public void setup() throws Exception {
    ...
    String tableId = String.format("%s.%s_%s", TEST_DB, test.getMethodName(), testConfigName);
    String warehouse =
  ...
  private PipelineLauncher.LaunchInfo generateDataAndWrite() throws IOException {
    ...
    source.apply("Iceberg[BQMS] Write", Managed.write(Managed.ICEBERG).withConfig(managedConfig));
