...
/**
 * IcebergIO stress tests using Hive catalog. The test is designed to assess the performance of
 * IcebergIO under various conditions. The test spins up a Hive Metastore and shuts it down in the
 * end.
 */
public final class IcebergIOHiveST extends IOStressTestBase {
  ...
  public void setup() throws Exception {
    ...
    String tableId = String.format("%s.%s", TEST_DB, test.getMethodName());
    String warehouse =
    ...
    managedConfig =
        ImmutableMap.<String, Object>builder()
            ...
            .put(
                "config_properties",
                ImmutableMap.of("hive.metastore.uris", hiveMetastoreExtension.metastoreUri()))
            .build();
  }
  ...
  private PipelineLauncher.LaunchInfo generateDataAndWrite() throws IOException {
    ...
    Schema schema = Schema.builder().addInt32Field("num").addByteArrayField("bytes").build();
            ...
            .apply(
                ParDo.of(
                    new DoFn<KV<byte[], byte[]>, Row>() {
                      ...
                      @ProcessElement
                      public void process(
                          @Element KV<byte[], byte[]> element, OutputReceiver<Row> out) {
                        int mod = count++ % NUM_PARTITIONS;
                        out.output(
                            Row.withSchema(schema).addValues(mod, element.getValue()).build());
                      }
                    }))
            .setRowSchema(schema);
    ...
    source.apply("Iceberg[Hive] Write", Managed.write(Managed.ICEBERG).withConfig(managedConfig));

    PipelineLauncher.LaunchConfig options =
        PipelineLauncher.LaunchConfig.builder("write-iceberg-hive")
  ...
  private PipelineLauncher.LaunchInfo readData() throws IOException {
    readPipeline
        .apply("Read from Iceberg", Managed.read(Managed.ICEBERG).withConfig(managedConfig))
        .getSinglePCollection()
        .apply("Counting element", ParDo.of(new CountingFn<>(READ_ELEMENT_METRIC_NAME)));

    PipelineLauncher.LaunchConfig options =
        PipelineLauncher.LaunchConfig.builder("read-iceberg-hive")
            .setSdk(PipelineLauncher.Sdk.JAVA)
  ...
  static class Configuration extends SyntheticSourceOptions {
    ...
    @JsonProperty public String influxMeasurement = IcebergIOHiveST.class.getName();
