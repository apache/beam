...
public class ByteSizeUtilsTest {
  ...
  @Test
  public void testParsePlainByteCount() {
    assertThat(parseSizeToBytes("0")).isEqualTo(0L);
    assertThat(parseSizeToBytes("16")).isEqualTo(16L);
    assertThat(parseSizeToBytes("1024")).isEqualTo(1024L);
  }

  @Test
  public void testParseSuffixesAreBinary() {
    assertThat(parseSizeToBytes("1K")).isEqualTo(KB);
    assertThat(parseSizeToBytes("64K")).isEqualTo(64 * KB);
  ...
  @Test
  public void testParseIgnoresCaseAndPadding() {
    assertThat(parseSizeToBytes(" 40g ")).isEqualTo(40 * GB);
  }

  @Test
  public void testParseRejectsUnknownFormats() {
    // The "GB" spelling is deliberately not supported, only a single letter suffix is.
    assertThrows(IllegalArgumentException.class, () -> parseSizeToBytes("40GB"));
    assertThrows(IllegalArgumentException.class, () -> parseSizeToBytes("1.5G"));
  ...
  public void testFormatBytes() {
    ...
    assertThat(formatBytes(2 * KB)).isEqualTo("2,048 B (2.00 KB)");
    assertThat(formatBytes(40 * GB)).isEqualTo("42,949,672,960 B (40.00 GB)");
    // Anything beyond the largest unit keeps scaling in that unit rather than overflowing it.
    assertThat(formatBytes(2 * TB)).isEqualTo("2,199,023,255,552 B (2.00 TB)");
  }
