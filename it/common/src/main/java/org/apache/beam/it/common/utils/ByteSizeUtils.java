...
public final class ByteSizeUtils {
  ...
  /** Binary unit suffixes accepted by {@link #parseSizeToBytes}, in increasing order. */
  private static final String SIZE_SUFFIXES = "KMGT";
   ...
   * of {@value #SIZE_SUFFIXES}, e.g. {@code "1024"}, {@code "64K"}, {@code "500M"} or {@code
   * "40G"}. The suffixes are binary, i.e. {@code 1K} is 1024 bytes.
   *
   * @param sizeStr the size to parse, case insensitive and possibly padded with spaces
  ...
  public static long parseSizeToBytes(String sizeStr) {
    String size = sizeStr.trim().toUpperCase(Locale.ROOT);
    int suffix = size.isEmpty() ? -1 : SIZE_SUFFIXES.indexOf(size.charAt(size.length() - 1));
    try {
      if (suffix < 0) {
        return Long.parseLong(size);
      }
      // K is 1024^1, M is 1024^2, and so on.
      long multiplier = 1L << (10 * (suffix + 1));
      return Long.parseLong(size.substring(0, size.length() - 1)) * multiplier;
    } catch (NumberFormatException e) {
      throw new IllegalArgumentException(
          "Invalid size: '"
              + sizeStr
              + "'. Expected a number of bytes, optionally suffixed with one of "
              + SIZE_SUFFIXES
              + ", e.g. '1024', '64K', '500M' or '40G'.",
          e);
    }
  }
