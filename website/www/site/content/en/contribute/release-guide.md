...
1. Test updated [staging environment](https://play-dev.beam.apache.org/) in a browser
    1. Open the menu (represented by '...' in the right top corner) and click on Versions. Validate that commit is the same for all listed containers, and hash belongs to a [recent master branch commit](https://github.com/apache/beam/commits/master)
...
1. Perform the same steps for "Deploy-Update-Playground-environment-prod" trigger as for "Deploy-Update-Playground-environment-stg" trigger
1. Test updated [prod environment](https://play.beam.apache.org/) in a browser. The process is similar to the staging environment.
1. Find the trigger "Playground-CI-stable"
    ...
    1. Click Save button. The window with settings must close without any errors
