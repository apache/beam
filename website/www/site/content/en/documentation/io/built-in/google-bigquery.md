...
[load jobs](https://cloud.google.com/bigquery/docs/batch-loading-data) and
...
[`WriteToBigQuery`](https://beam.apache.org/releases/pydoc/{{< param release_latest >}}/apache_beam.io.gcp.bigquery.html?highlight=apache_beam%20io%20gcp%20bigquery#apache_beam.io.gcp.bigquery.WriteToBigQuery)
for the list of the available methods and their restrictions.
{{< /paragraph >}}
...
shards written, or use `withAutoSharding` to enable dynamic sharding and the 
number of shards may be determined and changed at runtime. The sharding behavior
...
{{< paragraph class="language-py" >}}
You can set `with_auto_sharding=True` to enable dynamic sharding and the 
number of shards may be determined and changed at runtime. The sharding behavior
depends on the runners.
{{< /paragraph >}}
...
{{< paragraph class="language-java" wrap="span">}}
***Note:*** Streaming inserts by default enables BigQuery [best-effort 
deduplication mechanism](https://cloud.google.com/bigquery/streaming-data-into-bigquery#disabling_best_effort_de-duplication).
...
use `withAutoSharding` to enable dynamic sharding and the number of shards may 
be determined and changed at runtime. The sharding behavior depends on the 
runners.
...
Streaming inserts applies a default sharding for each table destination. You can
set `with_auto_sharding=True` to enable dynamic sharding and the number of 
shards may be determined and changed at runtime. The sharding behavior depends 
on the runners.
