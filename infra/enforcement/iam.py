...
class IAMPolicyComplianceChecker:
    def __init__(self, project_id: str, users_file: str, logger: logging.Logger):
...
# limitations under the License.
    ...
    def create_github_issue(self) -> None:
        ...
        if not diff:
            self.logger.info("No compliance issues found, no GitHub issue will be created.")
            return
...
def main():
    ...
    iam_checker = IAMPolicyComplianceChecker(config["project_id"], config["users_file"], logger)
    ...
    try:
        if action == "check":
            ...
            iam_checker.create_github_issue()
