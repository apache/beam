...
public class NexmarkUtils {
  ...
  public static enum QpsShape {
     ...
     * to follow in order to achieve {@code qps} using {@code numGenerators}.
     */
    public long interEventDelayUs(int qps, int numGenerators) {
      return (1_000_000L * numGenerators) / qps;
    }

    /**
     * Return array of successive inter-event delays, in microseconds, for each generator
     * to follow in order to achieve this shape with {@code firstQps/nextQps} using
     * {@code numGenerators}.
     */
    public long[] interEventDelayUs(int firstQps, int nextQps, int numGenerators) {
      if (firstQps == nextQps) {
        long[] interEventDelayUs = new long[1];
        interEventDelayUs[0] = (1_000_000L * numGenerators) / firstQps;
        ...
        case SQUARE:
          {
            ...
            interEventDelayUs[0] = (1_000_000L * numGenerators) / firstQps;
            interEventDelayUs[1] = (1_000_000L * numGenerators) / nextQps;
            return interEventDelayUs;
          }
        case SINE:
          {
            double mid = (firstQps + nextQps) / 2.0;
            double amp = (firstQps - nextQps) / 2.0; // may be -ve
            long[] interEventDelayUs = new long[N];
            for (int i = 0; i < N; i++) {
              double r = (2.0 * Math.PI * i) / N;
              double qps = mid + amp * Math.cos(r);
              interEventDelayUs[i] = (1_000_000L * numGenerators) / Math.round(qps);
            }
            return interEventDelayUs;
          }
      }
     ...
     * as to cycle through the entire sequence every {@code qpsPeriodSec}.
     */
    public int stepLengthSec(int qpsPeriodSec) {
      int n = 0;
      switch (this) {
        case SQUARE:
          n = 2;
  ...
  public static PTransform<PInput, PCollection<Event>> streamEventsSource(
    ...
    return Read.from(new UnboundedEventSource(NexmarkUtils.standardGeneratorConfig(configuration),
                         configuration.numEventGenerators, configuration.watermarkHoldbackSec,
                         configuration.isRateLimited)).named(name + ".ReadUnbounded");
  }
  ...
  public static ParDo.Bound<Event, Event> snoop(final String name) {
    return ParDo.named(name + ".Snoop")
        .of(new DoFn<Event, Event>() {
          final Aggregator<Long, Long> eventCounter = createAggregator("events", new SumLongFn());
          ...
          final Aggregator<Long, Long> bidCounter = createAggregator("bids", new SumLongFn());
  ...
  public static <T> ParDo.Bound<T, T> log(final String name) {
    return ParDo.named(name + ".Log")
        .of(new DoFn<T, T>() {
          @Override
          public void processElement(ProcessContext c) {
            error("%s result: %s", name, c.element());
            c.output(c.element());
          }
        });
  }
  ...
  public static <T> ParDo.Bound<T, String> format(String name) {
    return ParDo.named(name + ".Format")
        .of(new DoFn<T, String>() {
          final Aggregator<Long, Long> recordCounter = createAggregator("records", new SumLongFn());
    ...
    return new PTransform<PCollection<T>, PCollection<Long>>(name) {
      ...
      public PCollection<Long> apply(PCollection<T> input) {
        return input.apply(Window.<T>into(new GlobalWindows())
            ...
            .apply(ParDo.named("Hash").of(new DoFn<T, Long>() {
              @Override
              public void processElement(ProcessContext c) {
                long hash =
                    Hashing.murmur3_128()
