...
* [IcebergIO] Added batch and streaming CDC writes that applies INSERT/UPDATE_BEFORE/UPDATE_AFTER/DELETE changes to Iceberg V2+ tables by primary key. Invoke with `IcebergIO.writeCdcRows` (Java) or by setting the `cdc` option of Managed `ICEBERG` write (Java, Python, YAML) ([#X](https://github.com/apache/beam/issues/X)).
