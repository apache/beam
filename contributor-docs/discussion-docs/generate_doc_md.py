...
You can download these manually or modify the script to automate the download process.
...
a table of discussion documents with their authors, subjects, and submission dates.

Note:
The script currently extracts links to Google Docs and Apache short links (s.apache.org).
Ensure you have the necessary libraries installed (e.g., requests, bs4, mailbox).
...
def convert_to_md_table(email_messages: list[EmailMessage], year: int):
    ...
    with open(output_file, "w") as f:
        ...
        for eid, email in enumerate(email_messages):
            ...
            row_no = f'<a name="row_{eid+1}">{eid+1}</a>'
            f.write(
