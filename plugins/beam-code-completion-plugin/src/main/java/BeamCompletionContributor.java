    ...
    public void fillCompletionVariants(@NotNull final CompletionParameters parameters, @NotNull CompletionResultSet result) {
        if (AFTER_METHOD_CALL_PATTERN.accepts(parameters.getPosition())){
            try {
                ...
                String fullCode = parameters.getOriginalFile().getText();
                String codeToComplete = fullCode.substring(0, offset);

                Parameters modelParameters = new Parameters(0.1F, false);
                RequestBody reqBodyObj = new RequestBody(codeToComplete, modelParameters);

                String requestBodyJson = new Gson().toJson(reqBodyObj);
                String modelUrl = System.getenv().getOrDefault("MODEL_URL", "https://api-inference.huggingface.co/models/bigcode/starcoder");
