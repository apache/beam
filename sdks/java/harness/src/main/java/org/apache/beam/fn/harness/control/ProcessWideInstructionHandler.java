 ...
 * BeamFnApi.MonitoringInfosMetadataRequest} and {@link BeamFnApi.HarnessMonitoringInfosResponse}
 *
...
public class ProcessWideInstructionHandler {
  ...
  public static BeamFnApi.InstructionResponse.Builder harnessMonitoringInfos(
      BeamFnApi.InstructionRequest request) {
    ...
    if (container != null) {
      for (MetricsApi.MonitoringInfo info : container.getMonitoringInfos()) {
        response.putMonitoringData(
            MonitoringInfoShortIdCache.getShortIdCache().getShortId(info), info.getPayload());
      }
    }
