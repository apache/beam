...
public class SplittableTruncateSizedRestrictionsDoFnRunner<
  ...
  private final Coder<KV<KV<InputT, KV<RestrictionT, WatermarkEstimatorStateT>>, Double>>
      inputCoder;
  ...
  SplittableTruncateSizedRestrictionsDoFnRunner(
    ...
    // Main Input
    this.mainInputId = pTransform.getInputsOrThrow(ParDoTranslation.getMainInputName(pTransform));
    RunnerApi.PCollection mainInput = components.getPcollectionsOrThrow(mainInputId);
  ...
  private HandlesSplits.@Nullable SplitResult trySplitForWindowObservingTruncateRestriction(
    ...
    Coder<WindowedValue<?>> fullInputCoder =
        (Coder<WindowedValue<?>>) (Object) WindowedValue.getFullCoder(inputCoder, windowCoder);
