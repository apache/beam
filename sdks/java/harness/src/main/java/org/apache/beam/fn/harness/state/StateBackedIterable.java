...
public class StateBackedIterable<T> implements Iterable<T>, Serializable {
  ...
  public StateBackedIterable(
      ...
      List<T> prefix) {
    StateKey.Runner stateKey = StateKey.Runner.newBuilder().setKey(runnerKey).build();
    StateRequest.Builder requestBuilder = StateRequest.newBuilder();
    requestBuilder.setInstructionId(instructionId).getStateKeyBuilder().setRunner(stateKey);
