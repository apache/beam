...
/**
 * {@link DataStreamDecoder} treats multiple {@link ByteString}s as a single input stream producing
 * decoded values while {@link #outbound(OutputChunkConsumer)} treats a single {@link OutputStream}
 * producing multiple {@link ByteString}s.
 */
@SuppressWarnings({
    ...
    public boolean isReady() {
      switch (currentState) {
        ...
        case READ_REQUIRED:
          try {
            if (inbound.isEof()) {
              return inputByteStrings.isReady();
            }
            return true;
          } catch (IOException e) {
