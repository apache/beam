...
public class IncrementalChangelogSourceTest {
   ...
   * advancing past one snapshot never discards an earlier one's records.
   */
  @Test
  public void consecutiveSnapshotsThroughShuffleDropNoRecords() throws Exception {
    assertNoRecordsDroppedAcrossSnapshots(false);
  }

  /** Same, but driven by the per-snapshot watermark from {@link WatchForSnapshotsSdf}. */
  @Test
  public void consecutiveSnapshotsThroughShuffleDropNoRecordsWhenStreaming() throws Exception {
    assertNoRecordsDroppedAcrossSnapshots(true);
  }

  private void assertNoRecordsDroppedAcrossSnapshots(boolean streaming) throws Exception {
    TableIdentifier tableId = tableId();
    // SPLIT_SIZE=1 forces every bi-directional group onto the CoGroupByKey path rather than
    // LocalResolveDoFn, so the windowing/lateness behaviour is what's under test.
    Table table =
        warehouse.createTable(
            tableId,
            CDC_SCHEMA,
    ...
    DataFile v1 = commitAppend(table, "v1.parquet", ImmutableList.of(record(1L, "v1")));
    DataFile v2 = commitOverwrite(table, "v2.parquet", v1, record(1L, "v2"));
    commitOverwrite(table, "v3.parquet", v2, record(1L, "v3"));
