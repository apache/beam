...
public class MetadataUpdate implements IndexedRecord, SchemaConstructable {
  ...
  public MetadataUpdate(StructType partitionType,
    ...
    Map<StructType,String> dataFileNames = ImmutableMap.of(
    ...
    Map<StructType,String> deleteFileNames = ImmutableMap.of(
    ...
    this.avroSchema = SchemaBuilder.builder().record(getClass().getName())
        .fields()
        .name("dataFiles")
        .prop(AvroSchemaUtil.FIELD_ID_PROP,"-1")
        .type().nullable().array().items(dataFileSchema).noDefault()
        .name("deleteFiles")
        .prop(AvroSchemaUtil.FIELD_ID_PROP,"-1")
        .type().nullable().array().items(deleteFileSchema).noDefault()
        .endRecord();
  }
  ...
  public List<DataFile> getDataFiles() { return this.dataFiles; }
  public List<DeleteFile> getDeleteFiles() { return this.deleteFiles; }
  ...
  protected static class MetadataUpdateCoder extends Coder<MetadataUpdate> {
    ...
    @Override
    public void encode(MetadataUpdate value,
