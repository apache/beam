...
public class AddFiles extends PTransform<PCollection<String>, PCollectionRowTuple> {
  ...
  public PCollectionRowTuple expand(PCollection<String> input) {
    ...
    if (evolution.isEnabled()) {
      ...
      if (evolution.getDryRun()) {
        return report(schemas, onIncompatible);
      }
      PCollection<Long> committed = commitSchema(schemas, onIncompatible);
  ...
  private CommitSchemaUnion.TableCreation tableCreation() {
    return new CommitSchemaUnion.TableCreation(partitionFields, sortFields, tableProps);
  }
