...
public class HBaseTestUtils {
  ...
  public static String getCell(Table table, byte[] rowKey, byte[] colFamily, byte[] colQualifier)
      throws IOException {

    return Bytes.toString(
        getRowResult(table, rowKey).getValue(colFamily, colQualifier));
  }
  ...
  public static Table createTable(HBaseTestingUtility hbaseTestingUtil, String name)
    ...
    return hbaseTestingUtil.createTable(tableName, new String[] {Bytes.toString(colFamily),
        Bytes.toString(colFamily2)});
  }
  ...
  public static class HBaseMutationBuilder {
    ...
    public static Put createPut(
      ...
      return new Put(rowKey, atTimestamp)
          .addColumn(colFamily, colQualifier, value);
    }

    public static Delete createDelete(
        byte[] rowKey, byte[] colFamily, byte[] colQualifier, long atTimestamp) {
      return new Delete(rowKey, atTimestamp)
          .addColumns(colFamily, colQualifier);
    }
