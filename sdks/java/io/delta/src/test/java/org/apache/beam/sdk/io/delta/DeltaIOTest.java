...
public class DeltaIOTest {
  ...
  @Rule
  public TestPipeline filteringPipeline = TestPipeline.create();
  @Rule
  ...
  public void testPrintScanStateSchema() throws Exception {
    ...
    String commitContent = "{\"protocol\":{\"minReaderVersion\":1,\"minWriterVersion\":2}}\n"
    ...
    io.delta.kernel.defaults.engine.DefaultEngine engine = io.delta.kernel.defaults.engine.DefaultEngine.create(
    ...
    try (io.delta.kernel.utils.CloseableIterator<io.delta.kernel.data.FilteredColumnarBatch> scanFiles = scan
        .getScanFiles(engine)) {
      while (scanFiles.hasNext()) {
        io.delta.kernel.data.FilteredColumnarBatch batch = scanFiles.next();
        try (io.delta.kernel.utils.CloseableIterator<io.delta.kernel.data.Row> rows = batch.getRows()) {
          while (rows.hasNext()) {
  ...
  public void testCreateReadTasksDoFnGrouping() throws Exception {
    ...
    PCollection<String> taskDescriptions = output.apply(
        org.apache.beam.sdk.transforms.MapElements.into(
            org.apache.beam.sdk.values.TypeDescriptors.strings())
            .via(
                task -> {
                  ...
                  for (SerializableRow row : task.getScanFileRows()) {
                    ...
                    String fullPath = io.delta.kernel.internal.InternalScanFileUtils.getAddFileStatus(row)
                        .getPath();
  ...
  public void testFullPipelineRead() throws Exception {
    ...
    PCollection<Row> output = readPipeline.apply(DeltaIO.readRows().from(tableDir.getAbsolutePath()));
  ...
  private byte[] writeParquetFile(File file, Row row) throws Exception {
    org.apache.avro.Schema avroSchema = org.apache.beam.sdk.extensions.avro.schemas.utils.AvroUtils
        .toAvroSchema(row.getSchema());
    org.apache.avro.generic.GenericRecord record = org.apache.beam.sdk.extensions.avro.schemas.utils.AvroUtils
        .toGenericRecord(
            row, avroSchema);
    org.apache.hadoop.fs.Path path = new org.apache.hadoop.fs.Path(file.getAbsolutePath());
    try (
        org.apache.parquet.hadoop.ParquetWriter<org.apache.avro.generic.GenericRecord> writer = org.apache.parquet.avro.AvroParquetWriter.<org.apache.avro.generic.GenericRecord>builder(
            path)
  ...
  @Test
  public void testManagedReadBasic() throws Exception {
    // 1. Write the underlying parquet file
    Schema schema = Schema.builder().addStringField("name").build();
    Row row = Row.withSchema(schema).addValues("Alice").build();
    File tableDir = tempFolder.newFolder("managed-delta-table-basic");
    File dataDir = tableDir;
    File parquetFile = new File(dataDir, "part-00000.parquet");
    byte[] fileBytes = writeParquetFile(parquetFile, row);
    ...
    // 3. Read it using Managed I/O
    Map<String, Object> configMap = new HashMap<>();
    configMap.put("table_path", tableDir.getAbsolutePath());

    PCollection<Row> output = readPipeline
        .apply(Managed.read(Managed.DELTA).withConfig(configMap))
  ...
  public void testReadingLocalTable() throws Exception {
    PCollection<Row> output = readPipeline.apply(
        DeltaIO.readRows()
  ...
  public void testConvertToBeamSchema() {
    ...
    Schema nestedSchema = Schema.builder().addField("nested_string", Schema.FieldType.STRING).build();
  ...
  public void testDeltaReadTaskTracker() {
    ...
    org.apache.beam.sdk.io.range.OffsetRange range = new org.apache.beam.sdk.io.range.OffsetRange(0L, 3L);
    DeltaReadTaskTracker tracker = new DeltaReadTaskTracker(range, sizes);

    org.apache.beam.sdk.transforms.splittabledofn.RestrictionTracker.Progress progress = tracker.getProgress();
  ...
  public void testBeamParquetHandler() {
    ...
    org.apache.beam.sdk.io.range.OffsetRange range = new org.apache.beam.sdk.io.range.OffsetRange(0L, 2L);
    DeltaReadTaskTracker tracker = new DeltaReadTaskTracker(range, sizes);

    org.apache.hadoop.conf.Configuration conf = new org.apache.hadoop.conf.Configuration();
    io.delta.kernel.engine.ParquetHandler dummyDelegate = new io.delta.kernel.engine.ParquetHandler() {
      @Override
      public io.delta.kernel.utils.CloseableIterator<io.delta.kernel.engine.FileReadResult> readParquetFiles(
          io.delta.kernel.utils.CloseableIterator<io.delta.kernel.utils.FileStatus> fileIter,
          io.delta.kernel.types.StructType physicalSchema,
          java.util.Optional<io.delta.kernel.expressions.Predicate> predicate)
          throws java.io.IOException {
        return new io.delta.kernel.utils.CloseableIterator<io.delta.kernel.engine.FileReadResult>() {
          @Override
      ...
      public void writeParquetFileAtomically(
          String filePath,
          io.delta.kernel.utils.CloseableIterator<io.delta.kernel.data.FilteredColumnarBatch> data)
          throws java.io.IOException {
      }

      @Override
      public io.delta.kernel.utils.CloseableIterator<io.delta.kernel.utils.DataFileStatus> writeParquetFiles(
          String filePath,
          io.delta.kernel.utils.CloseableIterator<io.delta.kernel.data.FilteredColumnarBatch> data,
          java.util.List<io.delta.kernel.expressions.Column> statsColumns)
          throws java.io.IOException {
        return new io.delta.kernel.utils.CloseableIterator<io.delta.kernel.utils.DataFileStatus>() {
          @Override
          public boolean hasNext() {
            return false;
          }
    ...
    BeamEngine beamEngine = new BeamEngine(io.delta.kernel.defaults.engine.DefaultEngine.create(conf), handler);
  ...
  public void testBeamParquetHandlerWriteDelegation() throws Exception {
    ...
    org.apache.beam.sdk.io.range.OffsetRange range = new org.apache.beam.sdk.io.range.OffsetRange(0L, 1L);
    DeltaReadTaskTracker tracker = new DeltaReadTaskTracker(range, sizes);
    org.apache.hadoop.conf.Configuration conf = new org.apache.hadoop.conf.Configuration();

    boolean[] flags = new boolean[2];
    io.delta.kernel.engine.ParquetHandler delegate = new io.delta.kernel.engine.ParquetHandler() {
      @Override
      public io.delta.kernel.utils.CloseableIterator<io.delta.kernel.engine.FileReadResult> readParquetFiles(
          io.delta.kernel.utils.CloseableIterator<io.delta.kernel.utils.FileStatus> fileIter,
          io.delta.kernel.types.StructType physicalSchema,
  ...
  public void testBeamParquetHandlerReadFiltering() throws Exception {
    ...
    File parquetFile = new File(tableDir, "part-00000.parquet");
    io.delta.kernel.utils.FileStatus fileStatus = io.delta.kernel.utils.FileStatus.of(
        parquetFile.getAbsolutePath(), parquetFile.length(), 123456789L);

    org.apache.hadoop.conf.Configuration conf = new org.apache.hadoop.conf.Configuration();
    io.delta.kernel.types.StructType physicalSchema = new io.delta.kernel.types.StructType(
        java.util.Arrays.asList(
    ...
    DeltaReadTaskTracker trackerA = new DeltaReadTaskTracker(
    ...
    try (
        io.delta.kernel.utils.CloseableIterator<io.delta.kernel.engine.FileReadResult> iter = handlerA.readParquetFiles(
    ...
    DeltaReadTaskTracker trackerB = new DeltaReadTaskTracker(
    ...
    try (
        io.delta.kernel.utils.CloseableIterator<io.delta.kernel.engine.FileReadResult> iter = handlerB.readParquetFiles(
            io.delta.kernel.internal.util.Utils.singletonCloseableIterator(fileStatus),
            physicalSchema,
            java.util.Optional.empty())) {
      org.junit.Assert.assertFalse(iter.hasNext());
    ...
    // Case C: Claim fails
    DeltaReadTaskTracker trackerC = new DeltaReadTaskTracker(
    ...
    try (
        io.delta.kernel.utils.CloseableIterator<io.delta.kernel.engine.FileReadResult> iter = handlerC.readParquetFiles(
            io.delta.kernel.internal.util.Utils.singletonCloseableIterator(fileStatus),
            physicalSchema,
            java.util.Optional.empty())) {
      org.junit.Assert.assertFalse(iter.hasNext());
    ...
    DeltaReadTaskTracker trackerD = new DeltaReadTaskTracker(
        new org.apache.beam.sdk.io.range.OffsetRange(0L, 1L),
        java.util.Collections.singletonList(parquetFile.length()));
    BeamParquetHandler handlerD = new BeamParquetHandler(conf, dummyDelegate, trackerD);
    try (
        io.delta.kernel.utils.CloseableIterator<io.delta.kernel.engine.FileReadResult> iter = handlerD.readParquetFiles(
            io.delta.kernel.internal.util.Utils.singletonCloseableIterator(fileStatus),
            physicalSchema,
            java.util.Optional.empty())) {
