 ...
 * ====== READ ======
 * pipeline
 *     .apply(Managed.read(DELTA).withConfig(config))
...
public class DeltaIO {
  ...
  static class ConvertToRowFn extends DoFn<GenericRecord, Row> {
    private final Schema beamSchema;

    ConvertToRowFn(Schema beamSchema) {
      this.beamSchema = beamSchema;
    }

    @ProcessElement
    public void processElement(@Element GenericRecord record, OutputReceiver<Row> out) {
      out.output(AvroUtils.toBeamRowStrict(record, beamSchema));
    }
  }
}
