...
public class Solace {
    ...
    public abstract static class Record implements Serializable {
        ...
        /**
         * The ID for a particular message is only guaranteed to be the same for a particular copy
         * of a message on a particular queue or topic endpoint within a replication group. The same
         * message on different queues or topic endpoints within the same replication group may or
         * may not have the same replication group message ID. See more at <a
     ...
     *
     * <p>If the message has been published, {@link Solace.PublishResult#getPublished()} will be
     * true. If it is false, it means that the message could not be published, and {@link
    ...
    public abstract static class PublishResult {
        ...
        /**
         * The publishing latency in milliseconds. This is the difference between the time the
         * message was created, and the time the message was published. It is only available if the
         * {@link CorrelationKey} class is used as correlation key of the messages.
         */
        @SchemaFieldNumber("2")
    ...
    /**
     * The correlation key is an object that is passed back to the client during the event broker
     * ack or nack.
     *
        ...
        public static Record map(@Nullable BytesXMLMessage msg) {
            ...
            if (msg.getContentLength() != 0) {
                try {
                    ...
                    LOG.error(
                            "Could not write Bytes from the BytesXMLMessage to the Solace.record.",
                            e);
                }
            }
            ...
            if (originalDestination instanceof Topic) {
                ...
                LOG.error(
                        "SolaceIO: Unknown destination type for message {}, assuming that {} is a"
                                + " topic",
