...
public class GroupByKeyLoadIT {
  ...
  public interface Options extends TestPipelineOptions {
    ...
    @Default.Integer(5)
    Integer getGBKFanout();

    void setGBKFanout(Integer shuffleFanout);
  ...
  public void groupByKeyTest() {
    ...
    groupAndUngroupNTimes(input, options.getGBKFanout());

    pipeline.run().waitUntilFinish();
  }

  private void groupAndUngroupNTimes(final PCollection<KV<byte[], byte[]>> input, final Integer n) {
    if (n == 0) {
      return;
    } else {
      ...
      PCollection<KV<byte[], byte[]>> nextInput =
          groupedCollection.apply(
              String.format("Ungroup by key no: %s.", n),
              ParDo.of(new UngroupFn(options.getIterations())));

      groupAndUngroupNTimes(nextInput, n - 1);
    }
  }
  ...
  private static class UngroupFn extends DoFn<KV<byte[], Iterable<byte[]>>, KV<byte[], byte[]>> {
    ...
    public UngroupFn(Integer iterations) {
    ...
    public void processElement(ProcessContext c) {
      ...
      /*
       * Aren't we enlarging the data set for every key getIterations()' times?
       */
