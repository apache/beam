...
/**
 * Integration tests for {@link BigQueryIO#write(SerializableFunction)}.
 * This test writes 30MB data to BQ and verify the written row count.
 */
...
public class BigQueryIOStorageWriteIT {
	...
	private static final BigqueryClient BQ_CLIENT =
			new BigqueryClient("BigQueryStorageIOWriteIT");
	...
	private void runBigQueryIOStorageWritePipeline(int rowCount) {
		...
		TableRow rowToInsert =
				new TableRow().set("number", 0).set("str", "aaaaaaaaaa");
		...
		p.apply(input).apply(ParDo.of(new DoFn<TableRow, ArrayList<TableRow>>() {
		...
		String testQuery =
				String.format(
						"SELECT count(*) FROM [%s.%s];", DATASET_ID, tableName);
