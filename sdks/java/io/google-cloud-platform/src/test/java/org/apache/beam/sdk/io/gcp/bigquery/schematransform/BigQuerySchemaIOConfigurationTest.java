...
public class BigQuerySchemaIOConfigurationTest {
  ...
  static final List<ReadTestCase> testCases =
      Arrays.asList(
          ReadTestCase.of(
              BigQuerySchemaIOConfiguration.builderOfQueryType(""),
              InvalidConfigurationException.class),
          ReadTestCase.of(commonQueryConfigurationInput(), commonBaseQueryWant()),
          ReadTestCase.of(
              commonQueryConfigurationInput().setUseAvroLogicalTypes(true),
              commonBaseQueryWant().useAvroLogicalTypes()),
          ReadTestCase.of(
              commonQueryConfigurationInput().setUseAvroLogicalTypes(false), commonBaseQueryWant()),
          ReadTestCase.of(
          ...
          ReadTestCase.of(
              commonQueryConfigurationInput()
                  .setSelectedFields(Arrays.asList("field1", "field2", "field3")),
              commonBaseQueryWant()
                  .withSelectedFields(Arrays.asList("field1", "field2", "field3"))),
          ReadTestCase.of(
  ...
  static class ReadTestCase {
    ...
    static ReadTestCase of(
        BigQuerySchemaIOConfiguration.Builder input, BigQueryIO.TypedRead<TableRow> want) {
      return new ReadTestCase(input, want, null);
    }

    static ReadTestCase of(
        BigQuerySchemaIOConfiguration.Builder input, Class<? extends Exception> wantErr) {
  ...
  public void testToQueryTypedReadDefaults() {
    ...
    BigQuerySchemaIOConfiguration input =
  ...
  public void testToQueryTypedRead() {
    for (ReadTestCase caze : testCases) {
      ...
      Map<Identifier, Item> gotDisplayData =
          DisplayData.from(caze.input.build().toQueryTypedRead()).asMap();
