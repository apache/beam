...
class SchemaChangeDetectorHelper {
  ...
  private final boolean checkPayloadUnknownFields;

  public SchemaChangeDetectorHelper(
      boolean autoUpdateSchema,
      boolean ignoreUnknownValues,
      TableReference tableReference,
      boolean checkPayloadUnknownFields) {
    this.autoUpdateSchema = autoUpdateSchema;
    this.ignoreUnknownValues = ignoreUnknownValues;
    this.tableReference = tableReference;
    this.checkPayloadUnknownFields = checkPayloadUnknownFields;
  }
  ...
  static void bufferMismatchedRows(
      ...
      Instant originaElementTimestamp)
      throws IOException {
    ...
    org.joda.time.Instant minTimestamp =
        org.joda.time.Instant.ofEpochMilli(
            MoreObjects.firstNonNull(
                minPendingTimestamp.read(), GlobalWindow.INSTANCE.maxTimestamp().getMillis()));
    ...
    for (StoragePayloadWithDeadline row : rows) {
      if (appendClientInfo == null
        ...
        org.joda.time.Instant timestamp =
            MoreObjects.firstNonNull(
                row.getStoragePayload().getTimestamp(), originaElementTimestamp);
        ...
        @Nullable TableRow unknownFields = row.getStoragePayload().getUnknownFields();
        String errorMessage =
            (unknownFields != null && !unknownFields.isEmpty())
                ? "Timed out waiting for table schema update. Unknown fields: "
                    + unknownFields.keySet()
                : "Timed out waiting for table schema update (Mismatched schema)";
        BigQueryStorageApiInsertError error =
            new BigQueryStorageApiInsertError(
                failedRow, errorMessage, tableDestination.getTableReference());
        org.joda.time.Instant timestamp =
            MoreObjects.firstNonNull(
                row.getStoragePayload().getTimestamp(), originaElementTimestamp);
        failedRowsReceiver.outputWithTimestamp(error, timestamp);
