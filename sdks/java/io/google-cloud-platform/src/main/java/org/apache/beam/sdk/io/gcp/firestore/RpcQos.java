 ...
 * <p/>
 * Cloud Firestore has a number of considerations for interacting with the database in a reliable
 * manner.
 * <p/>
 ...
 * In the case of a write rpc, the QoS will also determine the size of the request in order to try
 * and maximize throughput with success rate.
 * <p/>
 * The lifecycle of an instance of {@link RpcQos} is expected to be bound to the lifetime of the
 * worker the RPC Functions run on. Explicitly, this instance should live longer than an individual
 * bundle.
 * <p/>
 * Each request processed via one of the {@link org.apache.beam.sdk.transforms.PTransform}s available
 * in {@link FirestoreV1} will work its way through a state machine provided by this {@link RpcQos}.
 * The high level state machine events are as follows:
 * <ol>
 *   <li>Create new {@link RpcAttempt}</li>
 *   <li>Check if it is safe to proceed with sending the request ({@link RpcAttempt#awaitSafeToProceed(Instant)})</li>
 *   <li>Record start of trying to send request ({@link RpcAttempt#recordStartRequest(Instant)})</li>
 *   <li>Record success or failure state of send request attempt</li>
 *   <li>If success output all returned responses</li>
 *   <li>
 *     If failure check retry ability ({@link RpcAttempt#checkCanRetry(RuntimeException)})
 *     <ol style="margin-top: 0">
 *       <li>Ensure the request has budget to retry ({@link RpcQosOptions#getMaxAttempts()})</li>
...
interface RpcQos {
  ...
  /**
   * Create a new stateful attempt for a read operation. The returned {@link RpcReadAttempt} will
  ...
  /**
   * Base interface representing the lifespan of attempting to successfully process a single request.
   */
  interface RpcAttempt {

    /**
     * Await until it is safe to proceed sending the rpc, evaluated relative to {@code start}.
     * If it is not yet safe to proceed, this method will block until it is safe to proceed.
     ...
     * <p/>
     * If no retry is available this {@link RpcAttempt} will move to a terminal failed state and
   ...
   * <p/>
   * This interface provides those methods which apply to read operations and the state tracked
   ...
   * <p/>
   * This interface provides those methods which apply to write operations and the state tracked
   * related to write operations.
   */
  interface RpcWriteAttempt extends RpcAttempt {

    /**
     * Create a new {@link FlushBuffer} that can be sized relative to the QoS state and to the provided {@code instant}.
     * @param instant The intended start time of the next rpc
     * @param <T> The type which will sent in the request
     * @param <E> The {@link Element} type which the returned buffer will contain
     ...
     * @param <E> The {@link Element} type which will be stored in this instance
     */
    interface FlushBuffer<E extends Element<?>> extends Iterable<E> {
      ...
      /**
       *
       * @return true if the buffer contains enough {@link Element}s such that a flush should happen, false otherwise
       */
      boolean isFull();
    }
     ...
     * ensuring an encapsulated lifecycle managed way of determining the serialized size of from
     * {@code T} which will need to be sent in a request.
     * <p/>
     * Due to the nature of how protobufs handle serialization, we want to ensure we only calculate
     * the serialization size once so we avoid unnecessary memory pressure. The serialized size is
     * read many times through out the lifecycle of determining to send a T.
     *
     * @param <T> The type which will sent in the request
     */
    interface Element<T> {
