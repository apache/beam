...
public abstract class BigQuerySchemaIOConfiguration {
  ...
  public static final String IDENTIFIER = String.format("%s:%s", API, VERSION);
  ...
  private static Builder builderOf(JobType jobType) {
    return new AutoValue_BigQuerySchemaIOConfiguration.Builder().setJobType(jobType.name());
  }

  public BigQueryIO.TypedRead<TableRow> toQueryTypedRead() {
    if (getQuery().isEmpty()) {
      throw new InvalidConfigurationException(
          "query is empty but required for a BigQuery Query Job");
    }
  ...
  public BigQueryIO.TypedRead<TableRow> toExtractTypedRead() {
    if (getTableSpec().isEmpty()) {
      throw new InvalidConfigurationException(
          "tableSpec is empty but required for a BigQuery Extract Job");
    }
  ...
  private BigQueryIO.TypedRead<TableRow> applyCommonTypedReadProperties(
      BigQueryIO.TypedRead<TableRow> read) {
  ...
  /**
   * Reads a BigQuery table specified as "[project_id]:[dataset_id].[table_id]" or
   ...
   *
   * <p>DEFAULT - The default behavior if no method is explicitly set. DIRECT_READ - Read the
   * contents of a table directly using the BigQuery storage API. EXPORT - Export data to Google
   * Cloud Storage in Avro format and read data files from that location.
   */
   ...
   *
   * <p>Setting this option when reading from a table will cause an error during validation.
   */
  public abstract Boolean getWithoutFlattenResults();

  /**
   * Enable/disable validation that the table exists or the query succeeds prior to pipeline
   ...
   *
   * <p>BATCH - Specifies that a query should be run with a BATCH priority. INTERACTIVE - Specifies
   * that a query should be run with an INTERACTIVE priority.
   */
   ...
   *
   * <p>Must be an SQL expression compatible with Google standard SQL.
   */
  ...
  public abstract static class Builder {
     ...
     * The priority of a query.
     *
     * <p>BATCH - Specifies that a query should be run with a BATCH priority. INTERACTIVE -
     ...
     *
     * <p>Except useStandardSql which defaults to true, all properties default to their zero value
     * i.e. boolean = false, String = "", etc.
     */
