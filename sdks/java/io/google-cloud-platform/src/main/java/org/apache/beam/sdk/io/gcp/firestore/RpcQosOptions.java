 ...
 * <p/>
 * Every RPC which is sent to Cloud Firestore is subject to QoS considerations. Successful, failed,
 * attempted requests are all tracked and directly drive the determination of when to attempt an RPC.
 * <p/>
 * Configuration of options can be accomplished by passing an instances of {@link RpcQosOptions}
 ...
 * A default instance of {@link RpcQosOptions} can be created via {@link RpcQosOptions#defaultOptions()}.
 * <p/>
 * @see FirestoreV1
 ...
 * @see <a href="https://cloud.google.com/firestore/quotas#limits">Standard limits</a>
 * @see <a href="https://cloud.google.com/firestore/docs/best-practices#designing_for_scale">Designing for scale</a>
 */
@Immutable
@ThreadSafe
public final class RpcQosOptions implements Serializable, HasDisplayData {
  ...
  public void populateDisplayData(DisplayData.Builder builder) {
    builder
        ...
        .add(DisplayData.item("samplePeriodBucketSize", samplePeriodBucketSize).withLabel("samplePeriodBucketSize"))
        ...
        .add(DisplayData.item("batchInitialCount", batchInitialCount).withLabel("batchInitialCount"))
        ...
        .add(DisplayData.item("batchTargetLatency", batchTargetLatency).withLabel("batchTargetLatency"))
        .add(DisplayData.item("hintMaxNumWorkers", hintMaxNumWorkers).withLabel("hintMaxNumWorkers"));
  }

  /**
   * The maximum number of times a request will be attempted for a complete successful result.
   * <p/>
   * For a stream based response, the full stream read must complete within the specified number of
   * attempts. Restarting a stream will count as a new attempt.
   * <p/>
   * <i>Default Value:</i> 5
   *
   ...
   * <a href="https://landing.google.com/sre/book/chapters/handling-overload.html">SRE Book -
   * Handling Overload</a>
   * <p/>
   * <i>Default Value:</i> 1.25
   *
  ...
  /**
   * A hint to the QoS system for the intended max number of workers for a pipeline. The
   * provided value can be used to try to scale calculations to values appropriate for the
   * pipeline as a whole.
   * <p/>
   * If you are running your pipeline on <a href="https://cloud.google.com/dataflow">Cloud
   * Dataflow</a> this parameter should be set to the same value as <a
  ...
  public int hashCode() {
    return Objects
        .hash(maxAttempts, initialBackoff, samplePeriod, samplePeriodBucketSize, overloadRatio,
            throttleDuration, batchInitialCount, batchMaxCount,
            batchMaxBytes, batchTargetLatency, hintMaxNumWorkers);
  }
  ...
  public String toString() {
    return "RpcQosOptions{" +
        ...
        ", samplePeriodBucketSize=" + samplePeriodBucketSize +
        ", overloadRatio=" + overloadRatio +
        ", throttleDuration=" + throttleDuration +
        ", batchInitialCount=" + batchInitialCount +
        ", batchMaxCount=" + batchMaxCount +
        ", batchMaxBytes=" + batchMaxBytes +
        ", batchTargetLatency=" + batchTargetLatency +
        ", hintWorkerCount=" + hintMaxNumWorkers +
        '}';
  }
  ...
  /**
   * Factory method to return a new instance of {@link Builder} with all values set to
   * their initial default values.
   *
   * @return New instance of {@link Builder} with all values set to their initial
   * default values
   * @see #defaultOptions()
   */
   ...
   * A new instance of {@link RpcQosOptions.Builder} can be created via {@link RpcQosOptions#newBuilder()}.
   * <p/>
   * <i>NOTE:</i> All {@code with} methods in this class function as set rather than copy with new value
   */
  public static final class Builder {

    /**
     * Cloud Firestore has a limit of 10MB per RPC. This is set lower than the 10MB limit on the
     * RPC, as this only accounts for the mutations themselves and not the Request wrapper
     * around them.
     */
    ...
    private Builder() {
      ...
      overloadRatio = 1.25; // TODO: What should this actually be?
      throttleDuration = Duration.standardSeconds(5);
      batchInitialCount = 200;
     ...
     * result.
     * <p/>
     * For a stream based response, the full stream read must complete within the specified number
    ...
    /**
     * The target ratio between requests sent and successful requests. This is "K" in the formula
     * in
     * <a href="https://sre.google/sre-book/handling-overload/#e2101">SRE Book -
     * Handling Overload</a>
     * <p/>
     * <i>Default Value:</i> 1.25
     *
     * @param overloadRatio the target ratio between requests sent and successful requests. A double
     * in the range 1.0 <= {@code overloadRatio} <= 2.0
     * @return this builder
     ...
     * <p/>
     * All provided values will be validated for non-nullness and that each value falls within its
     * allowed range.
    ...
    RpcQosOptions unsafeBuild() {
      return new RpcQosOptions(
          maxAttempts, initialBackoff, samplePeriod, samplePeriodBucketSize,
          overloadRatio, throttleDuration, batchInitialCount,
          batchMaxCount, batchMaxBytes, batchTargetLatency,
          hintMaxNumWorkers);
    }
    ...
    void validateIndividualFields() {
      ...
      checkInRange("overloadRatio", 1.0, 2.0, overloadRatio, Double::compare);
      ...
      checkInRange("batchInitialCount", 1, FIRESTORE_SINGLE_REQUEST_UPDATE_DOCUMENTS_MAX, batchInitialCount, Integer::compare);
      checkInRange("batchMaxCount", 1, FIRESTORE_SINGLE_REQUEST_UPDATE_DOCUMENTS_MAX, batchMaxCount, Integer::compare);
    ...
    void validateRelatedFields() {
      checkArgument(
          ...
          String.format("expected samplePeriodBucketSize <= samplePeriod, but was %s <= %s",
    ...
    private static <T> void checkInRange(String fieldName, T min, T max, T actual, Comparator<T> comp) {
      checkArgument(
          ...
          String.format("%s must be in the range %s to %s, but was %s", fieldName, min, max, actual)
      );
    }
