...
public class XmlIO {
   ...
   * }</pre>
   *
   * <p>Like for the source, it's possible to specify the charset used to write the file using
   * {@code withCharset(charset)} method. The following example shows you how to use it:
   *
   * <pre>{@code
   * p.apply(XmlIO.<Type>write()
   *      .withRecordClass(Type.class)
   *      .withRootElement(root_element)
   *      .withCharset(StandardCharset.ISO_8859_1)
   *      .toFilenamePrefix(output_filename));
   * }</pre>
   */
  // CHECKSTYLE.ON: JavadocStyle
