...
typedef
    : 'typedef' field_type IDENTIFIER type_annotations? -> ^(TYPEDEF IDENTIFIER field_type* type_annotations?)
    ;

enum_rule
    : 'enum' IDENTIFIER '{' enum_field* '}' type_annotations? -> ^(ENUM IDENTIFIER enum_field* type_annotations?)
    ;

enum_field
    : IDENTIFIER ('=' integer)? type_annotations? list_separator? -> ^(IDENTIFIER integer? type_annotations?)
    ;
