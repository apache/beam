...
public class EmployeeBatchSource extends BatchSource<String, String, StructuredRecord> {
  ...
  /**
   * Prepare Employee objects as it could be implemented in CDAP plugin.
   *
  ...
  public void prepareRun(BatchSourceContext context) {
    ...
    lineageRecorder.recordRead(
        "Reads",
        "Reading Employee objects",
        config.getSchema().getFields().stream()
            .map(Schema.Field::getName)
            .collect(Collectors.toList()));
    context.setInput(Input.of(NAME, new EmployeeInputFormatProvider(config)));
  }
