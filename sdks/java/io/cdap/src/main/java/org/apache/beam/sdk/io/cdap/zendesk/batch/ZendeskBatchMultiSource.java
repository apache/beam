...
public class ZendeskBatchMultiSource
  ...
  public void prepareRun(BatchSourceContext batchSourceContext) {
    ...
    schemasStrings.forEach(
        (objectName, objectSchema) ->
            arguments.set(
                MULTI_SINK_PREFIX + objectName.toLowerCase().replaceAll(" ", "_"), objectSchema));

    batchSourceContext.setInput(
        Input.of(
            config.referenceName,
            new ZendeskInputFormatProvider(config, config.getObjects(), schemasStrings)));
  }
