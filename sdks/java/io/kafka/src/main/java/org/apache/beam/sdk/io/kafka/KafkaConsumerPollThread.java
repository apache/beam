...
public class KafkaConsumerPollThread {
  ...
  private final AtomicReference<ImmutableList<KafkaCheckpointMark.PartitionMark>>
      finalizedPartitionMarks = new AtomicReference<>(ImmutableList.of());
   ...
   */
  boolean finalizeCheckpointMarksAsync(
      Collection<KafkaCheckpointMark.PartitionMark> partitionMarks) {
    return !finalizedPartitionMarks.getAndSet(ImmutableList.copyOf(partitionMarks)).isEmpty();
  }

  private void commitCheckpointMark() {
    ImmutableList<KafkaCheckpointMark.PartitionMark> partitionMarks =
        finalizedPartitionMarks.getAndSet(ImmutableList.of());
    if (partitionMarks.isEmpty()) {
      return;
    }

    LOG.debug("{}: Committing finalized checkpoint {}", this, partitionMarks);
    Consumer<byte[], byte[]> consumer = Preconditions.checkStateNotNull(this.consumer);

    consumer.commitSync(
        partitionMarks.stream()
