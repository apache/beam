 ...
 * {@link KafkaSchemaTransformReadConfiguration}.
 *
...
public class KafkaSchemaTransformReadProvider
  ...
  private static final String API = "kafka";
  ...
  /**
   * An implementation of {@link PTransform} for Kafka read jobs configured using {@link
   * KafkaSchemaTransformReadConfiguration}.
   */
  static class PCollectionRowTupleTransform
    ...
    /** An instance of {@link KafkaServices} used for testing. */
    private KafkaServices testKafkaServices = null;

    PCollectionRowTupleTransform(KafkaSchemaTransformReadConfiguration configuration) {
      this.configuration = configuration;
    }

    @VisibleForTesting
    void setTestKafkaServices(BigQueryServices testKafkaServices) {
      this.testKafkaServices = testKafkaServices;
    }
    ...
    public PCollectionRowTuple expand(PCollectionRowTuple input) {
      ...
      KafkaIO.RowsWithMetadata<> read = KafkaIO.RowsWithMetadata.Builder;
      if (testKafkaServices != null) {
        read = read.withTestServices(testKafkaServices).withoutValidation();
      }
