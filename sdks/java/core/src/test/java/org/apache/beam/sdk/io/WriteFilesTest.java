...
public class WriteFilesTest {
  ...
  public void testCustomShardingFunctionWrite() throws IOException {
    runShardedWrite(
        ...
        WriteFiles.to(makeSimpleSink())
            .withNumShards(4)
            .withShardingFunction(new TestShardingFunction(), DefaultShardKeyCoder.of()),
  ...
  private static class TestShardingFunction
      implements ShardingFunction<String, Void, DefaultShardKey> {
    @Override
    public DefaultShardKey assignShardKey(Void destination, String element, int shardCount)
        throws Exception {
      int shard = Integer.parseInt(element);
      return DefaultShardKey.of(0, shard);
    }
  }
}
