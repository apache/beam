...
public class StaticSchemaInference {
    ...
    if (alreadyVisitedSchemas.containsKey(typeDescriptor)) {
      ...
      if (existingSchema == null) {
        throw new IllegalArgumentException(
            "Cannot infer schema with a circular reference. Type: " + typeDescriptor);
      }
  ...
  private static Schema.FieldType fieldFromType(
    ...
    if (type.isArray()) {
      ...
      TypeDescriptor<Map<?, ?>> map = ((TypeDescriptor) type).getSupertype(Map.class);
      ...
      TypeDescriptor<Iterable<?>> iterable = ((TypeDescriptor) type).getSupertype(Iterable.class);
