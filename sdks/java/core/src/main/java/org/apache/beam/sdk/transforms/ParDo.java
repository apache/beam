...
public class ParDo {
  ...
  public static <InputT, OutputT> SingleOutput<InputT, OutputT> of(DoFn<InputT, OutputT> fn) {
    validate(fn);
    return new SingleOutput<>(fn, Collections.emptyMap(), displayDataForFn(fn), null);
  }
   ...
   */
  private static DoFnSchemaInformation inferDoFnSchemaInformation(
      DoFn<?, ?> fn, @Nullable Schema schema, Coder<?> coder, Pipeline p) {
    DoFnSignature signature = DoFnSignatures.getSignature(fn.getClass());
    DoFnSignature.ProcessElementMethod processElementMethod = signature.processElement();
    ...
    if (keyFieldAccess != null) {
      ...
      keyFieldAccess = keyFieldAccess.resolve(schema);
      keySchema = SelectHelpers.getOutputSchema(schema, keyFieldAccess);
  ...
  public static @Nullable FieldAccessDescriptor getDoFnFieldsDescriptor(
  ...
  public static class SingleOutput<InputT, OutputT>
    ...
    public SingleOutput<InputT, OutputT> withDoFnSchemaInformation(
      ...
      return new SingleOutput<>(fn, sideInputs, fnDisplayData, doFnSchemaInformation);
    }
  ...
  public static class MultiOutput<InputT, OutputT>
    ...
    public PCollectionTuple expand(PCollection<? extends InputT> input) {
      ...
      {
        if (signature.usesState() || signature.usesTimers() || signature.onWindowExpiration() != null) {
          validateStateApplicableForInput(fn, input, keyFieldAccess);
        }
      }
      ...
      if (keyFieldAccess == null) {
        ...
        outputs =
            input
                .apply("Extract schema keys", getWithSchemaKeys(keyFieldAccess))
                .apply(
                    new MultiOutputPrimitive<>(
                        getFn(),
                        getMainOutputTag(),
  ...
  public static class MultiOutputPrimitive<InputT, OutputT>
      extends PTransform<PCollection<? extends InputT>, PCollectionTuple> {
    private final DoFn<?, OutputT> fn;
    private final TupleTag<OutputT> mainOutputTag;
    private final TupleTagList additionalOutputTags;
    private final Map<String, PCollectionView<?>> sideInputs;
    private final DoFnSchemaInformation doFnSchemaInformation;

    public MultiOutputPrimitive(
        DoFn<?, OutputT> fn,
        TupleTag<OutputT> mainOutputTag,
        TupleTagList additionalOutputTags,
        Map<String, PCollectionView<?>> sideInputs,
    ...
    public PCollectionTuple expand(PCollection<? extends InputT> input) {
      ...
      outputs.get(getMainOutputTag()).setTypeDescriptor(getFn().getOutputTypeDescriptor());
