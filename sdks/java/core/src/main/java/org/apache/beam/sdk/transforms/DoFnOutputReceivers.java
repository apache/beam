...
public class DoFnOutputReceivers {
  ...
  private static class WindowedContextOutputReceiver<T>
      implements OutputReceiver<T>, ValueWithMetadataReceiver<T> {
    DoFn<?, ?>.WindowedContext context;
    @Nullable TupleTag<T> outputTag;
    ...
    public OutputBuilder<T> builder(T value) {
      return WindowedValue.builder(this).setValue(value);
    }

    @Override
    public void output(ValueWithMetadata<T> valueWithMetadata) {
      if (outputTag != null) {
        context.outputWindowedValue(
            outputTag,
