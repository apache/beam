  ...
  private final class InboundObserver implements StreamObserver<BeamFnApi.Elements> {
    ...
    private void forwardToConsumerForInstructionId(String instructionId, BeamFnApi.Elements value) {
      ...
      if (consumerFuture == null) {
        LOG.debug("Received data for instruction {} which was poisoned, dropping data.",
                instructionId);
        return;
      }
      ...
      try {
          // The consumer may not be registered until the bundle processor is fully constructed so we conservatively set
          // a high timeout.  Poisoning will prevent this for occurring for consumers that will not be registered.
