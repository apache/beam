...
public class ParDoTranslation {
  ...
  static {
    checkState(
        REQUIRES_STATEFUL_PROCESSING_URN.equals(
            BeamUrns.getUrn(StandardRequirements.Enum.REQUIRES_STATEFUL_PROCESSING)));
    checkState(
        REQUIRES_BUNDLE_FINALIZATION_URN.equals(
            BeamUrns.getUrn(StandardRequirements.Enum.REQUIRES_BUNDLE_FINALIZATION)));
    checkState(
        REQUIRES_STABLE_INPUT_URN.equals(
            BeamUrns.getUrn(StandardRequirements.Enum.REQUIRES_STABLE_INPUT)));
    checkState(
        REQUIRES_TIME_SORTED_INPUT_URN.equals(
            BeamUrns.getUrn(StandardRequirements.Enum.REQUIRES_TIME_SORTED_INPUT)));
    checkState(
        REQUIRES_SPLITTABLE_DOFN_URN.equals(
            BeamUrns.getUrn(StandardRequirements.Enum.REQUIRES_SPLITTABLE_DOFN)));
    checkState(
    ...
    checkState(ORDERED_LIST_USER_STATE.equals(BeamUrns.getUrn(StandardUserStateTypes.Enum.ORDERED_LIST)));
  }
  ...
  /** A {@link PTransformTranslation.TransformPayloadTranslator} for {@link ParDo}. */
  public static class ParDoTranslator
      implements PTransformTranslation.TransformTranslator<MultiOutput<?, ?>> {
  ...
  public static RunnerApi.PCollection getMainInput(
    ...
    checkArgument(
        PTransformTranslation.PAR_DO_TRANSFORM_URN.equals(ptransform.getSpec().getUrn())
            ...
            || PTransformTranslation.SPLITTABLE_SPLIT_AND_SIZE_RESTRICTIONS_URN.equals(
                ptransform.getSpec().getUrn())
            || PTransformTranslation.SPLITTABLE_TRUNCATE_SIZED_RESTRICTION_URN.equals(
                ptransform.getSpec().getUrn())
            || PTransformTranslation.SPLITTABLE_PROCESS_ELEMENTS_URN.equals(
                ptransform.getSpec().getUrn())
