 ...
 * by the pipeline.
 *
 * <p>For sources that provide non-heuristic watermarks (e.g., when using arrival times as event
 * times and processing time as the watermark), the watermark is a strict guarantee that no data
 * with an event time earlier than that watermark will ever be observed in the pipeline. In this
 * case, it's safe to assume that any pane triggered by an {@code AfterWatermark} trigger with a
 * reference point at or beyond the end of the window will be the last pane ever for that window.
 *
 * <p>For sources that provide heuristic watermarks (e.g., when using user-supplied event times),
 * the watermark itself becomes an <i>estimate</i> that no data with an event time earlier than
 * that watermark (i.e. "late data") will ever be observed in the pipeline. These heuristics can
 ...
 * once when the watermark passes the end of the window and then immediately thereafter when any
 * late data arrives, is one such example.
 *
 * <p>The watermark is the clock that defines {@link TimeDomain#EVENT_TIME}.
 *
 * Additional firings before or after the watermark can be requested by calling
