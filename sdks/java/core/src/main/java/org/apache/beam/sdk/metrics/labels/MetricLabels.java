...
public abstract class MetricLabels implements Serializable {
  ...
  public static MetricLabels create(MonitoringInfo monitoringInfo) {
    String ptransform = monitoringInfo.getLabelsMap().get(PTRANSFORM_LABEL);
    if (ptransform != null) {
      return ptransform(ptransform);
    }
    String pcollection = monitoringInfo.getLabelsMap().get(PCOLLECTION_LABEL);
    if (pcollection != null) {
      return pcollection(pcollection);
    }
    throw new IllegalStateException(
        "Can't create MetricLabels from MonitoringInfo: " + monitoringInfo);
  }
