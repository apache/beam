...
public class StructuredPipelineExecutionTest {
  ...
  @Test
  public void testNearEarthObjects() throws Exception {
    URL pipelineURL = ClassLoader.getSystemClassLoader().getResource("near_earth_objects");
    ...
    try (AutoCloseable x = expansionService.start()) {
      ...
      try (AutoCloseable p = pyRunner.start()) {
        ...
        spd.run(
            ImmutableMap.of(
                "PORTABLE_RUNNER_HOST", "localhost", "PORTABLE_RUNNER_PORT", "" + port));
      }
    }
  }
