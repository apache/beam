 ...
 * org.apache.beam.sdk.extensions.avro.schemas.utils.AvroUtils#schemaCoder(Class, Schema)} to make
 * it schema-aware.
 *
 * <pre>{@code
 * PCollection<AvroAutoGenClass> records = ...
 ...
 * value, so you may need not set it yourself. A {@link FilenamePolicy} must be set, and unique
 * windows and triggers must produce unique filenames.
 *
...
public class AvroIO {
  ...
  private static <T> PCollection<T> setBeamSchema(
    ...
    org.apache.beam.sdk.schemas.Schema beamSchema = AvroUtils.getSchema(clazz, schema);
    if (beamSchema != null) {
      pc.setSchema(
          beamSchema,
          TypeDescriptor.of(clazz),
          AvroUtils.getToRowFunction(clazz, schema),
