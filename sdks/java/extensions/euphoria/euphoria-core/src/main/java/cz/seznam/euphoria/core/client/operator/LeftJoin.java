...
public class LeftJoin {
  ...
  public static class UsingBuilder<LeftT, RightT, K> {
    ...
    public <OutputT> Join.WindowingBuilder<LeftT, RightT, K, OutputT> using(
        BinaryFunctor<LeftT, Optional<RightT>, OutputT> functor) {
      final BinaryFunctor<LeftT, RightT, OutputT> wrappedFunctor =
          (left, right, context) -> functor.apply(left, Optional.ofNullable(right), context);
      return new Join.WindowingBuilder<>(
          name, left, right, leftKeyExtractor, rightKeyExtractor, wrappedFunctor, Join.Type.LEFT);
    }
  }
}
