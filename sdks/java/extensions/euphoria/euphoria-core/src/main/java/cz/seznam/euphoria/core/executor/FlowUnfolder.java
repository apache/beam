...
public class FlowUnfolder {
  ...
  public static DAG<Operator<?, ?>> unfold(
      Flow flow, Set<Class<? extends Operator<?, ?>>> operatorClasses)
      throws IllegalArgumentException {
    return unfold(flow, op -> operatorClasses.contains(op.getClass()));
  }

  /**
   * Unfolds the flow so that it contains only operators for which {@code wantTranslate} returns
   * {@code true}. Operators for which {@code wantTranslate} doesn't return {@code true} are
   * expanded into their {@link Operator#getBasicOps()} and {@code wantTranslate} will be
   * recursively called on these. Hence, there is a certain set of basic operators which the {@code
   * wantTranslate} function has to accept.
   *
  ...
  public static DAG<Operator<?, ?>> translate(
      DAG<Operator<?, ?>> dag, UnaryPredicate<Operator<?, ?>> wantTranslate)
      throws IllegalArgumentException {
                ...
                if (basicOps.size() == 1) {
                  if (basicOps.nodes().findFirst().get().getClass() == n.get().getClass()) {
                    throw new IllegalArgumentException(
                        "Operator " + n.get() + " cannot be executed with given executor!");
                  }
                }
  ...
  private static List<Operator<?, ?>> getParents(
      Node<Operator<?, ?>> node, Map<Dataset<?>, Optional<Operator<?, ?>>> datasetProducents) {
              ...
              o -> {
                if (o == null) {
                  ...
                  throw new IllegalStateException(
                      "Inputs of operator " + op + " are inconsistent: " + op.listInputs());
                }
                return o.isPresent();
              })
          .map(Optional::get)
    ...
    return node.getParents()
        ...
        .map(
            o -> {
              if (o == null) {
                throw new IllegalStateException(
                    "Output of " + node.get() + " should have been stored into 'datasetProducents");
              }
              return o.get();
            })
        .collect(Collectors.toList());
      ...
      for (Operator<?, ?> op : operators) {
        if (!resolvedOperators.contains(op)) {
          if (op.listInputs().stream().allMatch(datasets::containsKey)) {
            ...
            List<Operator<?, ?>> parents =
                op.listInputs().stream().map(datasets::get).collect(Collectors.toList());
            ret.add(op, parents);
