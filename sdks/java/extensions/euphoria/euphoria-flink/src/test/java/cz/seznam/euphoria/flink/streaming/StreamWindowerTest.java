...
public class StreamWindowerTest {
  ...
  public void testTimeWindowing() throws Exception {
    ...
    WindowedStream<WindowedPair<Windowing.Time.TimeInterval, Integer, Integer>, Integer, FlinkWindow> windowed;
    windowed = windower.window(input, i -> i.getSecond() % 2, i -> 2 * i.getSecond(),
        Windowing.Time.of(Duration.ofSeconds(1)).using(p -> 1000L * p.getFirst()));


    DataStream<WindowedPair<Windowing.Time.TimeInterval, Integer, Integer>> reduce
        = sumReduce(windowed);
    
    ListDataSink<WindowedPair<Windowing.Time.TimeInterval, Integer, Integer>> sink
        = ListDataSink.get(1);
    ...
    SortedMap<Windowing.Time.TimeInterval, Set<Pair<Integer, Integer>>> results
        = new TreeMap<>((Windowing.Time.TimeInterval t1, Windowing.Time.TimeInterval t2) -> {
    ...
    Iterator<Map.Entry<Windowing.Time.TimeInterval, Set<Pair<Integer, Integer>>>> iterator
  ...
  public void testTimeSlidingWindowing() throws Exception {
    ...
    WindowedStream<Pair<WindowID<Void, Long>, Pair<Integer, Integer>>, Integer, FlinkWindow> windowed;

    windowed = windower.genericWindow(input, i -> i.getSecond() % 2, i -> 2 * i.getSecond(),
        Windowing.TimeSliding.of(Duration.ofSeconds(2), Duration.ofSeconds(1))
            .using(p -> 1000L * p.getFirst()));

    DataStream<Pair<WindowID<Void, Long>, Pair<Integer, Integer>>> reduced;
    reduced = windowed.reduce(
        new ReduceFunction<Pair<WindowID<Void, Long>, Pair<Integer, Integer>>>() {
          @Override
          public Pair<WindowID<Void, Long>, Pair<Integer, Integer>>
          reduce(Pair<WindowID<Void, Long>, Pair<Integer, Integer>> a,
              Pair<WindowID<Void, Long>, Pair<Integer, Integer>> b) {
            return Pair.of(a.getFirst(),
                Pair.of(a.getSecond().getFirst(),
                    a.getSecond().getSecond() + b.getSecond().getSecond()));
          }
        });
    ...
    reduced
        .map(new MapFunction<Pair<WindowID<Void, Long>, Pair<Integer, Integer>>,
                WindowedPair<Long, Integer, Integer>>() {
              @Override
              public WindowedPair<Long, Integer, Integer> map(
                  Pair<WindowID<Void, Long>, Pair<Integer, Integer>> t) {
                return WindowedPair.of(
                    t.getFirst().getLabel(),
                    t.getSecond().getFirst(),
                    t.getSecond().getSecond());
              }
            })
