...
public class WindowingTest {
  ...
  public void testWindowingByEventTime() throws Exception {
    ...
    Dataset<Pair<Item, Long>> words =
        MapElements.of(lines)
    ...
    Dataset<String> mapped =
        MapElements.of(reduced).using(p -> p.getFirst() + "-" + p.getSecond()).output();

    mapped =
        FlatMap.of(mapped)
            .using(
                (UnaryFunctor<String, String>)
                    (elem, c) -> {
    ...
    DatasetAssert.unorderedEquals(
        output.getOutputs(),
        /* 1st second window */ "1: one-1",
        "1: two-3",
        "1: three-1",
        "1: four-2",
        /* 2nd second window */ "2: one-3",
        "2: two-2",
        "2: three-2",
  ...
  public void testAttachedWindowing_ContinuousOutput()
    ...
    Dataset<Set<String>> mediator = MapElements.of(first).using(e -> e).output();
    ...
    // is received.
    Dataset<Set<String>> second = ReduceWindow.of(mediator).reduceBy(Fold.of(Sets::union)).output();
    ...
    Dataset<Pair<Long, Set<String>>> third =
        MapElements.of(second).using(what -> Pair.of(System.currentTimeMillis(), what)).output();
  ...
  public void testAttachedWindowing_InhertitedLabel() throws Exception {
    ...
    Dataset<TimeInterval> windows =
        FlatMap.of(distinct)
            .using(
                (UnaryFunctor<Pair<String, Void>, TimeInterval>)
                    (elem, context) -> context.collect((TimeInterval) context.getWindow()))
            .output();

    Dataset<Pair<TimeInterval, Long>> counts =
        ReduceByKey.of(windows).keyBy(e -> e).valueBy(e -> 1L).combineBy(Sums.ofLongs()).output();
    ...
    List<Pair<Long, Long>> ordered =
        output
            ...
            .collect(Collectors.toList());
    assertEquals(asList(Pair.of(2000000000000L, 5L), Pair.of(2000000001000L, 4L)), ordered);
  }
  ...
  public void testWindowing_EndOfWindow_RBK() throws InterruptedException, ExecutionException {
    ...
    Dataset<String> second =
        FlatMap.of(first)
            .using(
                (UnaryFunctor<Pair<String, Set<String>>, String>)
                    (e, c) -> {
  ...
  static final class Item {
    ...
    public String toString() {
      return "Item{" + "word='" + word + '\'' + ", evtTs=" + evtTs + '}';
    }
  }
}
