...
public class BeamSortRelTest extends BaseRelTest {
  ...
  public void prepare() {
    sqlEnv.registerTable("ORDER_DETAILS",
        MockedBoundedTable.of(
            ...
            1L, 1, 2.0, new Timestamp(1), new Date(1),
            2L, 4, 3.0, new Timestamp(2), new Date(2),
            2L, 1, 4.0, new Timestamp(3), new Date(3),
            5L, 5, 5.0, new Timestamp(4), new Date(4),
            6L, 6, 6.0, new Timestamp(5), new Date(5),
            7L, 7, 7.0, new Timestamp(6), new Date(6),
            8L, 8888, 8.0, new Timestamp(7), new Date(7),
            8L, 999, 9.0, new Timestamp(8), new Date(8),
            10L, 100, 10.0, new Timestamp(9), new Date(9)
        )
    );
  ...
  public void testOrderBy_timestamp() throws Exception {
    ...
    PAssert.that(rows).containsInAnyOrder(TestUtils.RowsBuilder.of(
        ...
        7L, 7, 7.0, new Timestamp(6),
        8L, 8888, 8.0, new Timestamp(7),
        8L, 999, 9.0, new Timestamp(8),
        10L, 100, 10.0, new Timestamp(9)
    ).getRows());
    pipeline.run().waitUntilFinish();
  }
