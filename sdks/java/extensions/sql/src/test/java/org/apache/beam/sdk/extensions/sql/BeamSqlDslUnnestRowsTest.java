...
public class BeamSqlDslUnnestRowsTest {
  ...
  /**
   * TODO([BEAM-14026]): This is a test of the incorrect behavior unnest
  ...
  public void testUnnestArrayWithNestedRows() {
    ...
    PCollection<Row> inputTable =
            PBegin.in(pipeline).apply(Create.of(level1Row1, level1Row2, level1Row3).withRowSchema(level1Type));
    ...
    PAssert.that(result)
            .containsInAnyOrder(
                    Row.withSchema(resultSchema).addValues(1, "row", 1.0, 1, "row", 1.0, 1, "row", 1.0).build(),
                    Row.withSchema(resultSchema).addValues(1, "row", 1.0, 2, "row", 2.0, 2, "row", 2.0).build(),
                    Row.withSchema(resultSchema).addValues(1, "row", 1.0, 3, "row", 3.0, 3, "row", 3.0).build(),
                    Row.withSchema(resultSchema).addValues(3, "row", 3.0, 1, "row", 1.0, 1, "row", 1.0).build(),
                    Row.withSchema(resultSchema).addValues(3, "row", 3.0, 2, "row", 2.0, 2, "row", 2.0).build(),
                    Row.withSchema(resultSchema).addValues(3, "row", 3.0, 3, "row", 3.0, 3, "row", 3.0).build(),
                    Row.withSchema(resultSchema).addValues(2, "row", 2.0, 1, "row", 1.0, 1, "row", 1.0).build(),
                    Row.withSchema(resultSchema).addValues(2, "row", 2.0, 2, "row", 2.0, 2, "row", 2.0).build(),
                    Row.withSchema(resultSchema).addValues(2, "row", 2.0, 3, "row", 3.0, 3, "row", 3.0).build()
            );
    pipeline.run();
  }
