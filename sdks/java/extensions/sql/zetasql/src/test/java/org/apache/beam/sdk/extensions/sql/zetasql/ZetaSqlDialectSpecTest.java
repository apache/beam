...
public class ZetaSqlDialectSpecTest extends ZetaSqlTestBase {
  ...
  @Test
  public void testZetaSQLVarPop() {
    String sql = "SELECT VAR_POP(row_id) FROM table_all_types GROUP BY bool_col";
    ...
    PAssert.that(stream)
        .containsInAnyOrder(
            ...
            Row.withSchema(schema).addValue(1.555555556).build());

    pipeline.run().waitUntilFinish(Duration.standardMinutes(PIPELINE_EXECUTION_WAITTIME_MINUTES));
  }

  @Test
  public void testZetaSQLVarSamp() {
    String sql = "SELECT VAR_SAMP(row_id) FROM table_all_types GROUP BY bool_col";
    ...
    PAssert.that(stream)
        .containsInAnyOrder(
            Row.withSchema(schema).addValues(2.333333333).build(),
            Row.withSchema(schema).addValue(2.0).build());

    pipeline.run().waitUntilFinish(Duration.standardMinutes(PIPELINE_EXECUTION_WAITTIME_MINUTES));
  }

  @Test
  public void testZetaSQLVariance() {
    String sql = "SELECT VARIANCE(row_id) FROM table_all_types GROUP BY bool_col";
