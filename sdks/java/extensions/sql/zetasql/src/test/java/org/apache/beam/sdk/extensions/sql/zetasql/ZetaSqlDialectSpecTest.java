...
public class ZetaSqlDialectSpecTest extends ZetaSqlTestBase {
  ...
  @Test
  @Ignore(
      "Zetasql layer does not support aggregate function with more than one argument. [https://issues.apache.org/jira/browse/BEAM-11808]")
  public void testZetaSqlCovarSamp() {
    String sql = "SELECT COVAR_SAMP(row_id,int64_col) FROM table_all_types  GROUP BY bool_col";
    ...
    PAssert.that(stream)
        .containsInAnyOrder(
            Row.withSchema(schema).addValue(-2.00000).build(),
            Row.withSchema(schema).addValue(-2.33333).build());
