...
export function jsEnvironment(
    ...
    artifacts: runnerApi.ArtifactInformation[] = []): runnerApi.Environment {
    return {
        urn: urn,
        payload: payload,
...
function asNewEnvironment(env: runnerApi.Environment, urn: string, payload: Uint8Array) {
    return {
        urn: urn,
        payload: payload,
...
export function asExternalEnvironment(env: runnerApi.Environment, address: string) {
...
export function asDockerEnvironment(env: runnerApi.Environment, containerImage: string) {
    return asNewEnvironment(
        env,
        "beam:env:docker:v1",
