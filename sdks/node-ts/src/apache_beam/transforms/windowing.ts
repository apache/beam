...
export class GlobalWindows implements WindowFn<GlobalWindow> {
    assignWindows(Instant) { return [new GlobalWindow()]; }
    windowCoder() { return new GlobalWindowCoder(); }
    toProto() { return { urn: "beam:window_fn:global_windows:v1", payload: new Uint8Array() }; }
    isMerging() { return false; }
    assignsToOneWindow() { return true; }
}
...
export class FixedWindows implements WindowFn<IntervalWindow> {
    ...
    constructor(sizeSeconds: number | Long, offsetSeconds: Instant = Long.fromValue(0)) {
    ...
    toProto() {
        return {
            ...
            payload: FixedWindowsPayload.toBinary({ size: millisToProto(this.size), offset: millisToProto(this.offset) }),
        }
    }
...
export class Sessions implements WindowFn<IntervalWindow> {
    ...
    toProto() {
        return {
            ...
            payload: SessionWindowsPayload.toBinary({ gapSize: millisToProto(this.gap) }),
        };
    }

    isMerging() { return true; }
