  ...
  splice(
    ...
    for (const t of Object.values(response.components!.transforms)) {
      t.inputs = Object.fromEntries(
        Object.entries(t.inputs).map(([k, v]) => [k, renamedInputs[v]])
      );
    }
...
async function main() {
  const kvCoder = new coders.KVCoder(
    new coders.VarIntCoder(),
    new coders.VarIntCoder()
  );
  const root = new base.Root(new base.Pipeline());
  const input = root
    .apply(new core.Create([{key: 1, value: 3}]))
  ...
  await input.asyncApply(
    new RawExternalTransform<base.PValue<any>, base.PValue<any>>(
      base.GroupByKey.urn,
      undefined!,
      'localhost:4444'
    )
