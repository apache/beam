...
func WritePipeline(expansionAddr, tableName, driverClassName, jdbcUrl, username, password string) *beam.Pipeline {
	...
	rows := JdbcWriteTestRow{1, 1.1, "Test1"}
...
func readRows(s beam.Scope, expansionAddr, tableName, driverClassName, jdbcUrl, username, password, query string) {
	s = s.Scope("jdbc_test.WriteToJdbc")
...
func ReadPipeline(expansionAddr, tableName, driverClassName, jdbcUrl, username, password, query string) *beam.Pipeline {
	p, s := beam.NewPipelineWithRoot()
	readRows(s, expansionAddr, tableName, driverClassName, jdbcUrl, username, password, query)
	return p
}
