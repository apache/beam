...
func TestMetrics(t *testing.T) {
	...
	for _, test := range tests {
		...
		qr := pr.Metrics().QueryByNamespace(test.name)
		if len(qr.Counters()) != test.counters || len(qr.Distributions()) != test.distributions || len(qr.Gauges()) != test.gauges {
			t.Errorf("Metrics filtering with Namespace failed.\nGot %d counters, %d distributions, %d gauges\n Want %d counters, %d distributions, %d gauges",
				len(qr.Counters()), len(qr.Distributions()), len(qr.Gauges()), test.counters, test.distributions, test.gauges)
		}
	}
}
