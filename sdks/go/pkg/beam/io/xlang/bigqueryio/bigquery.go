...
// table.
//
// Read requires the address for an expansion service for BigQuery Read transforms and a
// reflect.Type description of the struct to read from BigQuery. Additionally, either one Table
// or one Query must be provided via readOptions to define a destination to read from.
//
...
//   pcol := bigqueryio.Read(s, expansionAddr, outType, bigqueryio.FromTable(table))
func Read(s beam.Scope, addr string, elmT reflect.Type, opts ...readOption) beam.PCollection {
...
func FromTable(table string) readOption {
	return func(cfg *bigQueryConfig) {
		cfg.Table = &table
	}
}
...
// table.
//
// Write requires the address for an expansion service for BigQuery Write transforms, the ID of a
// BigQuery table, and an input PCollection. The type of the input PCollection is converted to a
// Beam schema, so it must have a valid Beam schema definition.
//
...
//   expansionAddr := "localhost:1234"
//   table := "project_id:dataset_id.table_id"
//   pcol := bigqueryio.Write(s, expansionAddr, table, input,
//      bigqueryio.CreateDisposition(bigqueryio.CreateIfNeeded))
func Write(s beam.Scope, addr string, table string, col beam.PCollection, opts ...writeOption) {
...
type writeOption func(*bigQueryConfig)
