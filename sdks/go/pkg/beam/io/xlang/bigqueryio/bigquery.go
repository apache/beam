...
func Read(s beam.Scope, elmT reflect.Type, opts ...readOption) beam.PCollection {
	...
	outT := typex.New(elmT)
	outs := beam.CrossLanguage(s, readURN, pl, rc.addr, nil, beam.UnnamedOutput(outT))
...
// ReadExpansionAddr specifies the expansion address to use for a Read transform.
...
func Write(s beam.Scope, table string, col beam.PCollection, opts ...writeOption) {
	...
	beam.CrossLanguage(s, writeURN, pl, wc.addr, beam.UnnamedInput(col), nil)
}
...
// WriteExpansionAddr specifies the expansion address to use for a Write transform.
