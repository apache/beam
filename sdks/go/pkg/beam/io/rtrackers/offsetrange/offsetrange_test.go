...
func TestNewGrowableTracker_Bad(t *testing.T) {
	_, err := NewGrowableTracker(0, nil)
	if err == nil {
...
func TestGrowableTracker_TryClaim(t *testing.T) {
	...
	tracker, err := NewGrowableTracker(0, &estimator)
	if err != nil {
...
func TestGrowableTracker_SplitAfterAllProcessed(t *testing.T) {
	estimator := offsetRangeEndEstimator{EstimateRangeEnd: 0}
	tracker, err := NewGrowableTracker(0, &estimator)
	if err != nil {
		t.Fatalf("error creating new GrowableTracker: %v", err)
	...
	if !tracker.TryClaim(int64(math.MaxInt64)) {
		t.Errorf("tracker.TryClaim(math.MaxInt64) = %v, want: %v", true, false)
	}
}
...
func TestGrowableTracker_Split(t *testing.T) {
	...
	if !tracker.TryClaim(int64(4)) {
		t.Errorf("tracker.TryClaim(4) = %v, want: %v", false, true)
	}
}
...
func TestGrowableTracker_IsBounded(t *testing.T) {
	...
	if tracker.IsBounded() {
		t.Errorf("GrowableTracker is bounded initially.")
	}
	...
	tracker, err = NewGrowableTracker(0, &offsetRangeEndEstimator{})
	if err != nil {
		t.Fatalf("error creating new GrowableTracker: %v", err)
	}
	if !tracker.TryClaim(int64(math.MaxInt64)) {
		t.Fatal("couldn't claim tracker.TryClaim(int64(math.MaxInt64))")
	}
	if !tracker.IsBounded() {
		t.Errorf("tracker should've been bounded, all work has been claimed.")
	}
}
...
func TestGrowableTracker_Progress(t *testing.T) {
	...
	start = 10
	estimator := offsetRangeEndEstimator{EstimateRangeEnd: 0}
	tracker, err := NewGrowableTracker(start, &estimator)
	if err != nil {
		t.Fatalf("error creating new GrowableTracker: %v", err)
	}
	cur = 20
	...
	if 0 != remaining {
		t.Errorf("work remaining:%v, expected: %v", remaining, 0)
	}

	estimator.SetEstimateRangeEnd(35)
	done, remaining = tracker.GetProgress()
	if float64(cur+1-start) != done {
		t.Errorf("wrong amount of work done, got: %v, want: %v", float64(cur+1-start), done)
	}
	if float64(35-cur) != remaining {
		t.Errorf("work remaining:%v, expected: %v", remaining, float64(35-cur))
	}

	estimator.SetEstimateRangeEnd(25)
	done, remaining = tracker.GetProgress()
	if float64(cur+1-start) != done {
		t.Errorf("wrong amount of work done, got: %v, want: %v", float64(cur+1-start), done)
	}
	if float64(25-cur) != remaining {
		t.Errorf("work remaining:%v, expected: %v", remaining, float64(25-cur))
	}

	estimator.SetEstimateRangeEnd(math.MaxInt64)
	done, remaining = tracker.GetProgress()
	if float64(cur+1-start) != done {
		t.Errorf("wrong amount of work done, got: %v, want: %v", float64(cur+1-start), done)
	}
	if float64(math.MaxInt64-cur) != remaining {
		t.Errorf("work remaining:%v, expected: %v", remaining, float64(math.MaxInt64-cur))
	}
}
