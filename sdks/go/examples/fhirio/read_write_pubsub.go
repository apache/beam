...
// read_write_pubsub is a streaming pipeline example using the fhirio connector
// to read FHIR resources from GCS, write them to a GCP FHIR store, and, upon
// store updates through PubSub notifications, read those written resources from
// the FHIR store and log them.
//
// Pre-requisites:
// 1. A Google Cloud FHIR store with PubSub notifications set up.
// (see: https://cloud.google.com/healthcare-api/docs/concepts/pubsub).
// 2. NDJSON-encoded FHIR resources stored in GCS.
// 3. Dataflow Runner enabled: https://cloud.google.com/dataflow/docs/quickstarts.
//
...
// path to the FHIR store where the resources should be written to, and the
// PubSub topic name your FHIR store is sending notifications to, in addition to
// the usual flags for the Dataflow runner.
//
...
var (
	...
	// Required flag with the pubsub topic of your FHIR store.
