...
class UnionHintTestCase(TypeHintTestCase):
  ...
  def test_getitem_nested_unions_flattened(self):
    ...
    hint = typehints.Union[
        typehints.Union[int, str], typehints.Union[float, bool]
    ]
...
class TupleHintTestCase(TypeHintTestCase):
  ...
  def test_type_check_valid_composite_types(self):
    hint = typehints.Tuple[
        typehints.Tuple[int, str], typehints.Tuple[int, bool]
    ]
...
class DictHintTestCase(TypeHintTestCase):
  ...
  def test_match_type_variables(self):
    ...
    self.assertEqual(
        {S: int, T: str}, hint.match_type_variables(typehints.Dict[int, str]))
...
class DecoratorHelpers(TypeHintTestCase):
  ...
  def test_getcallargs_forhints_builtins(self):
    if sys.version_info.major < 3:
      ...
      self.assertEquals({'self': str}, getcallargs_forhints(str.upper, str))
      # str.strip has an optional second argument.
      self.assertEquals(
          {'self': str, 'chars': Any}, getcallargs_forhints(str.strip, str))
      self.assertEquals(
          {'self': str, 'iterable': list},
          getcallargs_forhints(str.join, str, list))
