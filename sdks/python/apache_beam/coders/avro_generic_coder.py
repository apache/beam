...
class AvroGenericCoderImpl(SimpleCoderImpl):
  ...
  def encode(self, value):
    assert issubclass(type(value), AvroGenericRecord)
    with BytesIO() as buf:
  ...
  def decode(self, encoded):
    with BytesIO(encoded) as buf:
      return AvroGenericRecord(schemaless_reader(buf, self.parsed_schema))


class AvroGenericRecord(object):
  ...
  def __eq__(self, other):
    return (
        issubclass(type(other), AvroGenericRecord) and
