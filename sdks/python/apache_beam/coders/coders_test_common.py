...
class CodersTest(unittest.TestCase):
  ...
  def tearDownClass(cls):
    standard = set(
        ...
        and issubclass(c, coders.Coder)
        and 'Base' not in c.__name__)
    standard -= set(
        [
            coders.Coder,
  ...
  def test_deterministic_coder(self):
    ...
    self.check_coder(
        coders.TupleCoder((deterministic_coder, coder)),
        (1, dict()),
        ('a', [dict()]))
  ...
  def test_proto_coder(self):
    ...
    self.check_coder(
        coders.TupleCoder((proto_coder, coders.BytesCoder())),
        (ma, b'a'),
        (mb, b'b'))
