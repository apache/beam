...
class PipelineOptionsTest(unittest.TestCase):
  ...
  # TODO(mariagh): Require unique names only within a test.
  # For now, vp_arg<number> will be the convention to name value-provider
  # arguments in tests, as opposed to non_vp_arg<number>.
  # The number will grow as tests are added.
  # Last value-provider argument added to Beam (6/8/2017): vp_arg_12
  # Last non-value-provider argument added to Beam (6/8/2017): non_vp_arg_1
  def test_value_provider_options(self):
    ...
    options = UserOptions(['--vp_arg11', 'hello'])
    self.assertIsInstance(options.vp_arg11, StaticValueProvider)
    self.assertIsInstance(options.vp_arg12, RuntimeValueProvider)
    self.assertIsInstance(options.non_vp_arg1, int)

    # Values can be overwritten
    options = UserOptions(vp_arg11=5,
                          vp_arg12=StaticValueProvider(value_type=str,
                                                       value='bye'),
                          non_vp_arg1=RuntimeValueProvider(
                              option_name='foo',
                              value_type=int,
                              default_value=10))
    self.assertEqual(options.vp_arg11, 5)
    self.assertTrue(options.vp_arg12.is_accessible(),
                    '%s is not accessible' % options.vp_arg12)
    self.assertEqual(options.vp_arg12.get(), 'bye')
    self.assertFalse(options.non_vp_arg1.is_accessible())

    with self.assertRaises(RuntimeError):
      options.non_vp_arg1.get()
