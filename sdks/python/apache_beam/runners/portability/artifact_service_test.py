...
class BeamFilesystemArtifactServiceTest(unittest.TestCase):
  def setUp(self):
    ...
    self._service = artifact_service.BeamFilesystemArtifactService(
  ...
  def _run_staging(self, staging_service, retrieval_service):
    ...
    with self.assertRaises(Exception):
      staging_service.PutArtifact(
          iter(
              [
                  self.put_metadata(
                      staging_session_token, 'bad_hash', 'bad_hash'
                  ),
                  self.put_data(b'data'),
              ])
)
    ...
    manifest = beam_artifact_api_pb2.Manifest(
        artifact=[beam_artifact_api_pb2.ArtifactMetadata(name='name')])

    retrieval_token = staging_service.CommitManifest(
        beam_artifact_api_pb2.CommitManifestRequest(
            staging_session_token=staging_session_token, manifest=manifest)
    ).retrieval_token
