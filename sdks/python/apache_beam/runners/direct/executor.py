...
class _ExecutorServiceParallelExecutor(object):
  ...
  def __init__(
      self, value_to_consumers, transform_evaluator_registry, evaluation_context
  ):
  ...
  def start(self, roots):
    ...
    for root_node in self.root_nodes:
      provider = self.transform_evaluator_registry.get_root_bundle_provider(
          root_node)
  ...
  class _MonitorTask(_ExecutorService.CallableTask):
    ...
    def _fire_timers(self):
      ...
      for applied_ptransform, fired_timers in transform_fired_timers:
        ...
        empty_bundle = self._executor.evaluation_context.create_empty_committed_bundle(
