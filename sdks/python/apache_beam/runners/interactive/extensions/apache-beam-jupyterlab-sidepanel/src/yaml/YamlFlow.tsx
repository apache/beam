...
import {
    ...
    Panel,
    Node,
    Edge, applyNodeChanges,
    NodeChange, Connection
} from '@xyflow/react';
import { debounce } from 'lodash';
import dagre from 'dagre';
import { DefaultNode, InputNode, OutputNode, AnimatedSVGEdge } from './CustomStyle';
 ...
 * @param {(nodes: Node[]) => void} onNodesUpdate - Callback when nodes are updated
 * @param {(edges: Edge[]) => void} onEdgesUpdate - Callback when edges are updated
 * @param {(event: React.MouseEvent, node: Node) => void} onNodeClick - Callback when a node is clicked
 * @param {number} [debounceWait=500] - Debounce wait time in milliseconds for updates
 */
export const FlowEditor: React.FC<FlowEditorProps> = ({
    ...
    const applyAutoLayout = useCallback((nodes: Node[], edges: Edge[]): Node[] => {
    ...
    const handleNodesChange = useCallback(
        (changes: NodeChange[]) => {
            ...
            // Judge whether the node data is changed or not（except position / dragging / selected）
            ...
            if (structuralChanges.length > 0) {
                ...
                console.log('Auto-layout applied:', autoLayouted);
                console.log('Changes:', structuralChanges);
            } else {
    ...
    const NodeTypes =
        { default: DefaultNode, input: InputNode, output: OutputNode };
        ...
        >
            <Panel position="top-right">
                <button
                    ...
                    className="px-3 py-1 bg-blue-600 text-white rounded-md shadow hover:bg-blue-700 transition-all duration-200"
                >
                    Auto Layout
                </button>
            </Panel>
