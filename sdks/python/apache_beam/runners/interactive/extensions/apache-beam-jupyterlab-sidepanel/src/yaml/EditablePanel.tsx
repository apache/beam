...
/**
 * An editable key-value panel component for displaying and modifying node properties.
 * 
 * Features:
 ...
 * 
 * Props:
 * @param {Node} node - The node containing data to be edited
 * @param {(data: Record<string, any>) => void} onChange - Callback for data changes
 ...
 */
export class EditableKeyValuePanel extends React.Component<EditableKeyValuePanelProps, EditableKeyValuePanelState> {
    static defaultProps = {
        depth: 0,
    };
    ...
    componentDidUpdate(prevProps: EditableKeyValuePanelProps) {
        if (prevProps.node !== this.props.node && this.props.node) {
            ...
            console.log('EditableKeyValuePanel updated with new node data:', this.props.node.data);
        }
    }
    ...
    handleKeyChange = (oldKey: string, newKey: string) => {
        newKey = newKey.trim();
        if (newKey === oldKey || newKey === '') return alert('Invalid Key!');
        if (this.state.localData.hasOwnProperty(newKey)) return alert('Duplicated Key!');
    ...
    handleAddPair = () => {
        ...
        while (this.state.localData.hasOwnProperty(`${baseKey}${i}`)) i++;
    ...
    convertToObject = (key: string) => {
        if (typeof this.state.localData[key] === 'object' && this.state.localData[key] !== null) return;
    ...
    renderValueEditor = (key: string, value: any) => {
        const isMultiline = key === 'callable' || (typeof value === 'string' && value.includes('\n'));
