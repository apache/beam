      ...
      for dep in env.dependencies:
        ...
        role_payload = (
            beam_runner_api_pb2.ArtifactStagingToRolePayload.FromString(
                dep.role_payload))
        if type_payload.path and type_payload.path in path_hashes:
          _LOGGER.info(
              'Found duplicated artifact path: %s (%s)',
              type_payload.path,
              type_payload.sha256)
          staged_name = path_hashes[type_payload.path]
          dep.role_payload = beam_runner_api_pb2.ArtifactStagingToRolePayload(
              staged_name=staged_name).SerializeToString()
        elif type_payload.sha256 and type_payload.sha256 in sha256_hashes:
          _LOGGER.info(
              'Found duplicated artifact sha256: %s (%s)',
              type_payload.path,
              type_payload.sha256)
          staged_name = sha256_hashes[type_payload.sha256]
          dep.role_payload = beam_runner_api_pb2.ArtifactStagingToRolePayload(
              staged_name=staged_name).SerializeToString()
        else:
          staged_name = role_payload.staged_name
          resources.append((type_payload.path, staged_name))
          path_hashes[type_payload.path] = staged_name
          sha256_hashes[type_payload.sha256] = staged_name
