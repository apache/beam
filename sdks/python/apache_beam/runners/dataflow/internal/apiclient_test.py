...
class UtilTest(unittest.TestCase):
  ...
  def test_flexrs_cost(self):
    ...
    self.assertEqual(
        ...
        (
            dataflow.Environment.FlexResourceSchedulingGoalValueValuesEnum.FLEXRS_COST_OPTIMIZED
        ))
  ...
  def test_flexrs_speed(self):
    ...
    self.assertEqual(
        env.proto.flexResourceSchedulingGoal,
        (
            dataflow.Environment.FlexResourceSchedulingGoalValueValuesEnum.FLEXRS_SPEED_OPTIMIZED
        ))
  ...
  def test_translate_distribution_using_dataflow_distribution_counter(self):
    ...
    self.assertEqual(
        metric_proto.distribution.histogram.bucketCounts, histogram.bucketCounts
    )
    self.assertEqual(
  ...
  def test_default_ip_configuration(self):
    pipeline_options = PipelineOptions(
        ['--temp_location', 'gs://any-location/temp'])
    env = apiclient.Environment(
        [], pipeline_options, '2.0.0', FAKE_PIPELINE_URL)
  ...
  def test_worker_harness_image_tag_matches_released_sdk_version(self):
    ...
    if sys.version_info[0] == 2:
      ...
      self.assertEqual(
          ...
          (
              names.DATAFLOW_CONTAINER_IMAGE_REPOSITORY
              + '/python%d%d:2.2.0' % (sys.version_info[0], sys.version_info[1])
          ))
  ...
  @mock.patch(
      'apache_beam.runners.dataflow.internal.apiclient.sys.version_info', (3, 5)
  )
  ...
  @mock.patch(
      'apache_beam.runners.dataflow.internal.apiclient.sys.version_info', (2, 7)
  )
