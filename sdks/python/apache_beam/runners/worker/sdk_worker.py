...
class SdkWorker(object):
  ...
  def process_bundle(
    ...
    # type: (...) -> beam_fn_api_pb2.InstructionResponse
    _LOGGER.info(f"RTOP Investigation: Processing bundle with instruction_id: {instruction_id}")
    _LOGGER.info(f"RTOP Investigation: ProcessBundleRequest details: {request}")
    bundle_processor = self.bundle_processor_cache.get(
        instruction_id, request.process_bundle_descriptor_id)
    
    _LOGGER.info(f"RTOP Investigation: Retrieved bundle processor for instruction_id {instruction_id}. "
    ...
    try:
      with bundle_processor.state_handler.process_instruction_id(
        ...
        _LOGGER.info(f"RTOP Investigation: Processing state for instruction_id: {instruction_id} "
                          f"with cache tokens: {request.cache_tokens}")
        with self.maybe_profile(instruction_id):
          _LOGGER.info(f"RTOP Investigation: Starting bundle processing with instruction_id: {instruction_id}")
          delayed_applications, requests_finalization = (
              bundle_processor.process_bundle(instruction_id))
          
          _LOGGER.info(f"RTOP Investigation: Bundle processing completed for instruction_id {instruction_id}. "
                              f"Delayed applications: {delayed_applications}, "
                              f"Requests finalization: {requests_finalization}")

          monitoring_infos = bundle_processor.monitoring_infos()
          _LOGGER.info(f"RTOP Investigation: Monitoring information for instruction_id {instruction_id}: {monitoring_infos}")
          response = beam_fn_api_pb2.InstructionResponse(
              instruction_id=instruction_id,
      ...
      if not requests_finalization:
        _LOGGER.info(f"RTOP Investigation: Releasing bundle processor for instruction_id {instruction_id} "
                      f"since finalization is not required.")
        self.bundle_processor_cache.release(instruction_id)
      else:
        _LOGGER.info(f"RTOP Investigation: Bundle processor for instruction_id {instruction_id} retained for finalization.")
      return response
    except:  # pylint: disable=bare-except
      # Don't re-use bundle processors on failure.
      _LOGGER.error(f"RTOP Investigation: Error encountered during bundle processing for instruction_id: {instruction_id}. "
