...
class ShortIdCache(object):
  ...
  def get_infos(
      self, short_ids: Iterable[str]) -> Dict[str, metrics_pb2.MonitoringInfo]:
...
class SdkHarness(object):
  ...
  def __init__(
    ...
    self._responses: queue.Queue[Union[beam_fn_api_pb2.InstructionResponse,
  ...
  def _request_harness_monitoring_infos(
      self, request: beam_fn_api_pb2.InstructionRequest) -> None:
...
class BundleProcessorCache(object):
  ...
  @staticmethod
  def _shutdown_cached_bundle_processors(
      cached_bundle_processors: List[bundle_processor.BundleProcessor]) -> None:
    try:
      while True:
        # pop() is threadsafe
...
class SdkWorker(object):
  def __init__(
      ...
      state_cache_metrics_fn: Callable[
          [], Iterable[metrics_pb2.MonitoringInfo]] = list,
      profiler_factory: Optional[Callable[..., Profile]] = None,
  ) -> None:
...
class GrpcStateHandlerFactory(StateHandlerFactory):
  ...
  def __init__(
      ...
      credentials: Optional[grpc.ChannelCredentials] = None) -> None:
...
class CachingStateHandler(metaclass=abc.ABCMeta):
  @abc.abstractmethod
  @contextlib.contextmanager
  def process_instruction_id(
      self,
      bundle_id: str,
      cache_tokens: Iterable[beam_fn_api_pb2.ProcessBundleRequest.CacheToken]
  ) -> Iterator[None]:
    raise NotImplementedError(type(self))
...
class GrpcStateHandler(StateHandler):
  ...
  def __init__(self, state_stub: beam_fn_api_pb2_grpc.BeamFnStateStub) -> None:
    ...
    self._requests: queue.Queue[Union[beam_fn_api_pb2.StateRequest,
                                      Sentinel]] = queue.Queue()
  ...
  def _request(
      self, request: beam_fn_api_pb2.StateRequest
  ) -> _Future[beam_fn_api_pb2.StateResponse]:
...
class GlobalCachingStateHandler(CachingStateHandler):
  ...
  """
  def __init__(
      self, global_state_cache: StateCache,
      underlying_state: StateHandler) -> None:
  ...
  def _get_cache_token(self,
                       state_key: beam_fn_api_pb2.StateKey) -> Optional[bytes]:
  ...
  class ContinuationIterable(Generic[T]):
    def __init__(
        self,
        head: Iterable[T],
        continue_iterator_fn: Callable[[], Iterable[T]]) -> None:
      self.head = head
