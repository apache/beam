...
class SdkHarness(object):
  ...
  def __init__(self,
    ...
    self._process_bundle_queue = queue.Queue()  # type: queue.Queue[beam_fn_api_pb2.InstructionRequest]
    self._unscheduled_process_bundle = {}  # type: Dict[str, float]
...
class StateHandlerFactory(with_metaclass(abc.ABCMeta, object)):  # type: ignore[misc]
  ...
  def create_state_handler(self, api_service_descriptor):
    # type: (endpoints_pb2.ApiServiceDescriptor) -> CachingMaterializingStateHandler
...
class GrpcStateHandlerFactory(StateHandlerFactory):
  ...
  def __init__(self, state_cache_size, credentials=None):
    self._state_handler_cache = {}  # type: Dict[str, CachingMaterializingStateHandler]
