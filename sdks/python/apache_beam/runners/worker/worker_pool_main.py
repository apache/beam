...
class BeamFnExternalWorkerPoolServicer(
  ...
  def StartWorker(self,
                  ...
                  unused_context
                 ) -> beam_fn_api_pb2.StartWorkerResponse:
    try:
  ...
  def StopWorker(self,
                 ...
                 unused_context
                ) -> beam_fn_api_pb2.StopWorkerResponse:
...
def main(argv=None):
  ...
  _LOGGER.info(
      'Started worker pool servicer at port: %s with executable: %s',
      address,
      args.container_executable)
  try:
    while True:
      time.sleep(60 * 60 * 24)
