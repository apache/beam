...
if isinstance(InvalidDataError, type):
  PERMANENT_ERROR_TYPES = [PermanentException, InvalidDataError]
else:
...
def retry_on_server_errors_filter(exception):
  ...
  return not isinstance(exception, tuple(PERMANENT_ERROR_TYPES))
