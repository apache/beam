...
def _stage_extra_packages(extra_packages, staging_location, temp_dir,
  ...
  for package in extra_packages:
    ...
    if os.path.basename(package).endswith('.whl'):
      logging.warning(
          'The .whl package "%s" is provided in --extra_package. This '
          'functionality is not officially supported. Since wheel packages '
          'are binary distributions, this package must be binary-compatible '
          'with the worker environment (i.e. Python 2.7 running on an x64 '
          'Linux host).')
