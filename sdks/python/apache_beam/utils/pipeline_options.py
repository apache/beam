...
class SetupOptions(PipelineOptions):
  ...
  def _add_argparse_args(cls, parser):
    ...
    parser.add_argument(
        ...
        default='default',
        help=
        ('Override the default location from where Beam SDK is downloaded. It '
         'can be a URL, a GCS path, or a local path to an SDK tarball. '
         'Workflow submissions will download or copy an SDK tarball from here. '
         'If the string "default", a standard SDK location is used. If empty, '
         'no SDK is copied.'))
    parser.add_argument(
