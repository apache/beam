...
class WindowedBatch(object):
  ...
  def __init__(
    ...
    # TODO: Store timestamps_micros, convert to Timestamp as needed (as in
    # WindowedValue)
    self.timestamp_objects = [None] * len(timestamps)

    def convert_timestamp(timestamp: TimestampTypes) -> int:
      if isinstance(timestamp, int):
        return timestamp * 1000000, None
      else:
        # TODO: Cache Timestamp object as in WindowedValue?
        timestamp_object = (
            timestamp
            if isinstance(timestamp, Timestamp) else Timestamp.of(timestamp))
        return timestamp_object.micros, timestamp_object

    self.timestamp_objects = None
    self.timestamps_micros, self._tmp_timestamp_objects = zip(*[convert_timestamp(t) for t in timestamps])
    self.windows = windows
  ...
  def timestamps(self) -> Sequence[Timestamp]:
    if self.timestamp_objects is None:
      self.timestamp_objects = list(self._tmp_timestamp_objects)
      for i, obj in enumerate(self.timestamp_objects):
        if obj is None:
          self.timestamp_objects[i] = Timestamp(0, self.timestamps_micros[i])
