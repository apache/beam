...
class QdrantConnectionParameters:
    ...
    def __post_init__(self):
        if not (self.location or self.url or self.host or self.path):
            raise ValueError(
                "At least one of location, url, host, or path must be provided for Qdrant connection"
            )
...
class QdrantHybridSearchParameters:
    ...
    def __post_init__(self):
        if not self.dense or not self.sparse:
            raise ValueError(
                "Both dense and sparse search parameters must be provided for hybrid search"
            )
        if self.limit <= 0:
            raise ValueError(f"Search limit must be positive, got {self.limit}")
...
class QdrantSearchEnrichmentHandler(EnrichmentSourceHandler[InputT, OutputT]):
    ...
    def __call__(self, request: Union[Chunk, List[Chunk]], *args, **kwargs) -> OutputT:
    ...
    def _dense_search(self, chunks: List[Chunk]):
        ...
        for chunk in chunks:
            if not chunk.embedding or chunk.embedding.dense_embedding is None:
                raise ValueError(
                    f"Chunk {getattr(chunk, 'id', None)} missing dense embedding required for dense search"
                )
    ...
    def _sparse_search(self, chunks: List[Chunk]):
        ...
        for chunk in chunks:
            if not chunk.embedding or chunk.embedding.sparse_embedding is None:
                raise ValueError(
                    f"Chunk {getattr(chunk, 'id', None)} missing sparse embedding required for sparse search"
                )
    ...
    def _hybrid_search(self, chunks: List[Chunk]):
        ...
        for chunk in chunks:
            ...
            if dense_vec is None or sparse_emb is None:
                raise ValueError(
                    f"Chunk {getattr(chunk, 'id', None)} missing dense or sparse embedding required for hybrid search"
                )
    ...
    def _get_call_response(
        ...
        response = []
        for chunk, result in zip(chunks, search_result):
            hits = []
            for point in result:
                hit = {"id": point.id, "score": point.score, "payload": point.payload}
                hits.append(hit)
            response.append((chunk, {"hits": hits}))
        return response
