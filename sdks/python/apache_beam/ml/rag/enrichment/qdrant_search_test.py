...
class TestQdrantSearchEnrichment(unittest.TestCase):
    def test_invalid_connection_parameters(self):
        with self.assertRaises(ValueError) as context:
            ...
            _ = QdrantSearchEnrichmentHandler(
                connection_parameters=connection_params, search_parameters=search_params
            )
        self.assertIn("At least one of location, url, host, or path must be provided for Qdrant connection", str(context.exception))
    ...
    def test_invalid_search_parameters(self, create_params, expected_error_msg):
        with self.assertRaises(ValueError) as context:
            connection_params = QdrantConnectionParameters(url="http://localhost:6333")
...
class TestQdrantDenseSearchEnrichment(unittest.TestCase):
    ...
    def test_invalid_dense_search_parameters(self, create_params, expected_error_msg):
        with self.assertRaises(ValueError) as context:
            connection_params = QdrantConnectionParameters(url="http://localhost:6333")
            dense_search_params = create_params()
            search_params = QdrantSearchParameters(
                collection_name="test_collection", search_strategy=dense_search_params
            )
            _ = QdrantSearchEnrichmentHandler(
                connection_parameters=connection_params, search_parameters=search_params
            )
    ...
    def test_missing_dense_embedding(self):
        with self.assertRaises(ValueError) as context:
            chunk = Chunk(id=1, content=None, embedding=Embedding(dense_embedding=None))
            connection_params = QdrantConnectionParameters(url="http://localhost:6333")
        ...
        self.assertIn(
            "missing dense embedding required for dense search", str(context.exception)
        )
...
class TestQdrantSparseSearchEnrichment(unittest.TestCase):
    ...
    def test_invalid_sparse_search_parameters(self, create_params, expected_error_msg):
        with self.assertRaises(ValueError) as context:
            connection_params = QdrantConnectionParameters(url="http://localhost:6333")
            sparse_search_params = create_params()
            search_params = QdrantSearchParameters(
                collection_name="test_collection", search_strategy=sparse_search_params
            )
            _ = QdrantSearchEnrichmentHandler(
                connection_parameters=connection_params, search_parameters=search_params
            )
    ...
    def test_missing_sparse_embedding(self):
        ...
        self.assertIn(
            "missing sparse embedding required for sparse search",
...
class TestQdrantHybridSearchEnrichment(unittest.TestCase):
    @parameterized.expand(
        [
            ...
            (
                lambda: QdrantHybridSearchParameters(
                    dense=QdrantDenseSearchParameters(vector_name="dense"), sparse=None
                ),
                "Both dense and sparse search parameters must be provided for hybrid search",
            ),
            (
    ...
    def test_invalid_hybrid_search_parameters(self, create_params, expected_error_msg):
        with self.assertRaises(ValueError) as context:
            connection_params = QdrantConnectionParameters(url="http://localhost:6333")
            hybrid_search_params = create_params()
            search_params = QdrantSearchParameters(
                collection_name="test_collection", search_strategy=hybrid_search_params
            )
            _ = QdrantSearchEnrichmentHandler(
                connection_parameters=connection_params, search_parameters=search_params
            )
