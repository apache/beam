...
MILVUS_INGESTION_IT_CONFIG = {
    "fields": [
        ...
        FieldSchema(
            name="embedding", dtype=DataType.FLOAT_VECTOR, dim=3),
        FieldSchema(
...
def create_collection_with_partition(
  client: MilvusClient, collection_name: str, partition_name: str='',
...
class TestMilvusVectorWriterConfig(unittest.TestCase):
  ...
  def test_write_on_existent_collection_with_default_schema(self):
    ...
    for chunk in test_chunks:
      ...
      self.assertEqual(
        result_item["content"],
        chunk.content.text if hasattr(chunk.content, 'text') else chunk.content)
  ...
  def test_write_with_custom_column_specifications(self):
    ...
    custom_column_specs = [
        ...
        ColumnSpec(
            "content", str,
            lambda chunk: (chunk.content.text if hasattr(chunk.content, 'text') 
                          else chunk.content)),
        ColumnSpec("metadata", dict, lambda chunk: chunk.metadata or {}),
        ColumnSpec(
            "embedding", list,
            lambda chunk: chunk.embedding.dense_embedding or []),
        ColumnSpec(
            "sparse_embedding", dict,
            lambda chunk: (MilvusHelpers.sparse_embedding(
                chunk.embedding.sparse_embedding)
                if chunk.embedding and chunk.embedding.sparse_embedding 
                else {}))
    ]
  ...
  def test_idempotent_write(self):
    ...
    with TestPipeline(is_integration_test=True) as p:
      ...
      _ = (p | "Create initial" >> beam.Create(initial_chunks) | 
    ...
    with TestPipeline(is_integration_test=True) as p:
      p.not_use_test_runner_api = True
      _ = (p | "Create update1" >> beam.Create(updated_chunks) | 
    ...
    for i in range(3):
      with TestPipeline(is_integration_test=True) as p:
        p.not_use_test_runner_api = True
        _ = (p | f"Create repeat{i+2}" >> beam.Create(updated_chunks) | 
