...
class SpannerVectorWriterTest(unittest.TestCase):
  ...
  def test_write_flattened_metadata(self):
    ...
    chunks = [
        Chunk(
            ...
            embedding=Embedding(dense_embedding=[1.0, 2.0, 3.0]),
            content=Content(text='First document'),
            metadata={'source': 'book.pdf', 'page': 10, 'author': 'John'}),
        Chunk(
            id='doc2',
            embedding=Embedding(dense_embedding=[4.0, 5.0, 6.0]),
            content=Content(text='Second document'),
            metadata={'source': 'article.txt', 'page': 5, 'author': 'Jane'}),
    ]
    ...
    specs = (
        ...
        .with_embedding_spec()
        .with_content_spec()
        .add_metadata_field('source', str, column_name='source')
        .add_metadata_field('page', int, column_name='page_number')
        .with_metadata_spec()
        .build()
    )
  ...
  def test_write_with_converter(self):
    ...
    specs = (
        SpannerColumnSpecsBuilder()
        .with_id_spec()
        .with_embedding_spec(convert_fn=normalize)
        .with_content_spec()
        .with_metadata_spec()
        .build()
    )
  ...
  def test_write_with_timestamp(self):
    ...
    specs = (
        ...
        .with_embedding_spec()
        .with_content_spec()
        .add_metadata_field('created_at', str, column_name='created_at')
        .with_metadata_spec()
        .build()
    )
