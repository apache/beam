...
class OpenAIEmbeddingsTest(unittest.TestCase):
    ...
    def test_openai_text_embeddings(self):
        ...
        with beam.Pipeline() as pipeline:
            transformed_pcoll = (
                pipeline
                | "CreateData" >> beam.Create([{test_query_column: test_query}])
                | "MLTransform"
                >> MLTransform(
    ...
    def pipeline_with_configurable_artifact_location(
        ...
        if write_artifact_location:
            return pipeline | MLTransform(
                write_artifact_location=write_artifact_location
            ).with_transform(embedding_config)
        elif read_artifact_location:
            return pipeline | MLTransform(read_artifact_location=read_artifact_location)
        else:
    ...
    def test_embeddings_with_read_artifact_location(self):
        ...
        with beam.Pipeline() as p:
            data = p | "CreateData" >> beam.Create([{test_query_column: test_query}])
        ...
        with beam.Pipeline() as p:
            data = p | "CreateData" >> beam.Create(
                [{test_query_column: test_query}, {test_query_column: test_query}]
            )
    ...
    def test_with_int_data_types(self):
        ...
        with self.assertRaises(Exception):
            with beam.Pipeline() as pipeline:
                _ = (
                    pipeline
                    | "CreateData" >> beam.Create([{test_query_column: 1}])
                    | "MLTransform"
                    >> MLTransform(
                        write_artifact_location=self.artifact_location
                    ).with_transform(embedding_config)
    ...
    def test_mltransform_to_ptransform_with_openai(self):
        ...
        ptransform_list = (
            base._MLTransformToPTransformMapper.load_transforms_from_artifact_location(
