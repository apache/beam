...
class TensorRTRunInferenceTest(unittest.TestCase):
  ...
  def test_inference_single_tensor_feature(self):
    """
    This tests creating TensorRT network from scratch. Test replicates the same ONNX network above 
  ...
  def test_inference_single_tensor_feature_built_engine(self):
    """
    This tests already pre-built TensorRT engine from ONNX network. To execute this test
    succesfully, TensorRT engine that is used here, must have been built in the same 
    environment with the same GPU that will be used when running a test. In other words,
    using the same environment and same GPU we must pre-build the engine and after we run
    this test. Otherwise behavior might be unpredictable, read more:
