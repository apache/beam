...
Run as
   ...
   python -m apache_beam.tools.timestamped_value_microbenchmark
"""
...
def run_benchmark(num_readers=10, num_runs=10, num_elements_step=100):
  timings = {}
  for run in range(num_runs):
    num_elements = num_elements_step * run + 1
    start = time.time()
    with beam.Pipeline() as p:
      pc = p | 'start' >> beam.Create([(k, k) for k in range(num_elements)])
      for ix in range(num_readers):
        _ = (pc
             | 'map%d' % ix >> beam.Map(lambda x_t:
                                        TimestampedValue(x_t[0], x_t[1]))
             | 'w%d' % ix >> WindowInto(FixedWindows(5))
             | 'key%d' % ix >> beam.Map(lambda v: ('key', v))
             | 'group%d' % ix >> beam.GroupByKey())
  ...
  print("Per-element ", gradient // num_readers)
  print("R^2         ", r_value**2)
