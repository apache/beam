...
from apache_beam.portability.api import beam_pipeline_metrics_pb2
...
class CounterCell(Counter, MetricCell):
  ...
  def to_runner_api_monitoring_info(self):
    ...
    return beam_pipeline_metrics_pb2.Metric(
        counter_data=beam_pipeline_metrics_pb2.CounterData(
...
class DistributionData(object):
  ...
  def to_runner_api_monitoring_info(self):
    """Returns a Metric with this value for use in a MonitoringInfo."""
    return beam_pipeline_metrics_pb2.Metric(
        distribution_data=beam_pipeline_metrics_pb2.DistributionData(
            int_distribution_data=beam_pipeline_metrics_pb2.IntDistributionData(
                count=self.count, sum=self.sum, min=self.min, max=self.max)))
