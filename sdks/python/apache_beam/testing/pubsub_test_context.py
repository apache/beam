    ...
    def register_subscription(self, subscription_path: str):
        ...
        if subscription_path not in self.tracked_subscriptions:
            ...
            logger.info(
                ...
                self.caller_class, subscription_path
            )

    def __enter__(self):
        logger.info(
            "[START] [%s] Initializing Pub/Sub context (dry_run=%s)",
            self.caller_class, self.dry_run
        )
        return self
                ...
                if self.dry_run:
                    ...
                    try:
                        ...
                        logger.error(
                            ...
                            self.caller_class, sub_path, e
                        )
        except Exception as e:
            logger.error(
                "[%s] [Error] Could not list subs for topic %s: %s",
                self.caller_class, topic_path, e
            )
