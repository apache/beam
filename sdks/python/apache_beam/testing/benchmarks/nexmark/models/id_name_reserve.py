...
class IdNameReserveCoder(FastCoder):
  def _create_impl(self):
    return IdNameReserveCoderImpl()

  def is_deterministic(self):
    return True


class IdNameReserve(object):
  CODER = IdNameReserveCoder()

  def __init__(self, id, name, reserve):
    self.id = id
    self.name = name
...
class IdNameReserveCoderImpl(coder_impl.StreamCoderImpl):
  ...
  def decode_from_stream(self, stream, nested):
    ...
    return IdNameReserve(id, name, reserve)
