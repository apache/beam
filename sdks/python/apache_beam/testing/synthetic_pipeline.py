...
class SyntheticSource(iobase.BoundedSource):
  ...
  def __init__(self, input_spec):
    ...
    self._initial_splitting = (
        ...
        if 'bundleSizeDistribution' in input_spec
        else 'const')
    ...
    self._initial_splitting_num_bundles = (
        ...
        if 'forceNumInitialBundles' in input_spec
        else 0)
    ...
    self._dynamic_splitting = (
        'none'
        if ('splitPointFrequencyRecords' in input_spec
    ...
    if 'delayDistribution' in input_spec:
      ...
      if (self._sleep_per_input_record_sec
          and self._sleep_per_input_record_sec < 1e-3):
        raise ValueError(
  ...
  def split(self, desired_bundle_size, start_position=0, stop_position=None):
    ...
    if self._initial_splitting == 'zipf':
      ...
      samples = np.random.zipf(
          self._initial_splitting_distribution_parameter, desired_num_bundles)
      total = sum(samples)
      ...
      bundle_ranges = []
      for start in range(
          start_position, stop_position, bundle_size_in_elements):
...
def merge_using_gbk(name, pc1, pc2):
  ...
  grouped = {'pc1': pc1_with_key, 'pc2': pc2_with_key} | (
      name + 'Group'
  ) >> beam.CoGroupByKey()
  return grouped | (name + 'DeDup') >> beam.Map(
      lambda elm: elm[0]
  )  # Ignoring values
...
def expand_using_second_output(name, pc):
  ...
  pc1, pc2 = pc | name >> beam.ParDo(ExpandFn()).with_outputs(
      'second_out', main='main_out')
  return [pc1, pc2]
...
def _parse_steps(json_str):
  ...
  for val in json_data:
    ...
    steps['per_element_delay'] = (
        ...
        if 'per_element_delay_msec' in val
        else 0)
    steps['per_bundle_delay'] = (
        float(val['per_bundle_delay_sec'])
        if 'per_bundle_delay_sec' in val
        else 0)
    steps['output_records_per_input_record'] = (
        int(val['output_records_per_input_record'])
        if 'output_records_per_input_record' in val
        else 1)
    steps['output_filter_ratio'] = (
        float(val['output_filter_ratio']) if 'output_filter_ratio' in val else 0
    )
...
def parse_args(args):
  ...
  parser.add_argument(
      '--output', dest='output', default='', help='Destination to write output.'
  )
...
def run(argv=None):
  ...
  with TestPipeline(options=pipeline_options) as p:
    ...
    pc_list = []
    num_roots = (
        2 ** (len(known_args.steps) - 1)
        if (barrier == 'merge-gbk' or barrier == 'merge-side-input')
        else 1)
