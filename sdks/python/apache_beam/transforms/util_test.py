...
class BatchElementsTest(unittest.TestCase):
  ...
  def test_target_duration(self):
    ...
    batch_duration = lambda batch_size: 1 + 0.7 * batch_size
  ...
  def test_target_overhead(self):
    clock = FakeClock()
    batch_estimator = util._BatchSizeEstimator(
        target_batch_overhead=0.05, target_batch_duration_secs=None, clock=clock
    )
    batch_duration = lambda batch_size: 1 + 0.7 * batch_size
...
class IdentityWindowTest(unittest.TestCase):
  def test_window_preserved(self):
    ...
    assert_that(
        ...
        reify_windows=True)
    after_identity = before_identity | 'window' >> beam.WindowInto(
        beam.transforms.util._IdentityWindowFn(coders.IntervalWindowCoder()))
    assert_that(
        after_identity,
        equal_to(expected_windows),
  ...
  def test_no_window_context_fails(self):
    ...
    after_identity = (
        ...
        | 'window'
        >> beam.WindowInto(
            beam.transforms.util._IdentityWindowFn(coders.GlobalWindowCoder())
        )
...
class ReshuffleTest(unittest.TestCase):
  ...
  def test_reshuffle_after_gbk_contents_unchanged(self):
    ...
    after_gbk = pipeline | beam.Create(data) | beam.GroupByKey()
