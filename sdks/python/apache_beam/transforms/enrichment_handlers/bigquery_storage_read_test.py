...
class TestBigQueryStorageEnrichmentHandler(unittest.TestCase):
    ...
    @mock.patch.object(
        bigquery_storage_read.BigQueryStorageEnrichmentHandler, "_execute_storage_read"
    )
    ...
    def test_call_single_match_all_columns_aliased(self, mock_exec):
        ...
        handler = self.make_handler(
            column_names=["id as user_id", "value as user_value"], fields=["id"]
        )  # Note: fields still uses original column name
    ...
    def test_call_batch_all_columns_aliased(self, mock_exec):
        ...
        # Create handler with all columns aliased
        handler = self.make_handler(
            column_names=["id as customer_id", "value as customer_name"], fields=["id"]
        )
        ...
        self.assertEqual(result[0][1].customer_name, "John Doe")  # Aliased column name
        ...
        self.assertEqual(
            result[1][1].customer_name, "Jane Smith"
        )  # Aliased column name
