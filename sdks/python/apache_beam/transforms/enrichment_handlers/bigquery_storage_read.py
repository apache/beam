...
class BigQueryStorageEnrichmentHandler(
    EnrichmentSourceHandler[
        Union[BeamRow, list[BeamRow]], Union[BeamRow, list[BeamRow]]
    ]
):
        ...
        if self._select_all_columns:
            ...
            if self.primary_keys:
                for pk_field in self.primary_keys:
                    original_pk_name = {v: k for k, v in self._rename_map.items()}.get(
                        pk_field, pk_field
                    )
    ...
    def _get_condition_values_dict(self, req: BeamRow) -> Optional[Dict[str, Any]]:
        try:
            ...
            logger.error(
                "Error getting condition values for row %s: %s", req, e, exc_info=True
            )
            return None

    def _build_single_row_filter(
        self, req_row: BeamRow, condition_values_dict: Dict[str, Any]
    ) -> str:
    ...
    def _execute_storage_read(self, combined_row_filter: str) -> List[BQRowDict]:
        ...
        if max_workers <= 0:
            max_workers = 1
        logger.debug(
            "Reading %s API streams using %s threads.", num_api_streams, max_workers
        )
        futures = []
        try:
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ...
                        logger.error(
                            "Error processing future result: %s", e, exc_info=True
                        )
    ...
    def create_row_key(
        self, row: BeamRow, is_bq_result: bool = False
    ) -> Optional[tuple]:
        try:
            if self.condition_value_fn:
    ...
    def _process_bq_results_for_batch(
        self, bq_results_list_orig_keys: List[BQRowDict]
    ) -> Dict[tuple, List[BeamRow]]:
    ...
    def _select_response_row(
        self, matching_bq_rows: List[BeamRow], req_row: BeamRow
    ) -> BeamRow:
    ...
    def _process_batch_request(
        self, request: list[BeamRow]
    ) -> List[Tuple[BeamRow, BeamRow]]:
        """
        ...
        bq_results_key_map: Dict[tuple, List[BeamRow]] = {}  # Key -> BQ results mapping
        ...
        for req_key, req_row in requests_map.items():
            ...
            selected_response_row = self._select_response_row(matching_bq_rows, req_row)
    ...
    def _process_single_request(self, request: BeamRow) -> Tuple[BeamRow, BeamRow]:
        ...
        if bq_results_orig_keys:
            ...
            if self._latest_value_callback and renamed_bq_rows:
                try:
                    response_row = (
                        self._latest_value_callback(renamed_bq_rows, req_row)
                        or BeamRow()
                    )
                except Exception as cb_error:
                    logger.error(
            ...
            if len(bq_results_orig_keys) > 1 and not (
                self._latest_value_callback and response_row != BeamRow()
            ):  # Log if multiple and
    ...
    def get_cache_key(
        self, request: Union[BeamRow, list[BeamRow]]
    ) -> Union[str, List[str]]:
        if isinstance(request, list):
            return [
