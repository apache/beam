...
class BigQueryStorageEnrichmentHandler(
  ...
  def __init__(
    ...
    if column_names:
      for name_or_alias in column_names:
        ...
        if match:
          original_col, alias_col = match.group(1).strip(), match.group(
            2).strip()
    ...
    if self._select_all_columns:
      ...
      if key_gen_fields_set:
        logger.debug(
            "Selecting all columns ('*'). Key fields %s assumed "
            "present.",
            key_gen_fields_set)
    else:
    ...
    logger.info(
        "Handler Initialized. Selecting BQ Columns: %s. Renaming "
        "map: %s",
        self._bq_select_columns,
        self._rename_map)
  ...
  def _build_single_row_filter(
    ...
    try:
      if self.row_restriction_template_fn:
        ...
        raise ValueError(
            "Internal Error: No template or template function "
            "available.")
  ...
  def _execute_storage_read(self, combined_row_filter: str) -> List[BQRowDict]:
    ...
    table_resource = (
        f"projects/{table_project}/datasets/{dataset_id}/"
        f"tables/{table_id}")
    ...
    try:
      ...
      logger.error(
          "Unexpected error creating session. Filter: '%s'. Error: "
          "%s",
          combined_row_filter,
          e,
          exc_info=True)
    ...
    if (self.max_parallel_streams is not None and
    ...
    try:
      if self.condition_value_fn:
        ...
        if len(key_values_dict) != len(self.fields):  # Ensure all key fields
          # found and not None
          logger.debug(
              "Row missing key field(s) or None. Cannot generate "
              "key: %s",
              row)
          return None
      else:
      ...
      logger.error(
          "Failed _asdict() for key gen. Type: %s. Ensure input is "
          "beam.Row.",
          type(row))
      return None
    except Exception as e:
  ...
  def _process_batch_request(
    ...
    for req_row in request:
      ...
      if req_key not in requests_map:
        ...
        logger.warning(
            "Duplicate key '%s' in batch. Processing first "
            "instance.",
            req_key)
        batch_responses.append((req_row, BeamRow()))
  ...
  def _process_single_request(self,
    ...
    if bq_results_orig_keys:
      ...
      if self._latest_value_callback and renamed_bq_rows:
        ...
        response_row = renamed_bq_rows[0]
      if (len(bq_results_orig_keys) > 1 and
          not (self._latest_value_callback and
               response_row != BeamRow())):  # Log if multiple and
        # default/callback didn't pick one specifically
        logger.warning(
            "Single request -> %s BQ rows. Used selected/first. "
            "Filter:'%s'",
