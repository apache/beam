...
class TestBigQueryStorageEnrichmentIT(BigQueryStorageEnrichmentIT):
    ...
    def create_table(cls, table_id_suffix, schema_fields, data):
        ...
        if data:
            cls.bigquery_client.insert_rows(cls.project, cls.dataset_id, table_id, data)
    ...
    def test_enrichment_single_element(self):
        ...
        expected_output = [beam.Row(id=1, source_field="SourceA", name="A", quantity=2)]
    ...
    def test_enrichment_row_restriction_template_fn(self):
        ...
        expected_output = [
            beam.Row(
                lookup_id=5, lookup_name="C", quantity=100, distribution_center_id=4
            )
        ]
        with TestPipeline(is_integration_test=True) as p:
            enriched = p | beam.Create(requests) | Enrichment(handler)
    ...
    def test_enrichment_condition_value_fn(self):
        ...
        expected_output = [beam.Row(product_identifier=6, id=6, name="D", quantity=11)]
        with TestPipeline(is_integration_test=True) as p:
            enriched = p | beam.Create(requests) | Enrichment(handler)
            assert_that(enriched, equal_to(expected_output))
    ...
    def test_enrichment_additional_condition_fields(self):
        ...
        expected_output = [
            beam.Row(
                target_id=7, filter_on_name="C", quantity=7, distribution_center_id=1
            )
        ]
        with TestPipeline(is_integration_test=True) as p:
    ...
    def test_enrichment_latest_value_selector(self):
        def select_latest_by_ts(
            bq_results: List[beam.Row], request_row: beam.Row
        ) -> Optional[beam.Row]:
            if not bq_results:
                return None
        ...
        expected_output = [
            ...
            beam.Row(
                lookup_id=10, value="latest_value_10", update_ts="2023-01-03T00:00:00Z"
            )
        ]
        with TestPipeline(is_integration_test=True) as p:
            enriched = p | beam.Create(requests) | Enrichment(handler)
    ...
    def test_enrichment_bad_request_invalid_column_in_template(self):
        ...
        with TestPipeline(is_integration_test=True) as p:
            ...
            with self.assertRaises(
                GoogleAPICallError
            ) as e_ctx:  # Or specifically BadRequest
            ...
            self.assertTrue(
                isinstance(e_ctx.exception, BadRequest)
                or "NoSuchFieldError" in str(e_ctx.exception)
                or "not found in table" in str(e_ctx.exception).lower()
                or "unrecognized name" in str(e_ctx.exception).lower()
            )
