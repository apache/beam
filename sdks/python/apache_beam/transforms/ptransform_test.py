...
class PTransformTest(unittest.TestCase):
  ...
  def test_str(self):
    ...
    self.assertEqual('AppliedPTransform(ALabel/Read, Read)', str(res.producer))
    ...
    inputs_tr.inputs = ('ci',)
    self.assertEqual(
        """<Read(PTransform) label=[Read] inputs=('ci',)>""", str(inputs_tr))
    ...
    self.assertEqual(
        '<Read(PTransform) label=[Read] side_inputs=(4,)>', str(side_tr))
  ...
  def test_do_with_do_fn_returning_string_raises_warning(self):
    ...
    expected_error_prefix = (
        'Returning a str from a ParDo or FlatMap ' 'is discouraged.')
  ...
  def test_do_with_do_fn_returning_dict_raises_warning(self):
    ...
    expected_error_prefix = (
        'Returning a dict from a ParDo or FlatMap ' 'is discouraged.')
    self.assertStartswith(cm.exception.args[0], expected_error_prefix)
  ...
  def test_partition_with_callable(self):
    pipeline = TestPipeline()
    pcoll = pipeline | 'Start' >> beam.Create([0, 1, 2, 3, 4, 5, 6, 7, 8])
    partitions = pcoll | 'part' >> beam.Partition(
        lambda e, n, offset: (e % 3) + offset, 4, 1)
    assert_that(partitions[0], equal_to([]))
    assert_that(partitions[1], equal_to([0, 3, 6]), label='p1')
    assert_that(partitions[2], equal_to([1, 4, 7]), label='p2')
    assert_that(partitions[3], equal_to([2, 5, 8]), label='p3')
  ...
  def test_apply_to_list(self):
    ...
    self.assertCountEqual(
        [1], [0, 1, 2] | 'Odd' >> beam.Filter(lambda x: x % 2))
...
class PTransformTypeCheckTestCase(TypeHintTestCase):
  ...
  def test_pardo_does_not_type_check_using_type_hint_methods(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (
          ...
          | (
              'Score'
              >> beam.FlatMap(lambda x: [1] if x == 't' else [2])
              .with_input_types(str)
              .with_output_types(int))
          | (
              'Upper'
              >> beam.FlatMap(lambda x: [x.upper()])
              .with_input_types(str)
              .with_output_types(str))
)
  ...
  def test_group_by_key_only_output_type_deduction(self):
    d = (
        ...
        | (
            'Pair'
            >> beam.Map(lambda x: (x, ord(x))).with_output_types(
                typehints.KV[str, str])
)
        | _GroupByKeyOnly())
    ...
    self.assertCompatible(
        typehints.KV[str, typehints.Iterable[str]], d.element_type)
  ...
  def test_group_by_key_output_type_deduction(self):
    d = (
        ...
        | (
            'PairNegative'
            >> beam.Map(lambda x: (x % 5, -x)).with_output_types(
    ...
    self.assertCompatible(
        typehints.KV[int, typehints.Iterable[int]], d.element_type)
  ...
  def test_group_by_does_not_type_check(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (
          self.p
          | (beam.Create([[1], [2]]).with_output_types(typehints.Iterable[int]))
          | 'T' >> beam.GroupByKey())
  ...
  def test_run_time_type_checking_enabled_types_satisfied(self):
    ...
    result = (
        self.p
        | 'T'
        >> beam.Create(['t', 'e', 's', 't', 'i', 'n', 'g']).with_output_types(
            str)
  ...
  def test_pipeline_runtime_checking_violation_composite_type_input(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (
          ...
          | (
              'Add'
              >> beam.FlatMap(lambda x_y: [x_y[0] + x_y[1]])
              .with_input_types(typehints.Tuple[int, int])
              .with_output_types(int))
)
      self.p.run()
  ...
  def test_pipeline_runtime_checking_violation_composite_type_output(self):
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (
          ...
          | (
              'Swap'
              >> beam.FlatMap(lambda x_y1: [x_y1[0] + x_y1[1]])
  ...
  def test_pipeline_runtime_checking_violation_with_side_inputs_via_method(
    ...
    with self.assertRaises(typehints.TypeCheckError) as e:
      (
          ...
          | (
              'Add 1'
              >> beam.Map(lambda x, one: x + one, 1.0)
              .with_input_types(int, int)
  ...
  def test_combine_pipeline_type_check_using_methods(self):
    d = (
        ...
        | (
            'concat'
            >> beam.CombineGlobally(lambda s: ''.join(s))
            .with_input_types(str)
            .with_output_types(str))
)
  ...
  def test_combine_runtime_type_check_using_methods(self):
    ...
    d = (
        ...
        | (
            'Sum'
            >> beam.CombineGlobally(lambda s: sum(s))
            .with_input_types(int)
            .with_output_types(int))
)
  ...
  def test_combine_pipeline_type_check_violation_using_methods(self):
    with self.assertRaises(typehints.TypeCheckError) as e:
      (
          ...
          | (
              'SortJoin'
              >> beam.CombineGlobally(lambda s: ''.join(sorted(s)))
              .with_input_types(str)
              .with_output_types(str))
)
  ...
  def test_mean_per_key_pipeline_checking_satisfied(self):
    d = (
        ...
        | (
            'EvenGroup'
            >> beam.Map(lambda x: (not x % 2, x)).with_output_types(
                typehints.KV[bool, int])
)
  ...
  def test_mean_per_key_pipeline_checking_violated(self):
    with self.assertRaises(typehints.TypeCheckError) as e:
      (
          ...
          | (
              'UpperPair'
              >> beam.Map(lambda x: (x.upper(), x)).with_output_types(
                  typehints.KV[str, str])
)
          | 'EvenMean' >> combine.Mean.PerKey())
      self.p.run()
  ...
  def test_mean_per_key_runtime_checking_satisfied(self):
    ...
    d = (
        ...
        | (
            'OddGroup'
            >> beam.Map(lambda x: (bool(x % 2), x)).with_output_types(
                typehints.KV[bool, int])
)
  ...
  def test_count_perkey_runtime_type_checking_satisfied(self):
    ...
    d = (
        ...
        | beam.Create(['t', 'e', 's', 't']).with_output_types(str)
        | 'DupKey'
        >> beam.Map(lambda x: (x, x)).with_output_types(typehints.KV[str, str])
  ...
  @unittest.skipIf(
      ...
      and os.environ.get('RUN_SKIPPED_PY3_TESTS') != '1',
      'This test still needs to be fixed on Python 3.7. ' 'See BEAM-6986')
  ...
  def test_per_key_pipeline_checking_satisfied(self):
    d = (
        ...
        | (
            'GroupMod 3'
            >> beam.Map(lambda x: (x % 3, x)).with_output_types(
                typehints.KV[int, int])
)
        | 'TopMod' >> combine.Top.PerKey(1))

    self.assertCompatible(
        typehints.Tuple[int, typehints.Iterable[int]], d.element_type)
  ...
  def test_sample_per_key_pipeline_satisfied(self):
    d = (
        self.p
        | (
            beam.Create([(1, 2), (1, 2), (2, 3), (2, 3)]).with_output_types(
                typehints.KV[int, int])
)
  ...
  def test_to_dict_pipeline_check_satisfied(self):
    d = (
        self.p
        | beam.Create([(1, 2), (3, 4)]).with_output_types(
  ...
  def test_eager_execution_tagged_outputs(self):
    result = [1, 2, 3, 4] | beam.Map(
        lambda x: pvalue.TaggedOutput('bar', 2 * x)
    ).with_outputs('bar')
    self.assertEqual([2, 4, 6, 8], result.bar)
    with self.assertRaises(
        KeyError, msg='Tag \'foo\' is not a defined output tag'):
      result.foo
