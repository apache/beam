...
class TestParquet(unittest.TestCase):
  ...
  def _convert_to_timestamped_record(self, record):
    ...
    timestamped_record['favorite_number'] = pandas.Timestamp(
  ...
  def test_split_points(self):
    ...
    self.assertEquals(
        split_points_report[:10], [(0, RangeTracker.SPLIT_POINTS_UNKNOWN)] * 10
    )
  ...
  def test_read_all_from_parquet_single_file(self):
    ...
    with TestPipeline() as p:
      assert_that(
          p | Create([path]) | ReadAllFromParquet(), equal_to(self.RECORDS))
  ...
  def test_read_all_from_parquet_many_single_files(self):
    ...
    with TestPipeline() as p:
      assert_that(
          p | Create([path1, path2, path3]) | ReadAllFromParquet(),
  ...
  def test_read_all_from_parquet_file_pattern(self):
    ...
    with TestPipeline() as p:
      assert_that(
          p | Create([file_pattern]) | ReadAllFromParquet(),
