...
class _RowDictionariesToArrowTable(DoFn):
  ...
  def _clear_state(self):
    ...
    self._window_and_pane = {}

  def _init_window_state(self, w_key):
    if w_key not in self._buffer:
      self._buffer[w_key] = [[] for _ in range(len(self._schema.names))]
      self._record_batches[w_key] = []
      self._record_batches_byte_size[w_key] = 0

  def process(self, row, w=DoFn.WindowParam, pane=DoFn.PaneInfoParam):
    w_key = (w, pane)
    self._init_window_state(w_key)
    self._window_and_pane[w_key] = (w, pane)

    if len(self._buffer[w_key][0]) >= self._buffer_size:
      self._flush_buffer(w_key)

    if self._record_batches_byte_size[w_key] >= self._row_group_buffer_size:
      table = self._create_table(w_key)
      yield table
    ...
    for i, n in enumerate(self._schema.names):
      ...
      if field.nullable and n not in row:
        self._buffer[w_key][i].append(None)
      else:
        self._buffer[w_key][i].append(row[n])

  def finish_bundle(self):
    for w_key in list(self._buffer.keys()):
      if len(self._buffer[w_key][0]) > 0:
        self._flush_buffer(w_key)
      if self._record_batches_byte_size[w_key] > 0:
        table = self._create_table(w_key)
        w, pane = self._window_and_pane[w_key]
  ...
  def _create_table(self, w_key):
    table = pa.Table.from_batches(
        self._record_batches[w_key], schema=self._schema)
    self._record_batches[w_key] = []
    self._record_batches_byte_size[w_key] = 0
    return table

  def _flush_buffer(self, w_key):
    arrays = [[] for _ in range(len(self._schema.names))]
    for x, y in enumerate(self._buffer[w_key]):
      arrays[x] = pa.array(y, type=self._schema.types[x])
      self._buffer[w_key][x] = []
    rb = pa.RecordBatch.from_arrays(arrays, schema=self._schema)
    self._record_batches[w_key].append(rb)
    size = 0
    for x in arrays:
      for b in x.buffers():
        if b is not None:
          size = size + b.size
    self._record_batches_byte_size[w_key] = self._record_batches_byte_size[w_key] + size
