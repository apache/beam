...
class _TextSource(filebasedsource.FileBasedSource):
    ...
    class ReadBuffer(object):
        ...
        def position(self, value: int) -> None:
            ...
            if value > len(self._data):
                raise ValueError(
                    f"Cannot set position to {value} since it's larger than size of data {len(self._data)}."
                )
    ...
    def __init__(
        ...
        header_processor_fns: Tuple[Optional[Callable[..., Any]], Optional[Callable[..., Any]]] = (None, None),
        delimiter: Optional[bytes] = None,
        escapechar: Optional[bytes] = None,
    ) -> None:
    ...
    def read_records(self, file_name: str, range_tracker: iobase.RangeTracker) -> Iterator[Any]:
        ...
        def split_points_unclaimed(stop_position):
            return (
                0 if stop_position <= next_record_start_position else iobase.RangeTracker.SPLIT_POINTS_UNKNOWN
            )
    ...
    def _process_header(self, file_to_read: BinaryIO, read_buffer: " _TextSource.ReadBuffer") -> int:
    ...
    def _find_separator_bounds(
        self, file_to_read: BinaryIO, read_buffer: " _TextSource.ReadBuffer"
        ...
        while True:
            ...
            if next_delim >= 0:
                if (self._delimiter is None and read_buffer.data[next_delim - 1:next_delim] == b'\r'):
                    ...
                    if self._escapechar is not None and self._is_escaped(read_buffer, next_delim):
    ...
    def _try_to_ensure_num_bytes_in_buffer(
        self, file_to_read: BinaryIO, read_buffer: " _TextSource.ReadBuffer", num_bytes: int
    ) -> bool:
    ...
    def _skip_lines(self, file_to_read: BinaryIO, read_buffer: " _TextSource.ReadBuffer", num_lines: int) -> int:
    ...
    def _read_record(self, file_to_read: BinaryIO, read_buffer: " _TextSource.ReadBuffer") -> Tuple[bytes, int]:
    ...
    @staticmethod
    def _is_self_overlapping(delimiter: bytes) -> bool:
    ...
    def _is_escaped(self, read_buffer: " _TextSource.ReadBuffer", position: int) -> bool:
...
class _TextSourceWithFilename(_TextSource):
    def read_records(self, file_name: str, range_tracker: iobase.RangeTracker) -> Iterator[Tuple[str, Any]]:
        ...
        *,
        ...
        Args:
          ...
          shard_name_template: A template string containing placeholders for
            the shard number and shard count. Currently only ``''``, ``'-SSSSS-of-NNNNN'``,
            ``'-W-SSSSS-of-NNNNN'`` and ``'-V-SSSSS-of-NNNNN'`` are patterns accepted by the service.
...
try:
    ...
    def ReadFromCsv(
        ...
        **kwargs,
        ...
        return 'ReadFromCsv' >> ReadViaPandas('csv', path, splittable=splittable, filename_column=filename_column, **kwargs)
    ...
    def ReadFromJson(path: str, *, orient: str = 'records', lines: bool = True, dtype: Union[bool, Dict[str, Any]] = False, **kwargs):
        ...
        from apache_beam.dataframe.io import ReadViaPandas
        return 'ReadFromJson' >> ReadViaPandas('json', path, orient=orient, lines=lines, dtype=dtype, **kwargs)
