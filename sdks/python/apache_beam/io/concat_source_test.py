...
class RangeSource(iobase.BoundedSource):
  ...
  def read(self, range_tracker):
    for k in range(
        self._round_up(range_tracker.start_position()),
...
class ConcatSourceTest(unittest.TestCase):
  ...
  def test_estimate_size(self):
    source = ConcatSource(
        [RangeSource(0, 10), RangeSource(10, 100), RangeSource(100, 1000)])
  ...
  def test_position_at_fration(self):
    ...
    source = ConcatSource(
        [
            iobase.SourceBundle(
                (range[1] - range[0]) / 32.0, RangeSource(*range), None, None)
            for range in ranges
        ])

    range_tracker = source.get_range_tracker()
    self.assertEquals(range_tracker.position_at_fraction(0), (0, 0))
    self.assertEquals(range_tracker.position_at_fraction(0.01), (0, 1))
    self.assertEquals(range_tracker.position_at_fraction(0.1), (0, 4))
    self.assertEquals(range_tracker.position_at_fraction(0.125), (1, 4))
    self.assertEquals(range_tracker.position_at_fraction(0.2), (1, 7))
    self.assertEquals(range_tracker.position_at_fraction(0.7), (2, 23))
    self.assertEquals(range_tracker.position_at_fraction(0.75), (3, 24))
    self.assertEquals(range_tracker.position_at_fraction(0.8), (3, 26))
    ...
    self.assertEquals(range_tracker.position_at_fraction(0.01), (1, 5))
    self.assertEquals(range_tracker.position_at_fraction(0.5), (1, 14))
    self.assertEquals(range_tracker.position_at_fraction(0.599), (1, 16))
    self.assertEquals(range_tracker.position_at_fraction(0.601), (2, 17))
  ...
  def test_conact_source_exhaustive(self):
    source = ConcatSource(
        [RangeSource(0, 10), RangeSource(100, 110), RangeSource(1000, 1010)])
    source_test_utils.assert_split_at_fraction_exhaustive(source)
