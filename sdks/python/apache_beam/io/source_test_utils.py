...
class SourceTestUtils(object):
  ...
  correctness of source (``iobase.BoundedSource``) and range tracker
  ...
  * If your source implements dynamic work rebalancing, use the
  ...
  def assertSourcesEqualReferenceSource(reference_source_info, sources_info):
    ...
    reference_records = SourceTestUtils.readFromSource(
        *reference_source_info)
    ...
    for source_info in sources_info:
      ...
      if (type(reference_source_info[0].default_output_coder()) !=
        ...
        raise ValueError(
            ...
            type(source_info[0].default_output_coder()))
      source_records.extend(SourceTestUtils.readFromSource(*source_info))
  ...
  def assertSplitAtFractionBehavior(source, num_items_to_read_before_split,
    ...
    expected_items = SourceTestUtils.readFromSource(source, None, None)
    return SourceTestUtils._assertSplitAtFractionBehavior(
  ...
  def _assertSplitAtFractionBehavior(
    ...
    return SourceTestUtils._verifySingleSplitFractionResult(
  ...
  def _verifySingleSplitFractionResult(
    ...
    primary_items = SourceTestUtils.readFromSource(source, *primary_range)
    ...
    residual_items = (
        SourceTestUtils.readFromSource(source, *residual_range)
    ...
    if current_items != primary_items:
      raise ValueError('Current source %r and a source created using the '
                       ...
                       source,
                       SourceTestUtils._range_to_str(*primary_range),
                       split_fraction
                      )
    ...
    if expected_items != total_items:
      raise ValueError('Items obtained by reading the source %r for primary '
                       ...
                       source,
                       SourceTestUtils._range_to_str(*primary_range),
                       SourceTestUtils._range_to_str(*residual_range))
  ...
  def assertSplitAtFractionBinary(source, expected_items,
    ...
    if left_result is None:
      left_result = SourceTestUtils._assertSplitAtFractionBehavior(
          source, expected_items, num_items_to_read_before_split, left_fraction,
          ExpectedSplitOutcome.MUST_BE_CONSISTENT_IF_SUCCEEDS)

    if right_result is None:
      right_result = SourceTestUtils._assertSplitAtFractionBehavior(
          source, expected_items, num_items_to_read_before_split,
          right_fraction, ExpectedSplitOutcome.MUST_BE_CONSISTENT_IF_SUCCEEDS)

    middle_result = SourceTestUtils._assertSplitAtFractionBehavior(
  ...
  def assertSplitAtFractionExhaustive(source, perform_multi_threaded_test=True):
    ...
    for i in range(len(expected_items)):
      ...
      while True:
        num_trials += 1
        if (num_trials >
            SourceTestUtils.MAX_CONCURRENT_SPLITTING_TRIALS_PER_ITEM):
          logging.warn(
        ...
        if SourceTestUtils._assertSplitAtFractionConcurrent(
      ...
      if (num_total_trials >
          SourceTestUtils.MAX_CONCURRENT_SPLITTING_TRIALS_TOTAL):
  ...
  def _assertSplitAtFractionConcurrent(
    ...
    res = SourceTestUtils._verifySingleSplitFractionResult(
