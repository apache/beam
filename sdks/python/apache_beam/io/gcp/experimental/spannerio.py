...
class _NaiveSpannerReadDoFn(DoFn):
    ...
    def process(self, element, spanner_transaction):
        ...
        self._snapshot = BatchSnapshot.from_dict(
            self._database, transaction_info)
...
class _CreateTransactionFn(DoFn):
    ...
    def setup(self):
        ...
        self._database = self._instance.database(
            self._database_id, pool=self._pool)
...
class WriteMutation(object):
    ...
    def insert(table, columns, values):
        ...
        return _Mutator(
            mutation=Mutation(insert=batch._make_write_pb(
    ...
    def update(table, columns, values):
        ...
        cells = len(columns) * len(values)
        return _Mutator(
            mutation=Mutation(update=batch._make_write_pb(
                table, columns, values)),
    ...
    def replace(table, columns, values):
        ...
        cells = len(columns) * len(values)
        return _Mutator(
            mutation=Mutation(replace=batch._make_write_pb(
                table, columns, values)),
            ...
            with self._db_instance.batch() as b:
                for m in element:
                    ...
                    if m.operation == WriteMutation._OPERATION_DELETE:
                        ...
                        raise ValueError(
                            "Unknown operation action: %s" % m.operation)
                    batch_func(**m.kwargs)
