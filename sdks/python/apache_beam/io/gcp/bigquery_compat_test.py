...
class TestJobConfigCompatibility(unittest.TestCase):
  ...
  def test_insert_load_job_with_none_labels(self):
    ...
    jobs_compat = bigquery_compat._ClientJobsCompat(client)
    request = mock.MagicMock()
    request.projectId = "test-project"
    request.job = mock.MagicMock()
    request.job.jobReference = bigquery_compat.JobReference(jobId="load_job_id")
    request.job.configuration = mock.MagicMock(spec=["load", "labels"])
    request.job.configuration.load.destinationTable = (
        bigquery_compat.TableReference(
            projectId="test-project",
            datasetId="test_dataset",
            tableId="test_table"))
    request.job.configuration.load.sourceUris = ["gs://test-bucket/test.csv"]
    request.job.configuration.load.schema = None
    request.job.configuration.load.writeDisposition = None
    request.job.configuration.load.createDisposition = None
    request.job.configuration.labels = None

    jobs_compat.Insert(request)
  ...
  def test_insert_copy_job_with_none_labels(self):
    ...
    request.projectId = "test-project"
    request.job = mock.MagicMock()
    request.job.jobReference = bigquery_compat.JobReference(jobId="copy_job_id")
    request.job.configuration = mock.MagicMock(spec=["copy", "labels"])
    request.job.configuration.copy.sourceTables = [
        bigquery_compat.TableReference(
            projectId="test-project",
            datasetId="test_dataset",
            tableId="src_table")
    ]
    request.job.configuration.copy.destinationTable = (
        bigquery_compat.TableReference(
            projectId="test-project",
            datasetId="test_dataset",
            tableId="dst_table"))
    request.job.configuration.copy.writeDisposition = None
    request.job.configuration.copy.createDisposition = None
    request.job.configuration.labels = None
  ...
  def test_perform_extract_job_with_none_labels(self):
    ...
    request.projectId = "test-project"
    request.job = mock.MagicMock()
    request.job.jobReference = bigquery_compat.JobReference(
        jobId="extract_job_id")
    request.job.configuration = mock.MagicMock(spec=["extract", "labels"])
    request.job.configuration.extract.sourceTable = (
        bigquery_compat.TableReference(
            projectId="test-project",
            datasetId="test_dataset",
            tableId="src_table"))
    request.job.configuration.extract.destinationUris = [
        "gs://test-bucket/output.csv"
    ]
    request.job.configuration.extract.destinationFormat = "CSV"
    request.job.configuration.labels = None
