...
def _table_field_to_dict(field):
  ...
  if hasattr(field, 'fields') and field.fields:
    result['fields'] = [_table_field_to_dict(f) for f in field.fields]
  return result
  ...
  if isinstance(schema, (list, tuple)):
    ...
    for f in schema:
      if gcp_bigquery is not None and isinstance(
        ...
        dict_field = _table_field_to_dict(f)
...
class _ClientJobsCompat:
  ...
  def Insert(self, request, upload=None):
    ...
    dict_labels = _extract_dict_labels(
        getattr(config, "labels", None)) if config else None
