...
from typing import Optional, Sequence, Tuple, TypeVar, Union, Dict, List, TYPE_CHECKING
...
def to_hashable_table_ref(
    table_ref_elem_kv: Tuple[Union[str, TableReference], V],
) -> Tuple[str, V]:
...
def parse_table_reference(table, dataset=None, project=None):
    ...
    if isinstance(table, TableReference):
        return TableReference(projectId=table.projectId,
...
# Module-level table definition cache constants and variables
_TABLE_DEFINITION_TTL = 1  # 1 second (default)
_TABLE_CACHE_MAX_SIZE = 1000  # Default maximum cache size
_TABLE_CACHE = cachetools.TTLCache(maxsize=_TABLE_CACHE_MAX_SIZE,
                                   ttl=_TABLE_DEFINITION_TTL)
_CACHE_ENABLED = True  # Flag to track if caching is enabled (TTL > 0)
...
class BigQueryWrapper(object):
    ...
    DEFAULT_TABLE_DEFINITION_TTL = 1  # 1 second

    # Default maximum cache size (number of entries)
    DEFAULT_TABLE_CACHE_MAX_SIZE = 1000
    ...
    def __init__(self, client=None, temp_dataset_id=None, temp_table_ref=None):
        ...
        self.gcp_bq_client = client or gcp_bigquery.Client(
            client_info=ClientInfo(user_agent="apache-beam-%s" %
        ...
        # Handle temp dataset and table references
        ...
        # We no longer initialize instance-level cache, using module-level cache instead
    ...
    def set_table_definition_ttl(self, ttl_seconds):
        ...
        """
        global _TABLE_DEFINITION_TTL, _TABLE_CACHE, _CACHE_ENABLED

        # Validate that TTL is not negative
        if ttl_seconds < 0:
            raise ValueError(f"TTL must be non-negative: {ttl_seconds}")

        # If caching was disabled (TTL=0) and is now being enabled, recreate the cache
        if not _CACHE_ENABLED and ttl_seconds > 0:
            _TABLE_CACHE = cachetools.TTLCache(maxsize=_TABLE_CACHE_MAX_SIZE,
                                               ttl=ttl_seconds)
            _CACHE_ENABLED = True
        # If caching was enabled and now has a different TTL, recreate the cache with new TTL
        elif (_CACHE_ENABLED and ttl_seconds > 0 and
              _TABLE_DEFINITION_TTL != ttl_seconds):
            _TABLE_CACHE = cachetools.TTLCache(maxsize=_TABLE_CACHE_MAX_SIZE,
                                               ttl=ttl_seconds)
        # If caching is being disabled, replace the cache with an empty one
        elif ttl_seconds == 0:
            _TABLE_CACHE = cachetools.TTLCache(
                maxsize=_TABLE_CACHE_MAX_SIZE,
                ttl=sys.maxsize)  # Use a very large TTL that will never expire
            _TABLE_CACHE.clear()  # Clear the cache to make sure it's empty
            _CACHE_ENABLED = False

        _TABLE_DEFINITION_TTL = ttl_seconds

    def set_table_cache_max_size(self, max_size):
        """Sets the maximum size of the table definition cache.
        ...
        """
        if max_size <= 0:
            raise ValueError(f"Cache size must be positive: {max_size}")

        global _TABLE_CACHE_MAX_SIZE, _TABLE_CACHE

        # Create a new cache with the new size but preserve existing entries
        # This is done by creating a new cache and updating it with the old entries
        new_cache = cachetools.TTLCache(
            maxsize=max_size,
            ttl=_TABLE_DEFINITION_TTL if _CACHE_ENABLED else sys.maxsize,
        )

        # Only copy entries if caching is enabled
        if _CACHE_ENABLED:
            # Copy entries from the old cache to the new one
            # LRU policy will automatically apply if we exceed the new max_size
            for key, value in list(_TABLE_CACHE.items()):
                new_cache[key] = value

        _TABLE_CACHE = new_cache
        _TABLE_CACHE_MAX_SIZE = max_size

    def clear_table_cache(self,
                          project_id=None,
                          dataset_id=None,
                          table_id=None):
        """Clears the table definition cache.

        This method allows selective clearing of the table definition cache based on
        project, dataset, and table specifications. If no parameters are provided,
        the entire cache is cleared.

        Args:
          project_id: The project containing tables to remove.
                     If None, the entire cache is cleared if dataset_id and table_id are also None.
          dataset_id: The dataset containing tables to remove.
                     Must also specify project_id.
          table_id: The table to remove. Must also specify dataset_id.

        Raises:
          ValueError: If dataset_id is specified without project_id,
                     or if table_id is specified without dataset_id,
                     or if table_id is specified with project_id but without dataset_id.
        """
        if dataset_id is not None and project_id is None:
            raise ValueError(
                "project_id must be specified if dataset_id is specified")
        if table_id is not None and dataset_id is None:
            raise ValueError(
                "dataset_id must be specified if table_id is specified")
        if table_id is not None and project_id is not None and dataset_id is None:
            raise ValueError(
                "dataset_id must be specified if table_id and project_id are specified"
            )

        # Using global keyword to indicate we're modifying the global cache
        global _TABLE_CACHE
        ...
        to_delete = []
        for key in _TABLE_CACHE.keys():
            # Cache keys are formatted as: "project_id:dataset_id.table_id"
            parts = key.split(":")
            if len(parts) != 2:
                continue

            p = parts[0]
            dt_parts = parts[1].split(".")
            if len(dt_parts) != 2:
                continue

            d, t = dt_parts

            if project_id is not None and p != project_id:
                continue
            if dataset_id is not None and d != dataset_id:
                continue
            if table_id is not None and t != table_id:
                continue
    ...
    def unique_row_id(self):
        """Returns a unique row ID used to avoid multiple insertions.

        If this row ID is provided, BigQuery will make a best effort to not insert
        the same row multiple times during fail and retry scenarios where the insert
        request may be issued several times. This is especially important for sinks
        executed in a local runner.

        Returns:
          str: A unique row ID string that can be used with BigQuery insert operations.
        """
        self._unique_row_id += 1
        return f"{self._row_id_prefix}_{self._unique_row_id}"
    ...
    def get_query_location(self, project_id, query, use_legacy_sql):
        ...
        if referenced_tables:  # Guards against both non-empty and non-None
            for table in referenced_tables:
                try:
                    location = self.get_table_location(table.projectId,
                                                       table.datasetId,
                                                       table.tableId)
    ...
    def get_table(self, project_id, dataset_id, table_id):
        ...
        This method retrieves a table's metadata from BigQuery, using cache if
        available and enabled. The cache behavior is controlled by set_table_definition_ttl()
        and set_table_cache_max_size().

        Args:
          project_id (str): The GCP project ID containing the table.
          dataset_id (str): The BigQuery dataset ID containing the table.
          table_id (str): The ID of the specific table to retrieve.

        Returns:
          bigquery.Table: The table metadata object from the BigQuery API.

        Raises:
          HttpError: If the table lookup fails (e.g., table doesn't exist).
        """
        # Using global notation for _TABLE_CACHE since we modify it
        global _TABLE_CACHE

        # Skip caching if module-level caching is disabled
        if not _CACHE_ENABLED:
            return self._fetch_table(project_id, dataset_id, table_id)

        cache_key = f"{project_id}:{dataset_id}.{table_id}"

        # Try to get the table from cache
        table = _TABLE_CACHE.get(cache_key)
        if table:
            return table

        # Table not in cache, fetch it and add to cache
    ...
    def _fetch_table(self, project_id, dataset_id, table_id):
        ...
        Raises:
          HttpError: If the table lookup fails.
        """
    ...
    def _start_job(
        ...
        try:
            ...
            if exn.status_code == 409:
                ...
                _LOGGER.info("Failed to insert job %s: %s",
                             request.job.jobReference, exn)
                raise
    ...
    def _create_table(self,
        ...
        table = bigquery.Table(
            tableReference=TableReference(projectId=project_id,
                                          datasetId=dataset_id,
                                          tableId=table_id),
            schema=schema,
    ...
    def is_user_configured_dataset(self):
        return self.temp_dataset_id and not self.temp_dataset_id.startswith(
            self.TEMP_DATASET)
    ...
    def create_temporary_dataset(self, project_id, location, labels=None):
        self.get_or_create_dataset(project_id,
    ...
    def clean_up_temporary_dataset(self, project_id):
        ...
        try:
            ...
            if not self.is_user_configured_dataset():
                self._delete_dataset(temp_table.projectId, temp_table.datasetId,
                                     True)
            else:
                self._delete_table(temp_table.projectId, temp_table.datasetId,
                                   temp_table.tableId)
            self.created_temp_dataset = False
        except HttpError as exn:
    ...
    def _clean_up_beam_labelled_temporary_datasets(self,
                                                   project_id,
                                                   dataset_id=None,
                                                   table_id=None,
                                                   labels=None):
        ...
        if not self.is_user_configured_dataset() and labels is not None:
            response = self.client.datasets.List(
                bigquery.BigqueryDatasetsListRequest(projectId=project_id,
                                                     filter=filter_str))
    ...
    def get_or_create_table(
        ...
        Raises:
          `RuntimeError`: For various mismatches between the state of the table and
            the create/write dispositions passed in. For example if the table is no
        ...
        if found_table and write_disposition != BigQueryDisposition.WRITE_TRUNCATE:
            ...
            try:
                ...
                if exn.status_code == 409:
                    _LOGGER.debug(
                        ...
                        (project_id, dataset_id, table_id))
                    created_table = self.get_table(project_id, dataset_id,
                                                   table_id)
                else:
                    raise
            _LOGGER.info(
            ...
            if write_disposition == BigQueryDisposition.WRITE_TRUNCATE:
                # BigQuery can route data to the old table for 2 mins max so wai
                # that much time before creating the table and writing i
                _LOGGER.warning(
    ...
    def run_query(
        ...
        while True:
            ...
            if not response.jobComplete:
                # The jobComplete field can be False if the query request times ou
    ...
    def insert_rows(
        ...
        Args:
          ...
          ignore_unknown_values: Set this option to true to ignore unknown column
            names. If the input rows contain columns that are no
            part of the existing table's schema, those columns are ignored, and
            the rows are successfully inserted.

        Returns:
          A tuple (bool, errors). If first element is False then the second elemen
    ...
    def _convert_cell_value_to_dict(self, value, field):
        """Converts an API value to the proper dictionary value.

        Args:
          value: The value to be converted.
          field: The field descriptor for the value.
        ...
        if field.type == "STRING" or field.type == "BYTES":
            ...
            result = self.convert_row_to_dict(value, field.fields)
        return result
...
class RowAsDictJsonCoder(coders.Coder):
    ...
    This is the default coder for sources and sinks if the coder argument is no
    specified.
    """

    def encode(self, table_row):
        # The normal error when dumping NAN/INF values is:
        # ValueError: Out of range float values are not JSON complian
        ...
        try:
            ...
            raise ValueError("%s. %s. Row: %r" %
                             (e, JSON_COMPLIANCE_ERROR, table_row))
...
class AvroRowWriter(io.IOBase):
    ...
    def __init__(self, file_handle, schema):
        ...
        self._avro_writer = fastavro.write.Writer(self._file_handle,
                                                  avro_schema)
    ...
    def tell(self):
        # Flush the fastavro Writer to the underlying stream, otherwise there isn'
...
def beam_row_from_dict(row: dict, schema):
    ...
    for field in schema.fields:
        ...
        # Beam Row PCollection schema requires all fields to be presen
        ...
        if type in ["RECORD", "STRUCT"] and value:
            ...
            if mode == "REPEATED":
                # Handle repeated records by converting each record in the lis
                list_of_beam_rows = []
                for record in value:
...
def get_beam_typehints_from_tableschema(schema):
    ...
    if not isinstance(schema,
                      (bigquery.TableSchema, bigquery.TableFieldSchema)):
        schema = get_bq_tableschema(schema)
    typehints: List[Tuple[str, Any]] = []
    for field in schema.fields:
        name, field_type, mode = field.name, field.type.upper(
        ), field.mode.upper()
        ...
        if field_type in ["STRUCT", "RECORD"]:
            ...
            raise ValueError(f"Converting BigQuery type [{field_type}] to "
                             "Python Beam type is not supported.")

        if mode == "REPEATED":
            typehint = Sequence[typehint]  # type: ignore
        elif mode != "REQUIRED":
            typehint = Optional[typehint]  # type: ignore
...
def get_bq_tableschema(schema):
    """Convert the table schema to a TableSchema object.
      ...
      Args:
        schema (str, dict, ~apache_beam.io.gcp.internal.clients.bigquery.
    bigquery_v2_messages.TableSchema): The schema to be used if the BigQuery table to write has to be created.
          This can either be a dict or string or in the TableSchema format.
