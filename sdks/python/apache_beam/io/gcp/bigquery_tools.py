...
class BigQueryWrapper(object):
  ...
  def _create_table(
      self, project_id, dataset_id, table_id, schema, additional_parameters=None
  ):
  ...
  def _delete_dataset(self, project_id, dataset_id, delete_contents=True):
    ...
    try:
      ...
      if exn.status_code == 404:
        logging.warning('Dataset %s:%s does not exist', project_id, dataset_id)
        return
      else:
        raise
  ...
  def get_or_create_table(
    ...
    if found_table and write_disposition != BigQueryDisposition.WRITE_TRUNCATE:
      ...
      logging.info(
          'Created table %s.%s.%s with schema %s. ' 'Result: %s.',
          project_id,
          dataset_id,
          table_id,
  ...
  def run_query(
    ...
    job_id, location = self._start_query_job(
        project_id,
        query,
        use_legacy_sql,
        flatten_results,
...
class BigQueryReader(dataflow_io.NativeSourceReader):
  ...
  def __init__(
    ...
    if auth.is_running_in_gce:
      ...
      self.executing_project = source.pipeline_options.view_as(
          GoogleCloudOptions
      ).project
    else:
...
class BigQueryWriter(dataflow_io.NativeSinkWriter):
  ...
  def __init__(self, sink, test_bigquery_client=None, buffer_size=None):
    ...
    if self.project_id is None and hasattr(sink, 'pipeline_options'):
      self.project_id = sink.pipeline_options.view_as(
          GoogleCloudOptions
      ).project
...
class RetryStrategy(object):
  ...
  def should_retry(strategy, error_message):
    if strategy == RetryStrategy.RETRY_ALWAYS:
      ...
      return False
    elif (strategy == RetryStrategy.RETRY_ON_TRANSIENT_ERROR
        and error_message not in RetryStrategy._NON_TRANSIENT_ERRORS):
      return True
    else:
      return False
