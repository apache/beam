...
_TABLE_CACHE = cachetools.TTLCache(
    maxsize=_TABLE_CACHE_MAX_SIZE, ttl=_TABLE_DEFINITION_TTL)
...
class BigQueryWrapper(object):
  ...
  def set_table_definition_ttl(self, ttl_seconds):
    ...
    if not _CACHE_ENABLED and ttl_seconds > 0:
      _TABLE_CACHE = cachetools.TTLCache(
          maxsize=_TABLE_CACHE_MAX_SIZE, ttl=ttl_seconds)
      _CACHE_ENABLED = True
  ...
  def clear_table_cache(self, project_id=None, dataset_id=None, table_id=None):
    ...
    if table_id is not None and dataset_id is None:
      raise ValueError("dataset_id must be specified if table_id is specified")
        ...
        job=bigquery.Job(
            configuration=bigquery.JobConfiguration(
                ...
                query=bigquery.JobConfigurationQuery(
                    ...
                    destinationTable=(
                        self._get_temp_table(project_id)
