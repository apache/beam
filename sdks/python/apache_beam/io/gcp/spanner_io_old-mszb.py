...
class NewReadFromSpanner(object):
  ...
  def create_transaction(project_id, instance_id, database_id, timestamp=None):
    ...
    return database, snapshot_options
...
class _TransactionReadDoFn(beam.DoFn):

  def process(self, element, mapping):
    self._transaction = mapping[0].snapshot(**mapping[1])

    with self._transaction as transaction:
      for row in getattr(transaction, element.transaction_action)(**element.kwargs):
        yield row
...
class _TransactionRead(PTransform):
  ...
  def expand(self, pbegin):
    return (pbegin
            ...
            _TransactionReadDoFn(), self._transaction)
            )
...
#     self._read_operations = read_operations
#
#     self._transaction = transaction[0].snapshot(**transaction[1])
#
#   def expand(self, pbegin):
