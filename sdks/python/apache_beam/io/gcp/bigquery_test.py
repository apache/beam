...
def test_with_batched_input_splits_large_batch(self):
  ...
  # Each row with 'data' * 10 is about 200 bytes, so 2 rows should fit, 3rd should cause flush
  rows = [
      ({
          'data': 'x' * 10
      }, 'insertid1'),
  ...
  # With 800 byte limit and ~341 byte rows, we should be able to fit 2 rows per batch
