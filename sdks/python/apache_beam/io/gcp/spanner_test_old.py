...
class DazdazTest(unittest.TestCase):
  ...
  def test_read_actual_transaction(self):
    pipeline = TestPipeline()
    ...
    records = pipeline | NewReadFromSpanner(
        TEST_PROJECT_ID, self.TEST_INSTANCE_ID, self.DB_NAME) \
      .with_transaction(transaction) \
      .with_query('SELECT * FROM users where Key < 10') | beam.Map(pp)
    pipeline.run()
  ...
  def test_read_all_actual(self):
    ...
    records = pipeline \
        ...
        TEST_PROJECT_ID,
        self.TEST_INSTANCE_ID,
        self.DB_NAME).read_all(reads) | beam.Map(pp)
    pipeline.run()
  ...
  def test_read_all_actual_transaction(self):
    ...
    reads = [
        ReadOperation.with_query('SELECT * FROM users where key < 2'),
        ReadOperation.with_table("users", ['name'])
    ]
...
class ReadFromSpannerTest(unittest.TestCase):
  ...
  def test_read_actual(self):
    ...
    records = pipeline \
        ...
        TEST_PROJECT_ID,
        TEST_INSTANCE_ID,
        DB_NAME).with_query('SELECT * FROM users where Key < 10') | beam.Map(pp)
    pipeline.run()
