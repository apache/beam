...
class Pipeline(object):
    ...
    class TransformUpdater(PipelineVisitor):  # pylint: disable=used-before-assignment
      ...
      def _replace_if_needed(self, original_transform_node):
        ...
        if override.matches(original_transform_node):
          ...
          # TODO(https://github.com/apache/beam/issues/21178): Merge rather than override.
  ...
  def _infer_result_type(
    ...
    if (isinstance(result_pcollection, pvalue.PCollection) and
      ...
      # TODO(https://github.com/apache/beam/issues/18957): Add support for tagged type hints.
...
class AppliedPTransform(object):
  ...
  def to_runner_api(self, context):
    ...
    from apache_beam.transforms import external
    if isinstance(self.transform, external.ExternalTransform):
      # TODO(https://github.com/apache/beam/issues/18371): Support resource hints in XLang transforms.
  ...
  def from_runner_api(
    ...
    # TODO(https://github.com/apache/beam/issues/20136): use key, value pairs instead of depending on tags with
    # index as a suffix.
    indexed_side_inputs = [
