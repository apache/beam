...
class ParDoTest(unittest.TestCase):
  ...
  def test_pardo_with_tagged_outputs(self):
    ...
    results = words | beam.ParDo(
        ProcessWords(), cutoff_length=2, marker='x'
    ).with_outputs(
        'above_cutoff_lengths', 'marked strings', main='below_cutoff_strings')
...
class SnippetsTest(unittest.TestCase):
  ...
  def test_model_pcollection(self):
    ...
    self.assertEqual(
        self.get_output(temp_path, sorted_output=False),
        [
  ...
  def test_examples_wordcount_streaming(self, *unused_mocks):
    ...
    beam.io.ReadFromPubSub = FakeReadFromPubSub(
        topic=input_topic, values=input_values)
  ...
  def test_model_use_and_query_metrics(self):
    ...
    custom_distribution = result.metrics().query(
        MetricsFilter().with_name('word_len_dist')
    )['distributions']
    custom_counter = result.metrics().query(
        MetricsFilter().with_name('unmatched_words')
    )['counters']
  ...
  def test_model_setting_trigger(self):
    ...
    with TestPipeline(options=pipeline_options) as p:
      ...
      pcollection = (
          p | test_stream | 'pair_with_one' >> beam.Map(lambda x: (x, 1)))
...
class CombineTest(unittest.TestCase):
  ...
  def test_setting_fixed_windows(self):
    with TestPipeline() as p:
      ...
      items = unkeyed_items | 'key' >> beam.Map(
          lambda x: beam.window.TimestampedValue(('k', x), x))
      # [START setting_fixed_windows]
      from apache_beam import window

      fixed_windowed_items = items | 'window' >> beam.WindowInto(
  ...
  def test_setting_sliding_windows(self):
    with TestPipeline() as p:
      ...
      sliding_windowed_items = items | 'window' >> beam.WindowInto(
  ...
  def test_setting_session_windows(self):
    with TestPipeline() as p:
      ...
      session_windowed_items = items | 'window' >> beam.WindowInto(
  ...
  def test_setting_timestamp(self):
    with TestPipeline() as p:
      ...
      fixed_windowed_items = timestamped_items | 'window' >> beam.WindowInto(
