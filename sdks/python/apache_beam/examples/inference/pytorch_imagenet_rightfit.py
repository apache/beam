...
def decode_and_preprocess(image_bytes: bytes, size: int = 224) -> torch.Tensor:
  ...
  with PILImage.open(io.BytesIO(image_bytes)) as img:
    ...
    left = (w - size) // 2
    top = (h - size) // 2
    img = img.crop((max(0, left), max(0, top), min(w, left + size), min(h, top + size)))
...
class MakeKeyDoFn(beam.DoFn):
  ...
  def process(self, element: str | bytes):
    ...
    if self.input_mode == "bytes":
      ...
      import hashlib
      uri = element.decode("utf-8") if isinstance(element, (bytes, bytearray)) else str(element)
...
class DecodePreprocessDoFn(beam.DoFn):
  ...
  def __init__(self, input_mode: str, image_size: int = 224, decode_threads: int = 4):
    self.input_mode = input_mode
...
class PostProcessDoFn(beam.DoFn):
  ...
  def process(self, kv: Tuple[str, PredictionResult]):
    image_id, pred = kv
    logits = pred.inference["logits"]  # torch.Tensor [B, num_classes] or [num_classes]
...
def parse_known_args(argv):
  ...
  # I/O & runtime
  parser.add_argument('--project', default='apache-beam-testing')
  parser.add_argument('--mode', default='streaming', choices=['streaming', 'batch'])
  parser.add_argument('--output_table', required=True, help='BigQuery output table: dataset.table')
  parser.add_argument('--publish_to_big_query', default='true', choices=['true', 'false'])
  parser.add_argument('--input_mode', default='gcs_uris', choices=['gcs_uris', 'bytes'])
  parser.add_argument('--input', required=True, help='GCS path to file with URIs (for load) OR unused for bytes')
  parser.add_argument('--pubsub_topic', default='projects/apache-beam-testing/topics/images_topic')
  parser.add_argument('--pubsub_subscription', default='projects/apache-beam-testing/subscriptions/images_subscription')
  parser.add_argument('--rate_limit', type=float, default=None, help='Elements per second for load pipeline')

  # Model & inference
  parser.add_argument('--pretrained_model_name', default='efficientnet_b0',
                      help='OSS model name (e.g., efficientnet_b0|mobilenetv3_large_100)')
  parser.add_argument('--model_state_dict_path', default=None, help='Optional state_dict to load')
  parser.add_argument('--device', default='GPU', choices=['CPU', 'GPU'])
  parser.add_argument('--image_size', type=int, default=224)
  parser.add_argument('--top_k', type=int, default=5)
  parser.add_argument('--inference_batch_size', default='auto',
...
def run_load_pipeline(known_args, pipeline_args):
  ...
  if known_args.rate_limit:
    lines = lines | 'RateLimit' >> beam.ParDo(RateLimitDoFn(rate_per_sec=known_args.rate_limit))
...
def run(argv=None, save_main_session=True, test_pipeline=None) -> PipelineResult:
  ...
  if known_args.mode == 'streaming':
    threading.Thread(
      target=lambda: (time.sleep(900), run_load_pipeline(known_args, pipeline_args)),
      daemon=True).start()

  # StandardOptions
  pipeline_options = PipelineOptions(pipeline_args)
  pipeline_options.view_as(SetupOptions).save_main_session = save_main_session
  pipeline_options.view_as(StandardOptions).streaming = (known_args.mode == 'streaming')
  ...
  for bs in tried:
    try:
      model_handler = PytorchModelHandlerTensor(
        ...
        device=device,
        inference_batch_size=bs if bs is not None else 64,  # start guess for warmup
      )
      # quick warmup to validate memory (single dummy tensor)
      dummy = torch.zeros((3, known_args.image_size, known_args.image_size), dtype=torch.float32)
  ...
  if bs_ok is None:
    logging.warning("Falling back to batch_size=8 due to previous errors: %s", last_err)
    bs_ok = 8
    model_handler = PytorchModelHandlerTensor(
      model_class=lambda: create_timm_model(known_args.pretrained_model_name),
      model_params={},
  ...
  if known_args.mode == 'batch':
    pcoll = (pipeline
             | 'ReadURIsBatch' >> beam.Create(list(read_gcs_file_lines(known_args.input)))
    ...
    pcoll = (pipeline
        ...
        trigger=beam.trigger.AfterProcessingTime(known_args.trigger_proc_time_sec),
        accumulation_mode=beam.trigger.AccumulationMode.DISCARDING,
        allowed_lateness=0))

  keyed = (pcoll | 'MakeKey' >> beam.ParDo(MakeKeyDoFn(input_mode=known_args.input_mode)))
  ...
  if known_args.publish_to_big_query == 'true':
    ...
    _ = (results
        ...
        schema='image_id:STRING, model_name:STRING, topk:STRING, infer_ts_ms:INT64',
