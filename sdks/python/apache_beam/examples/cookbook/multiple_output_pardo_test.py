...
class MultipleOutputParDoTest(unittest.TestCase):
  ...
  EXPECTED_WORDS = ['fantastic: 1', 'point: 1', 'view: 
                    1', 'whole: 1', 'world: 1']
  def test_multiple_output_pardo(self):
    p = beam.Pipeline('DirectPipelineRunner')
    sample_text = p | beam.Create(self.SAMPLE_TEXT_Iterable)
    results = sample_text 
                    | beam.ParDo(multiple_output_pardo
                                 .SplitLinesToWordsFn())
                    .with_outputs('tag_short_words'
                                  , 'tag_character_count', main='words')
    result_count = (results.tag_character_count
                    | 'pair_with_key' >> beam.Map(lambda x
                                                  : ('chars_temp_key', x))
                    | beam.GroupByKey()
                    | 'count chars' >> beam.Map(lambda (_, counts)
                                                : sum(counts)))
    result_words = results.words 
                    | 'count words' >> multiple_output_pardo
                    .CountWords()
    result_short_words = results.tag_short_words 
                    | 'count short words' >> multiple_output_pardo
                    .CountWords()
    beam.assert_that(result_words, beam.equal_to(self.EXPECTED_WORDS))
    beam.assert_that(result_short_words
                     , beam.equal_to(self.EXPECTED_SHORT_WORDS)
                     , label='assert:tag_short_words')     
    beam.assert_that(result_count, beam.equal_to([self.text_len])
