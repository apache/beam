...
class OrderedWindowElementsDoFn(beam.DoFn):
  ...
  Attributes:
    ...
    BUFFER_MAX_TS_STATE: A `ReadModifyWriteStateSpec` (TimestampCoder) to
      keep track of the maximum timestamp currently present in the
      `buffer_state` for deciding whether to set further timers or not.
  ...
  BUFFER_MAX_TS_STATE = ReadModifyWriteStateSpec(
      'buffer_max_ts', TimestampCoder())
  ...
  def process(
      ...
      buffer_max_ts_state=beam.DoFn.StateParam(BUFFER_MAX_TS_STATE),
    ...
    Args:
      ...
      buffer_max_ts_state: The `ReadModifyWriteState` instance for the
        maximum timestamp in the buffer.
    ...
    buffer_max_ts = buffer_max_ts_state.read()
    if not buffer_max_ts or timestamp > buffer_max_ts:
      buffer_max_ts_state.write(timestamp)
  ...
  def _get_windowed_values_from_state(
      self, buffer_state, window_start_ts, window_end_ts):
    ...
    return [v[1] for v in windowed_values]
  ...
  def on_timer(
    ...
    if self.allowed_lateness > 0:
      ...
      while late_start_ts < window_start_ts:
        ...
        windowed_values = self._get_windowed_values_from_state(
            buffer_state, late_start_ts, late_end_ts)
    ...
    if buffer_max_ts and buffer_max_ts < next_window_start_ts:
      # buffer is empty, don't set the timer.
      buffer_max_ts_state.clear()
      buffer_min_ts_state.clear()
