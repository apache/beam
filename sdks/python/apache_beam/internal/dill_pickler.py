...
def _nested_type_wrapper(fun):
  ...
  def wrapper(pickler, obj):
    ...
    except _dill_internal.PicklingError:
...
_dill_internal.Pickler.dispatch[type] = _nested_type_wrapper(
    _dill_internal.Pickler.dispatch[type])
...
_dill_internal.Pickler.dispatch[types.GeneratorType] = _reject_generators

# This if guards against dill not being full initialized when generating docs.
if 'save_module' in dir(_dill_internal):
