...
def _nested_type_wrapper(fun):
  ...
  def wrapper(pickler, obj):
    ...
    except dill._dill.PicklingError:
      # pylint: disable=protected-access
      return pickler.save_reduce(
          dill._dill._create_type,
          (type(obj), obj.__name__, obj.__bases__, obj.__dict__.copy()),
          obj=obj)
...
dill._dill.Pickler.dispatch[type] = _nested_type_wrapper(
    dill._dill.Pickler.dispatch[type])
...
dill._dill.Pickler.dispatch[types.GeneratorType] = _reject_generators

# This if guards against dill not being full initialized when generating docs.
if 'save_module' in dir(dill._dill):

  # Always pickle non-main modules by name.
  old_save_module = dill._dill.save_module

  @dill._dill.register(dill._dill.ModuleType)
  def save_module(pickler, obj):
    if dill._dill.is_dill(pickler) and obj is pickler._main:
      return old_save_module(pickler, obj)
    else:
      dill._dill.logger.info('M2: %s' % obj)
      # pylint: disable=protected-access
      pickler.save_reduce(dill._dill._import_module, (obj.__name__, ), obj=obj)
      # pylint: enable=protected-access
      dill._dill.logger.info('# M2')

  # Pickle module dictionaries (commonly found in lambda's globals)
  # by referencing their module.
  old_save_module_dict = dill._dill.save_module_dict
  known_module_dicts = {
  }  # type: Dict[int, Tuple[types.ModuleType, Dict[str, Any]]]

  @dill._dill.register(dict)
        ...
        for m in list(sys.modules.values()):
          try:
            if (m and m.__name__ != '__main__' and
                isinstance(m, dill._dill.ModuleType)):
              d = m.__dict__
              known_module_dicts[id(d)] = m, d
          except AttributeError:
            # Skip modules that do not have the __name__ attribute.
            pass
    if obj_id in known_module_dicts and dill._dill.is_dill(pickler):
      m = known_module_dicts[obj_id][0]
      try:
        # pylint: disable=protected-access
        dill._dill._import_module(m.__name__)
        return pickler.save_reduce(
  ...
  dill._dill.save_module_dict = new_save_module_dict
  ...
  def _nest_dill_logging():
    ...
    """
    old_log_info = dill._dill.log.info
    ...
    dill._dill.logger.info = new_log_info
