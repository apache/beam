...
_dill = dill._dill  # pylint: disable=protected-access
    ...
    try:
      ...
      return pickler.save_reduce(
          ...
          (
              ...
              obj.__name__,
              obj.__bases__,
              _dill._dict_from_dictproxy(obj.__dict__)),
          obj=obj)
...
_dill.Pickler.dispatch[type] = _nested_type_wrapper(
    _dill.Pickler.dispatch[type])
...
_dill.Pickler.dispatch[types.GeneratorType] = _reject_generators

# This if guards against dill not being full initialized when generating docs.
if 'save_module' in dir(_dill):

  # Always pickle non-main modules by name.
  old_save_module = _dill.save_module

  @_dill.register(_dill.ModuleType)
  def save_module(pickler, obj):
    if _dill.is_dill(pickler) and obj is pickler._main:
      return old_save_module(pickler, obj)
    else:
      _dill.log.info('M2: %s' % obj)
      # pylint: disable=protected-access
      pickler.save_reduce(_dill._import_module, (obj.__name__, ), obj=obj)
      # pylint: enable=protected-access
      _dill.log.info('# M2')

  # Pickle module dictionaries (commonly found in lambda's globals)
  # by referencing their module.
  old_save_module_dict = _dill.save_module_dict
  known_module_dicts = {
        ...
        for m in list(sys.modules.values()):
          try:
            if (m and m.__name__ != '__main__' and
                isinstance(m, _dill.ModuleType)):
              d = m.__dict__
              known_module_dicts[id(d)] = m, d
          except AttributeError:
            # Skip modules that do not have the __name__ attribute.
            pass
    if obj_id in known_module_dicts and _dill.is_dill(pickler):
      m = known_module_dicts[obj_id][0]
      try:
        # pylint: disable=protected-access
        _dill._import_module(m.__name__)
        return pickler.save_reduce(
  ...
  _dill.save_module_dict = new_save_module_dict
...
def dumps(o, enable_trace=True, use_zlib=False):
  ...
  with _pickle_lock:
    try:
      ...
      if enable_trace:
        _dill._trace(True)  # pylint: disable=protected-access
        s = dill.dumps(o, byref=settings['dill_byref'])
      else:
        raise
    finally:
      _dill._trace(False)  # pylint: disable=protected-access
