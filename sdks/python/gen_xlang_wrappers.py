...
PY_WRAPPER_OUTPUT_DIR = os.path.join(
    PYTHON_SDK_ROOT, 'apache_beam', 'transforms', '_external_transforms')
...
def generate_transforms_config(input_services, output_file):
  ...
  `apache_beam/transforms/_external_transforms/io.py`. This contains all
  ...
  **destination** package:
  ...
  `apache_beam/transforms/_external_transforms/io.py`. With these modifications
  however, we instead use the provided name `MyCustomTransformName` and write
  it to `apache_beam/transforms/_external_transforms/io_gcp.py`.
