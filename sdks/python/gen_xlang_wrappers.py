...
def generate_transforms_config(input_services, output_file):
  ...
  for service in services:
    ...
    for identifier, wrapper in discovered.items():
      ...
      for sdk, destination in modified_transform.get('destinations', {}).items():
        if sdk not in SUPPORTED_SDK_DESTINATIONS:
          raise ValueError(
  ...
  with open(output_file, 'w') as f:
    ...
    f.write(
        "# Configs are generated based on the expansion service configuration \n"
        f"# in {input_services.replace(PROJECT_ROOT, '')}.\n")
    f.write(
        "# Refer to the utilities in sdks/python/gen_xlang_wrappers.py for more info.\n"
    )
